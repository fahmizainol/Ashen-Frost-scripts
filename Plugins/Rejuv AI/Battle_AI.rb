require_relative "./Plugins/Rejuv AI/adapted_fixes"
class AI_MonData
	...
	def initialize(trainer, index,battle)
		@trainer	= trainer # Supposedly Trainer Class
		@index 		= index
		@skill 		= trainer.nil? ? 0 : 100
...
class PokeBattle_AI
	...
	attr_accessor		:move					#Current move being scored						(Battle::Move)
	attr_accessor		:attacker				#User of the current move being scored			(Battle::Battler)
	attr_accessor		:opponent				#Opposing pokemon that the move will be used on	(Battle::Battler)
	...
	def initialize(battle)
		...
		@aiMoveMemory = {}
		player = @battle.playerTrainer
		opponent = @battle.opponentTrainer
	...
	def checkZMoves
		...
		for i in 0...@attacker.zmoves.length
			...
			if (move.move == :NATUREPOWER && @attacker.item == :NORMALIUMZ)
				newmove = Battle::Move.pbFromPBMove(@battle,PBMove.new(@battle.field.naturePower),@attacker)
				if newmove.basedamage > 0
					zmove = Battle::Move.pbFromPBMove(@battle,PBMove.new(PBStuff::CRYSTALTOZMOVE[PBStuff::TYPETOZCRYSTAL[newmove.type]]),@attacker,newmove)
					break 
				end
			end
	...
	def buildMoveScores
		...
		if !@battle.opponentTrainer && @battle.pbIsOpposing?(@index) && !(Rejuv && (@battle.battlers[@index].isbossmon || @battle.battlers[@index].issossmon)) #First check if this is a wild battle
		...
		if @battle.doublebattle	#this JUST gets the numbers. other things can be computed later.
			for monindex in 0...@battle.battlers.length
				...
				for moveindex in 0...@attacker.moves.length
					...
					if @move.basedamage != 0
						@mondata.roughdamagearray[monindex][moveindex] = [(pbRoughDamageRejuv*100)/(@opponent.hp),110].min
				...
				if @mondata.zmove && @attacker.pbPartner&.index != monindex
					...
					if @move.basedamage != 0 && @opponent.hp > 0
						@mondata.roughdamagearray[monindex][-1] = [(pbRoughDamageRejuv*100)/(@opponent.hp),110].min
					else
				...
				if has_to_struggle
					...
					@mondata.roughdamagearray[monindex][0] = [(pbRoughDamageRejuv*100)/(@opponent.hp),110].min
			...
			for moveindex in 0...@attacker.moves.length
				...
				@move = pbChangeMove(@attacker.moves[moveindex],@attacker)
				if @move.basedamage != 0	
					@mondata.roughdamagearray[0][moveindex] = [(pbRoughDamageRejuv*100)/(@opponent.hp),110].min
					#The old function makes some adjustments for two-turn moves here. I'm leaving that for later.
				else
			...
			if @mondata.zmove
				@move = @mondata.zmove
				if @move.basedamage != 0
					@mondata.roughdamagearray[0][-1] = [(pbRoughDamageRejuv*100)/(@opponent.hp),110].min
				else
			...
			if has_to_struggle
				@move = @battle.struggle
				@mondata.roughdamagearray[0][0] = [(pbRoughDamageRejuv*100)/(@opponent.hp),110].min
	...
	def findChoosableMoves(battler,mondata)
		...
		for moveindex in 0...4
			...
			if !@battle.opponentTrainer && @battle.pbIsOpposing?(battler.index) && !(battler.isbossmon || battler.issossmon)
		...
		if mondata.zmove && (chooseablemoves.all? {|array| array[:score] < 100} || [:CONVERSION,:CELEBRATE,:SPLASH,:CLANGOROUSSOULBLAZE].include?(mondata.zmove.move))
			...
			elsif originalmove.is_a?(Battle::Move)
	...
	def threatAssesment
		...
		@battle.battlers.each_with_index {|opp,i|
			...
			# Boosts
			threatscore[i]*= 1+0.2*opp.stages[:ATTACK] 		if opp.attack > opp.spatk
			threatscore[i]*= 1+0.2*opp.stages[:SPECIAL_ATTACK] 		if opp.spatk > opp.attack
			threatscore[i]*= 1+0.05*opp.stages[:DEFENSE] 	if aimons.any? {|mon| mon.attack > mon.spatk}
			threatscore[i]*= 1+0.05*opp.stages[:SPECIAL_DEFENSE] 		if aimons.any? {|mon| mon.spatk > mon.attack}
			threatscore[i]*= 1+0.10*opp.stages[:SPEED] 		if (opp.stages[:SPEED] > 0) ^ @battle.trickroom!=0
			threatscore[i]*= [1+0.20*opp.stages[:ACCURACY],0.3].max	if opp.stages[:ACCURACY] < 0
			threatscore[i]*= 1+0.20*opp.stages[:EVASION]	if opp.stages[:EVASION] >0
	...
	def getMoveScore(initialscores=[],scoreindex=-1)
		...
		if @move.pbIsPriorityMoveAI(@attacker) && @attacker != @opponent.pbPartner
			...
			if (@battle.doublebattle || (@opponent.status!=:SLEEP && @opponent.status!=:FROZEN && !@opponent.effects[PBEffects::Truant] && @opponent.effects[PBEffects::HyperBeam] == 0)) && !seedProtection?(@attacker) # This line might be in the wrong place, but we're trying our best here-- skip priority if opponent is incapacitated
				if score>100
					...
					score*=0.7
				elsif @attacker.crested == :VESPIQUEN && !aifaster && @attacker.effects[PBEffects::VespiCrest] == false
					score*=0.7
				end
				movedamage = -1
				opppri = false
				...
				if !aifaster || aifaster_partner===false || getAIMemory().any? {|moveloop| moveloop!=nil && moveloop.basedamage > 0 && moveloop.pbIsPriorityMoveAI(@opponent)} || getAIMemory(@opponent.pbPartner).any? {|moveloop| moveloop!=nil && moveloop.basedamage > 0 && moveloop.pbIsPriorityMoveAI(@opponent.pbPartner)} && @battle.doublebattle
					testmemory= getAIMemory() + [Battle::Move_FFF.new(@battle,@opponent, @opponent.type1)]
					testmemory= testmemory + [Battle::Move_FFF.new(@battle,@opponent, @opponent.type2)] if !@opponent.type2.nil?
					for i in testmemory
						tempdam = pbRoughDamageRejuv(i,@opponent,@attacker)
					...
					testmemory= getAIMemory(@opponent.pbPartner) + [Battle::Move_FFF.new(@battle,@opponent.pbPartner, @opponent.pbPartner&.type1)]
					testmemory= testmemory + [Battle::Move_FFF.new(@battle,@opponent.pbPartner, @opponent.pbPartner&.type2)] if !@opponent.pbPartner&.type2.nil?
					for i in testmemory
						tempdam = pbRoughDamageRejuv(i,@opponent.pbPartner,@attacker)
		...
		if !@move.zmove && @move.isSoundBased?
			$ai_log_data[@attacker.index].final_score_moves.push(0) if (@opponent.ability == :SOUNDPROOF && !moldBreakerCheck(@attacker)) || @attacker.effects[PBEffects::ThroatChop]!=0
			return 0 if (@opponent.ability == :SOUNDPROOF && !moldBreakerCheck(@attacker)) || @attacker.effects[PBEffects::ThroatChop]!=0
		...
		if checkAIhealing && !pbAIfaster?(@move) && @mondata.skill >= BESTSKILL && move.basedamage > 0
			...
			score*= [pbRoughDamageRejuv/newhp.to_f, 1.1].min
		end
		...
		if !@move.nil? && !@move.zmove && @move.highCritRate?
			if !(@opponent.ability == :SHELLARMOR || @opponent.ability == :BATTLEARMOR || @attacker.effects[PBEffects::LaserFocus]>0)
				boostercount = 0
				if @move.pbIsPhysical?()
					boostercount += @opponent.stages[:DEFENSE] if @opponent.stages[:DEFENSE]>0
					boostercount -= @attacker.stages[:ATTACK] if @attacker.stages[:ATTACK]<0
				elsif @move.pbIsSpecial?()
					boostercount += @opponent.stages[:SPECIAL_DEFENSE] if @opponent.stages[:SPECIAL_DEFENSE]>0
					boostercount -= @attacker.stages[:SPECIAL_ATTACK] if @attacker.stages[:SPECIAL_ATTACK]<0
				end
		...
		if !@move.zmove && @move.contactMove? && !(@attacker.item == :PROTECTIVEPADS) && @attacker.ability != :LONGREACH
			...
			when :CUTECHARM 	then contactscore*=0.8 if  @attacker.effects[PBEffects::Attract]<0 && initialscores.length>0 && initialscores[scoreindex] < 110
			when :ROUGHSKIN, :IRONBARBS then contactscore*= @attacker.hp < 0.2*@attacker.totalhp ? 0.5 : 0.85
			when :GOOEY, :TANGLINGHAIR, :COTTONDOWN
				if @attacker.pbCanReduceStatStage?(:SPEED)
					contactscore*=0.9
		...
		if @opponent.effects[PBEffects::TwoTurnAttack]!=0 && @mondata.skill>=HIGHSKILL
			...
			if (@move.accuracy>0 || @move.function==0xA5 || @move.zmove && @move.basedamage > 0 || @move.move == :WHIRLWIND) && (PBStuff::TWOTURNMOVE.include?(invulmove) || @opponent.effects[PBEffects::SkyDrop]) &&
		...
		accuracy=pbRoughAccuracyRejuv(@move,@attacker,@opponent)
		...
		case @move.function
			...
			when 0x12 # Fake Out
				if @attacker.turncount==0 && !(@opponent.effects[PBEffects::Substitute] > 0 || @opponent.ability == :INNERFOCUS || secondaryEffectNegated?())
					...
					score*=1.5 if (@attacker.pbPartner&.pbHasMove?(:TRICKROOM) && @battle.trickroom == 0) || (@attacker.pbPartner&.pbHasMove?(:TAILWIND) || @attacker.pbOwnSide.effects[PBEffects::Tailwind] == 0)
			...
			when 0x1a # Safeguard
				...
				if @attacker.pbOwnSide.effects[PBEffects::Safeguard]<=0 
					...
					if !@battle.opponentTrainer.is_a?(Array)
						if (@battle.opponentTrainer.trainertype==:CAMERUPT)
							score+=150
						end
					end
				end
			...
			when 0x1e # Defense Curl
				...
				score*=1.3 if @attacker.pbHasMove?(:ROLLOUT) && @attacker.effects[PBEffects::DefenseCurl]==false
			...
			when 0x21 # Charge
				...
				miniscore*=1.5 if @attacker.moves.any?{|moveloop| moveloop!=nil && moveloop.pbType(@attacker)==:ELECTRIC} && @attacker.effects[PBEffects::Charge]==0
			...
			when 0x4e # Captivate
				...
				if (agender==2 || ogender==2 || agender==ogender || @opponent.effects[PBEffects::Attract]>=0 || ((@opponent.ability == :OBLIVIOUS || @opponent.ability == :AROMAVEIL || @opponent.pbPartner&.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker)))
					miniscore = 0
				else
			...
			when 0x52 # Power Swap
				miniscore = statswapcode(:ATTACK,:SPECIAL_ATTACK)
			when 0x53 # Guard Swap
				miniscore = statswapcode(:DEFENSE,:SPECIAL_DEFENSE)
			when 0x54 # Heart Swap
				...
				miniscore *= splitcode(:HP) if @battle.FE == :NEWWORLD
			when 0x55 # Psych Up
				boostarray,droparray = psychupcode()
			...
			when 0x58 # Power Split
				miniscore = splitcode(:ATTACK)
			when 0x59 # Guard Split
				miniscore = splitcode(:DEFENSE)
			when 0x5a # Pain Split
				miniscore = splitcode(:HP)
			when 0x5b # Tailwind
			...
			when 0x7c # Smelling Salts
				if @opponent.status== :PARALYSIS  && @opponent.effects[PBEffects::Substitute]<=0
					score*=0.8
					score*=0.5 if @opponent.speed>@attacker.speed && @opponent.speed/2.0<@attacker.speed
				end
			when 0x7d # Wake-up Slap
				if @opponent.status== :SLEEP && @opponent.effects[PBEffects::Substitute]<=0
					score*=0.8
			...
			when 0x88 # Pursuit
				...
				if @attacker.stages[:SPEED]!=6 && score>=100
					miniscore*=1.5
			...
			when 0x91 # Fury Cutter
				miniscore = echocode()
				miniscore *= (1 + 0.15 * @attacker.stages[:ACCURACY])
				miniscore *= (1 - 0.08 * @opponent.stages[:EVASION])
			...
			when 0x96 # Natural Gift
				score*=0 if @attacker.item.nil? || !pbIsBerry?(@attacker.item) || @attacker.ability == :KLUTZ || @battle.state.effects[PBEffects::MagicRoom]>0 || @attacker.effects[PBEffects::Embargo]>0 || (@opponent.ability == :UNNERVE || @opponent.ability == :ASONE)
			...
			when 0xa1 # Lucky Chant
				score+=20 if @attacker.pbOwnSide.effects[PBEffects::LuckyChant]==0  && @attacker.ability != :BATTLEARMOR || @attacker.ability != :SHELLARMOR && (@opponent.effects[PBEffects::FocusEnergy]>1 || @opponent.effects[PBEffects::LaserFocus]>0)
			when 0xa2 # Reflect
			...
			when 0xa6 # Lock On, Mind Reader
				...
				if @battle.FE == :PSYTERRAIN && @move.move == :MINDREADER
					...
					score+=10 if @attacker.stages[:SPECIAL_ATTACK]<6
				end
			...
			when 0xc3 # Weasel Slash
				miniscore = weaselslashcode() unless @battle.FE == :CLOUDS || @battle.FE == :SKY || (Rejuv && @battle.FE == :GRASSY) || (@battle.state.effects[PBEffects::GRASSY] > 0)
			...
			when 0xce # Sky Drop
				...
				miniscore=0 if @battle.state.effects[PBEffects::Gravity]!=0 || @battle.FE == :CAVE
			when 0xcf # Fire Spin, Magma Storm, Sand Tomb, Bind, Clamp, Wrap, Infestation, Thunder Cage, Snap Trap
				...
				when :SANDTOMB
					...
					score+=10*oppstatdrop([0,0,0,0,0,1,0]) unless opponent.stages[:ACCURACY]<(-2) if @battle.FE == :ASHENBEACH
				when :INFESTATION
			...
			when 0xd0 # Whirlpool
				miniscore = firespincode()
				miniscore*=1.3 if !$cache.moves[opponent.effects[PBEffects::TwoTurnAttack]].nil? && 
				$cache.moves[@opponent.effects[PBEffects::TwoTurnAttack]].function==0xCB
				miniscore*=0.7 if @battle.FE == :ASHENBEACH
				if @battle.FE == :WATERSURFACE || @battle.FE == :UNDERWATER
					miniscore*=1.3
					miniscore*=confucode() if !Rejuv && opponent.effects[PBEffects::Confusion]<=0
				end
			...
			when 0xd8 # Synthesis, Moonlight, Morning Sun
				...
				recoveramount = (@attacker.totalhp*0.25).floor  if @battle.pbWeather != 0 && !@attacker.hasWorkingItem(:UTILITYUMBRELLA)
			...
			when 0x100 # Rain Dance
				...
				if !@battle.opponentTrainer.is_a?(Array)
					if Reborn && (@battle.opponentTrainer.trainertype==:SHELLY) && (@battle.FE == :GRASSY || @battle.FE == :FOREST || @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN)) # Shelly
						miniscore *= 4
			...
			when 0x119 # Magnet Rise
				miniscore = magnocode()
				miniscore*=1.3 if @mondata.skill>=BESTSKILL && (@battle.FE == :ELECTERRAIN || @battle.FE == :FACTORY || @battle.FE == :SHORTCIRCUIT || @battle.state.effects[PBEffects::ELECTERRAIN] > 0)
			...
			when 0x11d # After You
				...
				if @battle.opponentTrainer.is_a?(Array) && @battle.opponentTrainer.any? {|opp| opp.trainertype == :UMBNOEL } &&
			...
			when 0x137 # Flying Press
				#score*=2 if opponent.effects[PBEffects::Minimize] #handled in pbRoughDamageRejuv from now on
				miniscore = 0 if @battle.state.effects[PBEffects::Gravity]!=0
			...
			when 0x13a # Aromatic Mist
				miniscore=arocode(:SPECIAL_DEFENSE)
			...
			when 0x146 # Magnetic Flux
				if !(@attacker.ability == :PLUS || @attacker.ability == :MINUS || @attacker.pbPartner&.ability == :PLUS || @attacker.pbPartner&.ability == :MINUS)
				...
				elsif @attacker.pbPartner&.stages[:SPECIAL_DEFENSE]!=6 && @attacker.pbPartner&.stages[:DEFENSE]!=6
					miniscore=0.7
					...
					miniscore*=0.3 if @attacker.pbPartner&.effects[PBEffects::Yawn]>0 || @attacker.pbPartner&.effects[PBEffects::LeechSeed]>=0 || @attacker.pbPartner&.effects[PBEffects::Attract]>=0 || !@attacker.pbPartner&.status.nil?
			...
			when 0x147 # Fell Stinger
				if @attacker.stages[:ATTACK]!=6 && score>=100
					miniscore = 2.0
					miniscore*=2 if pbAIfaster?(@move)
				end
			...
			when 0x150 # Flower Shield
				miniscore = arocode(:DEFENSE)
				score = flowershieldcode(score)

			when 0x151 # Rototiller
				miniscore = arocode(:ATTACK)
			...
			when 0x17C # Tar Shot
				miniscore=oppstatdrop([0,0,0,0,1,0,0])
				if !@opponent.effects[PBEffects::TarShot] && (PBTypes.twoTypeEff(:FIRE,@opponent.type1,@opponent.type2) != 0) && @opponent.ability != :FLASHFIRE
			...
			when 0x313 # Burning Jealousy
				miniscore = burncode() if (checkAImoves(PBStuff::SETUPMOVE) && !pbAIfaster?()) || @opponent.effects[PBEffects::Jealousy]
			when 0x314 # Lash Out
				miniscore = 1.5 if (checkAImoves(PBStuff::STATNERFMOVE) && !pbAIfaster?()) && !@attacker.effects[PBEffects::LashOut]
...
#All functions here return a modifier to the original score, similar to miniscore
	...
	def poisoncode
		...
		miniscore=1.2
		ministat=0
		ministat+=@opponent.stages[:DEFENSE]
		ministat+=@opponent.stages[:SPECIAL_DEFENSE]
		ministat+=@opponent.stages[:EVASION]
	...
	def paracode
		...
		miniscore*=1.2 if @opponent.hp==@opponent.totalhp
		ministat= @opponent.stages[:ATTACK] + @opponent.stages[:SPECIAL_ATTACK] + @opponent.stages[:SPEED]
		miniscore*=1+0.05*ministat if ministat>0
		miniscore*=0.3 if @opponent.ability == :NATURALCURE
		...
		if !pbAIfaster?() && (pbRoughStatRejuv(@opponent,:SPEED)/2.0)<@attacker.pbSpeed && @battle.trickroom==0
			miniscore*=1.2
		elsif pbAIfaster?() && (pbRoughStatRejuv(@opponent,:SPEED)/2.0)<@attacker.pbSpeed && @battle.trickroom>1
			miniscore*=0.7
		end
		if pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK)>pbRoughStatRejuv(@opponent,:ATTACK)
			miniscore*=1.1
		end
	...
	def burncode
		...
		miniscore=1.2
		ministat=0
		ministat+=@opponent.stages[:ATTACK]
		ministat+=@opponent.stages[:SPECIAL_ATTACK]
		ministat+=@opponent.stages[:SPEED]
		miniscore*=1+0.05*ministat if ministat>0
		miniscore*=0.3 if @opponent.ability == :NATURALCURE
		miniscore*=0.7 if @opponent.ability == :MARVELSCALE
		...
		if pbRoughStatRejuv(@opponent,:ATTACK)>pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK)
			miniscore*=1.4
		end
		miniscore*=0.4 if @opponent.effects[PBEffects::Yawn]>0
		miniscore = pbSereneGraceCheck(miniscore) if @move.basedamage>0
	...
	def flinchcode
		return @move.basedamage > 0 ? 1 : 0 if @opponent.effects[PBEffects::Substitute] > 0 || @opponent.ability == :INNERFOCUS || secondaryEffectNegated?()
		return @move.basedamage > 0 ? 1 : 0 if !pbAIfaster?(@move)
		miniscore = 1.0
		miniscore*= 1.3 if !hasgreatmoves()
		miniscore*= 1.3 if @battle.trickroom > 0 && @attacker.pbSpeed > pbRoughStatRejuv(@opponent,:SPEED)
		...
		miniscore*= 1.2 if @attacker.pbOpposingSide.effects[PBEffects::Tailwind] > 0
	...
	def thunderboostcode
		miniscore = 1.0
		invulmove=$cache.moves[@opponent.effects[PBEffects::TwoTurnAttack]].function rescue nil
	...
	def confucode
		...
		miniscore*=1+0.1*@opponent.stages[:ATTACK] if @opponent.stages[:ATTACK] > 0
		if pbRoughStatRejuv(@opponent,:ATTACK)>pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK)
			miniscore*=1.2
		end
	...
	def attractcode
		agender=@attacker.gender
		ogender=@opponent.gender
		return 0 if (agender==2 || ogender==2 || agender==ogender || @opponent.effects[PBEffects::Attract]>=0 || ((@opponent.ability == :OBLIVIOUS || @opponent.ability == :AROMAVEIL || @opponent.pbPartner&.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker)))
		miniscore=1.2
		miniscore*=0.7 if @attacker.ability == :CUTECHARM
		miniscore*=1.3 if @mondata.roles.include?(:PHYSICALWALL) || @mondata.roles.include?(:SPECIALWALL)
		miniscore*=1.1 if @opponent.effects[PBEffects::Confusion]>0
		miniscore*=1.1 if @opponent.status== :PARALYSIS
		miniscore*=0.5 if @opponent.effects[PBEffects::Yawn]>0 || @opponent.status== :SLEEP
	...
	def psychocode
		return 0 if @attacker.status.nil? || !@opponent.status.nil? || @opponent.effects[PBEffects::Substitute]>0 || @opponent.effects[PBEffects::Yawn]!=0
		...
		if @attacker.status== :BURN && @opponent.pbCanBurn?(false)
			miniscore*=1.2 if pbRoughStatRejuv(@opponent,:ATTACK)>pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK)
			miniscore*=0.7 if @opponent.ability == :FLAREBOOST
		end
		if @attacker.status== :PARALYSIS && @opponent.pbCanParalyze?(false)
			miniscore*=1.1 if pbRoughStatRejuv(@opponent,:ATTACK)<pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK)
	...
	def selfstatboost(stats)
		...
		if stats[:ATTACK] != 0 || stats[:SPECIAL_ATTACK] != 0
			for j in @attacker.moves
				next if j.nil?
				specmove=true if j.pbIsSpecial?()
				physmove=true if j.pbIsPhysical?()
			end
			stats[:ATTACK] = 0 if !physmove && @battle.FE != :PSYTERRAIN
			stats[:SPECIAL_ATTACK] = 0 if !specmove
		end
		...
		if @move.basedamage == 0
			...
			if (stats[:ATTACK] > 0 || stats[:SPECIAL_ATTACK] > 0) && (stats[:SPEED] > 0) # Speed and offense i.e dragon dance
				miniscore *= 1.8 
			elsif (stats[:ATTACK] > 1 || stats[:SPECIAL_ATTACK] > 1) # Double offense i.e swords dance, nasty plot
				miniscore *= 1.5 
			elsif (stats[:ATTACK] > 0 || stats[:SPECIAL_ATTACK] > 0) && (stats[:DEFENSE] > 0 || stats[:SPECIAL_DEFENSE] > 0) # Defense and offense i.e bulk up
				miniscore *= 1.5 
			elsif (stats[:DEFENSE] > 0 && stats[:SPECIAL_DEFENSE] > 0) # Both defenses i.e cosmic power
				miniscore *= 1.5 
			end	
		end
		for i in 1...stats.length
			...
			case i
				...
				when :SPEED
					...
					if pbAIfaster?()
						miniscore*=0.8
						miniscore*=0.2 if statsboosted == stats[:SPEED]
					end
					...
					if @attacker.moves.any? {|moveloop| moveloop!=nil && moveloop.basedamage > 0 && moveloop.priority < 1 && @mondata.roughdamagearray.transpose[@attacker.moves.find_index(moveloop)].max > 10 } # thank u perry 4 saving me									
						...
						if @attacker.attack<@attacker.spatk
							miniscore*=(1+0.05*@attacker.stages[:SPECIAL_ATTACK]) if @attacker.stages[:SPECIAL_ATTACK]<0
						else
							miniscore*=(1+0.05*@attacker.stages[:SPECIAL_ATTACK]) if @attacker.stages[:ATTACK]<0
						end
						ministat=0
						ministat+=@opponent.stages[:DEFENSE]
						ministat+=@opponent.stages[:SPECIAL_DEFENSE]
				...
				when :ACCURACY
					...
					miniscore*=(1+0.05*@opponent.stages[:EVASION]) if @opponent.stages[:EVASION]>0
		...
		if seedProtection?(@attacker) || (@attacker.effects[PBEffects::Substitute]>0 || ((@attacker.effects[PBEffects::Disguise] || (@attacker.effects[PBEffects::IceFace] && (@opponent.attack > @opponent.spatk || @battle.FE == :FROZENDIMENSION))) && !moldBreakerCheck(@opponent)) && !@battle.doublebattle)
			miniscore*=1.3
		end
		miniscore*=0.5 if (@opponent.effects[PBEffects::Substitute]>0 || ((@opponent.effects[PBEffects::Disguise] || (@opponent.effects[PBEffects::IceFace] && (@attacker.attack > @attacker.spatk || @battle.FE == :FROZENDIMENSION))) && !moldBreakerCheck(@attacker)))
		...
		if @mondata.skill>=MEDIUMSKILL
			...
			if maxdam < (@attacker.hp/4.0) && sweep
				...
				miniscore*=1.1
			elsif maxdam < (@attacker.hp/4.0) && (stats[:DEFENSE] != 0 && stats[:SPECIAL_DEFENSE] != 0) #cosmic power
				miniscore*=1.5
				...
				miniscore*=0.1 if maxdam > @attacker.hp && !(@attacker.effects[PBEffects::Substitute]>0 || ((@attacker.effects[PBEffects::Disguise] || (@attacker.effects[PBEffects::IceFace] && (@opponent.attack > @opponent.spatk || @battle.FE == :FROZENDIMENSION))) && !moldBreakerCheck(@opponent)) || seedProtection?(@attacker))
			end

			if maxdam * 2 > @attacker.hp + (hpGainPerTurn(@attacker)-1)*@attacker.totalhp && (stats[:ATTACK]==1 || stats[:SPECIAL_ATTACK]==1) && 
				((bestmove.pbIsPhysical?(bestmove.type) && stats[:DEFENSE] == 0) || (bestmove.pbIsSpecial?(bestmove.type) && stats[:SPECIAL_DEFENSE] == 0)) && move.basedamage == 0
				miniscore*=0.4
			end
		end
		...
		if sweep
			...
			if @attacker.stages[:SPEED]<0 && stats[:SPEED]==0
				miniscore*=(1+0.05*@attacker.stages[:SPEED])
			end
			...
			if @attacker.status== :PARALYSIS
				miniscore*=0.5
				miniscore*=0.5 if stats[:SPEED] != 0 #stacks
			end
			...
			miniscore*=0.6 if @attacker.effects[PBEffects::Toxic]>0 && @attacker.ability!=:POISONHEAL && @attacker.crested != :ZANGOOSE
			miniscore*=0.6 if checkAIpriority()
			miniscore*=0.6 if (@opponent.ability == :SPEEDBOOST || (((@opponent.ability == :MOTORDRIVE && @battle.FE == :ELECTERRAIN) || (@opponent.ability == :STEAMENGINE && [:UNDERWATER,:WATERSURFACE,:VOLCANIC,:VOLCANICTOP,:INFERNAL].include?(@battle.FE))) && (stats[:SPEED] < 2)))
			miniscore*=1.4 if notOHKO?(@attacker,@opponent)
		else
			...
			if @opponent.effects[PBEffects::Encore]>0
				if @opponent.moves[(@opponent.effects[PBEffects::EncoreIndex])].basedamage==0
					miniscore*=1.3
					miniscore*=1.2 if (stats[:DEFENSE] != 0 && stats[:SPECIAL_DEFENSE] != 0)
				end
			end
			miniscore*=0.2 if checkAImoves(PBStuff::PHASEMOVE)
			miniscore*=0.7 if @attacker.status== :POISON && @attacker.ability!=:POISONHEAL && @attacker.crested != :ZANGOOSE
			miniscore*=0.2 if @attacker.effects[PBEffects::Toxic]>0 && @attacker.ability!=:POISONHEAL && @attacker.crested != :ZANGOOSE
			miniscore*=0.3 if @attacker.effects[PBEffects::LeechSeed]>=0 || @attacker.effects[PBEffects::Attract]>=0
		end
		if tank
			if @attacker.stages[:SPECIAL_DEFENSE]>0 || @attacker.stages[:DEFENSE]>0
				ministat=0
				ministat+=@attacker.stages[:SPECIAL_DEFENSE] if stats[:SPECIAL_DEFENSE] != 0
				ministat+=@attacker.stages[:DEFENSE] if stats[:DEFENSE] != 0
				miniscore*=(1-0.05*ministat)
			end
		...
		if @attacker.effects[PBEffects::Confusion]>0
			if stats[:ATTACK] != 0 #if move boosts attack
				miniscore*=0.2
				miniscore*=0.5 if stats[:ATTACK] > 1 #using swords dance or shell smash while confused is Extra Bad
				miniscore*=1.5 if stats[:DEFENSE] != 0#adds a correction for moves that boost attack and defense
			else
				miniscore*=0.5
			end
		end
	...
	def selfstatdrop(stats,score)
		...
		if stats[:ATTACK] != 0 #Basically just to catch superpower
			if score<100
				miniscore*=0.9
				if !pbAIfaster?()
					miniscore*=1.1
		...
		elsif stats[:DEFENSE] != 0 || stats[:SPECIAL_DEFENSE] != 0
			if score<100
				miniscore*=0.9
				miniscore*=0.9 if stats[:SPEED] != 0
				if !pbAIfaster?()
					miniscore*=1.1
				else
		...
		elsif stats[:SPECIAL_ATTACK] != 0
			if @mondata.skill>=BESTSKILL && @battle.FE == :GLITCH && @attacker.getSpecialStat(which_is_higher: true) == :SPECIAL_DEFENSE
				miniscore*=1.4
			elsif score<100
				miniscore*=0.9
				miniscore*=0.5 if checkAIhealing()
	...
	def oppstatboost(stats)
		...
		if @opponent.index != @attacker.pbPartner&.index
			...
			miniscore*=1.5 if (@opponent.attack<@opponent.spatk && stats[:ATTACK] != 0) || (@opponent.attack>@opponent.spatk && stats[:SPECIAL_ATTACK] != 0)
			miniscore*=0.3 if (1.0/@opponent.totalhp)*@opponent.hp < 0.6
			if @opponent.effects[PBEffects::Attract]>=0 || @opponent.status== :PARALYSIS || @opponent.effects[PBEffects::Yawn]>0 || @opponent.status== :SLEEP
				miniscore*=0.3
			end
	...
	def oppstatdrop(stats)
		...
		if stats[:DEFENSE]>0 || stats[:SPECIAL_DEFENSE]>0
			for j in @attacker.moves
				next if j.nil?
				specmove=true if j.pbIsSpecial?()
				physmove=true if j.pbIsPhysical?()
			end
			stats[:DEFENSE] = 0 if !physmove
			stats[:SPECIAL_DEFENSE] = 0 if !specmove
		end
		if stats[:ATTACK]>0 || stats[:SPECIAL_ATTACK]>0
			bestmove = checkAIbestMove()
			stats[:SPECIAL_ATTACK] = 0 if (pbRoughStatRejuv(@opponent,:ATTACK)*0.9>pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK)) && bestmove.pbIsPhysical?()
			stats[:ATTACK] = 0 if (pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK)*0.9>pbRoughStatRejuv(@opponent,:ATTACK)) && bestmove.pbIsSpecial?()
		end
		if stats[:SPEED] > 0 
			stats[:SPEED] = 0 if pbAIfaster?() && @battle.trickroom == 0
			stats[:SPEED] = 0 if @battle.trickroom > 1
		end
		...
		if stats[:DEFENSE]>0 || stats[:SPECIAL_DEFENSE]>0    #defense stuff
			miniscore*=1.1
			miniscore*=1.2 if checkAIdamage() < @opponent.hp
			miniscore*=1.5 if @move.function == 0x4C
		else			#non-defense stuff
			if @mondata.roles.include?(:PHYSICALWALL) || @mondata.roles.include?(:SPECIALWALL)
				miniscore*=1.3 if stats[:ATTACK]>0 || stats[:SPECIAL_ATTACK]>0 || stats[:ACCURACY]>0
				miniscore*=1.1 if stats[:SPEED]>0
			end
		end
		if stats[:SPEED]>0  #speed stuff
			if (pbRoughStatRejuv(@opponent,:SPEED)*0.66) > @attacker.pbSpeed
				miniscore*= hasgreatmoves() ? 1 : 1.5
			end
			miniscore*=1+0.05*@opponent.stages[:SPEED] if @opponent.stages[:SPEED]<0 && !secondaryEffectNegated?()
		...
		if stats[:DEFENSE]>0 || stats[:SPECIAL_ATTACK]>0 || stats[:SPECIAL_DEFENSE]>0
		...
		if stats[:SPEED]>0
			...
			miniscore*=1.5 if @attacker.crested == :ARIADOS && @opponent.stages[:SPEED] > -1
			miniscore*=0.5 if checkAImoves([:GYROBALL])
			miniscore*=0.1 if  @battle.trickroom!=0 || checkAImoves([:TRICKROOM])
		end
		if @battle.pbPokemonCount(@battle.pbParty(@opponent.index))==1 || @attacker.ability == :SHADOWTAG || @opponent.effects[PBEffects::MeanLook]>0
			miniscore*=1.2
		end
	...
	def defogcode
		...
		if @move == :COURTCHANGE 
			miniscore*=3 if @opponent.pbOwnSide.effects[PBEffects::Tailwind]>0

			miniscore*=0.5 if @attacker.pbOwnSide.effects[PBEffects::Reflect]>0
			miniscore*=0.5 if @attacker.pbOwnSide.effects[PBEffects::LightScreen]>0
			miniscore*=0.7 if @attacker.pbOwnSide.effects[PBEffects::Safeguard]>0
			miniscore*=0.3 if @attacker.pbOwnSide.effects[PBEffects::AuroraVeil]>0
			miniscore*=0.7 if @attacker.pbOwnSide.effects[PBEffects::Mist]>0
			miniscore*=0.2 if @attacker.pbOwnSide.effects[PBEffects::Tailwind]>0
		end
		return miniscore
	end
	...
	def volcanoeruptioncode
		...
		miniscore = 1.0
		if PBTypes.twoTypeEff(:FIRE,@attacker.type1,@attacker.type2) == 0 || [:MAGMAARMOR,:FLASHFIRE,:FLAREBOOST,:BLAZE,:FLAMEBODY,:SOLIDROCK,:STURDY,:BATTLEARMOR,:SHELLARMOR,:WATERBUBBLE,:MAGICGUARD,:WONDERGUARD,:PRISMARMOR].include?(@attacker.ability) || @attacker.effects[PBEffects::AquaRing]
			miniscore*=1.0
		...
		if !@attacker.pbPartner&.pokemon.nil?
			if PBTypes.twoTypeEff(:FIRE,@attacker.pbPartner&.type1,@attacker.pbPartner&.type2) == 0 || [:MAGMAARMOR,:FLASHFIRE,:FLAREBOOST,:BLAZE,:FLAMEBODY,:SOLIDROCK,:STURDY,:BATTLEARMOR,:SHELLARMOR,:WATERBUBBLE,:MAGICGUARD,:WONDERGUARD,:PRISMARMOR].include?(@attacker.pbPartner&.ability) || @attacker.pbPartner&.effects[PBEffects::AquaRing]
				miniscore*=1.0
		...
		miniscore*=1.5 if (@attacker.ability == :FLASHFIRE && !@attacker.effects[PBEffects::FlashFire]) 
		miniscore*=1.5 if (@attacker.pbPartner&.ability == :FLASHFIRE && !@attacker.pbPartner&.effects[PBEffects::FlashFire])
		miniscore*=1.5 if (@attacker.ability == :BLAZE && !@attacker.effects[PBEffects::Blazed]) 
		miniscore*=1.5 if (@attacker.pbPartner&.ability == :BLAZE && !@attacker.pbPartner&.effects[PBEffects::Blazed])
		...
		miniscore*=0.5 if (@opponent.ability == :FLASHFIRE && !@opponent.effects[PBEffects::FlashFire]) 
		miniscore*=0.5 if (@opponent.pbPartner&.ability == :FLASHFIRE && !@opponent.pbPartner&.effects[PBEffects::FlashFire])
		miniscore*=0.5 if (@opponent.ability == :BLAZE && !@opponent.effects[PBEffects::Blazed]) 
		miniscore*=0.5 if (@opponent.pbPartner&.ability == :BLAZE && !@opponent.pbPartner&.effects[PBEffects::Blazed])
		...
		miniscore*=1.3 if @attacker.effects[PBEffects::LeechSeed]>=0 
		miniscore*=1.3 if @attacker.pbPartner&.effects[PBEffects::LeechSeed]>=0
		miniscore*=0.8 if @opponent.effects[PBEffects::LeechSeed]>=0 
		miniscore*=0.8 if @opponent.pbPartner&.effects[PBEffects::LeechSeed]>=0
		yourpartycount = @battle.pbPokemonCount(@battle.pbPartySingleOwner(@attacker.index))
		theirpartycount = @battle.pbPokemonCount(@battle.pbPartySingleOwner(@opponent.index))
		minimini1 = 1.0
		if yourpartycount>1
			minimini1*=1.5 if @attacker.pbOwnSide.effects[PBEffects::StealthRock]
			minimini1*=2 if @attacker.pbOwnSide.effects[PBEffects::StickyWeb]
			minimini1*=(1.3**@attacker.pbOwnSide.effects[PBEffects::Spikes])
			minimini1*=(1.5**@attacker.pbOwnSide.effects[PBEffects::ToxicSpikes])
		end
		minimini1 = -1.0
		minimini1*=(yourpartycount-1) if yourpartycount>1
		minimini2 = 1.0
		if theirpartycount>1
			minimini2*=0.75 if @opponent.pbOwnSide.effects[PBEffects::StealthRock]
			minimini2*=0.5 if @opponent.pbOwnSide.effects[PBEffects::StickyWeb]
			minimini2*=(0.9**@opponent.pbOwnSide.effects[PBEffects::Spikes])
			minimini2*=(0.85**@opponent.pbOwnSide.effects[PBEffects::ToxicSpikes])
		end
		minimini2 -= 1.0
	...
	def statswapcode(stat1,stat2)
		...
		if (pbRoughStatRejuv(@attacker,stat1)>pbRoughStatRejuv(@attacker,stat2))
		...
		if (pbRoughStatRejuv(@opponent,stat1)>pbRoughStatRejuv(@opponent,stat2))
	...
	def psychupcode
		...
		statarray[0] = (@opponent.stages[:ATTACK]-@attacker.stages[:ATTACK])
		statarray[1] = (@opponent.stages[:DEFENSE]-@attacker.stages[:DEFENSE])
		statarray[2] = (@opponent.stages[:SPECIAL_ATTACK]-@attacker.stages[:SPECIAL_ATTACK])
		statarray[3] = (@opponent.stages[:SPECIAL_DEFENSE]-@attacker.stages[:SPECIAL_DEFENSE])
		statarray[4] = (@opponent.stages[:SPEED]-@attacker.stages[:SPEED])
		statarray[5] = (@opponent.stages[:ACCURACY]-@attacker.stages[:ACCURACY])
		statarray[6] = (@opponent.stages[:EVASION]-@attacker.stages[:EVASION])
		for i in 0..6
	...
	def powertrickcode
		...
		if @attacker.attack - @attacker.defense >= 100
			...
			miniscore*=2 if pbRoughStatRejuv(@opponent,:ATTACK)>pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK)
	...
	def tailwindcode
		...
		miniscore*=2.5 if !@battle.opponentTrainer.nil? && !@battle.opponentTrainer.is_a?(Array) && @battle.opponentTrainer.trainertype==:ADRIENN
		return miniscore
	end
	...
	def ohkode
		...
		return 3.5 if @opponent.effects[PBEffects::LockOn]>0 || @opponent.ability==:NOGUARD || @attacker.ability==:NOGUARD || (@attacker.ability==:FAIRYAURA && @battle.FE == :FAIRYTALE)
		return 0.7
	end
	...
	def counterattackcode
		...
		if @move.function == 0x71 # Counter
			if pbRoughStatRejuv(@opponent,:ATTACK) > (pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK) * 1.1) # attack is at least 10% higher than sp.atk
				miniscore*=1.1
			elsif pbRoughStatRejuv(@opponent,:ATTACK)<(pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK) * 0.6)
				miniscore*=0.3
			else 
				miniscore*=0.6
			end
			...
			if (pbRoughStatRejuv(@opponent,:ATTACK) * 1.1)<pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK) # attack is at least 10% higher than sp.atk
				miniscore*=1.1
			elsif (pbRoughStatRejuv(@opponent,:ATTACK) * 0.6)>pbRoughStatRejuv(@opponent,:SPECIAL_ATTACK)
				miniscore*=0.3
			else 
				miniscore*=0.6
			end
	...
	def permacritcode(initialscore)
		return 0 if @opponent.index == @attacker.pbPartner&.index && (@opponent.ability != :ANGERPOINT || @opponent.stages[:ATTACK]==6)
		...
		miniscore = 1.0
		miniscore += 0.1 * @opponent.stages[:DEFENSE] if @opponent.stages[:DEFENSE]>0
		miniscore += 0.1 * @opponent.stages[:SPECIAL_DEFENSE] if @opponent.stages[:SPECIAL_DEFENSE]>0
		miniscore -= 0.1 * @attacker.stages[:ATTACK] if @attacker.stages[:ATTACK]<0
		miniscore -= 0.1 * @attacker.stages[:SPECIAL_ATTACK] if @attacker.stages[:SPECIAL_ATTACK]<0
		miniscore -= 0.1 * @attacker.effects[PBEffects::FocusEnergy] if @attacker.effects[PBEffects::FocusEnergy]>0
		return miniscore if !(@opponent.ability == :ANGERPOINT && @opponent.stages[:ATTACK]!=6)
	...
	def screencode
		return 0 if (@attacker.pbOwnSide.effects[PBEffects::Reflect]>0 && @move.function == 0xA2) || (@attacker.pbOwnSide.effects[PBEffects::LightScreen]>0 && @move.function == 0xA3) || (@attacker.pbOwnSide.effects[PBEffects::AreniteWall]>0 && @move.function == 0x203)
		return 0 if @attacker.pbOwnSide.effects[PBEffects::AuroraVeil]>0 && (@move.function == 0x15b || @move.function == 0x80C)
		...
		miniscore=1.2
		miniscore*=0.2 if @attacker.pbOwnSide.effects[PBEffects::AuroraVeil]>0 && @move.function != 0x203 # Arenite Wall applies seperately
			...
			if @mondata.skill>=MEDIUMSKILL
				if getAIMemory().length > 0
					...
					for j in getAIMemory()
						...
						tempdam = pbRoughDamageRejuv(j,@opponent,@attacker)
	...
	def nevermisscode(score)
		...
		miniscore*= (1 - 0.05*@attacker.stages[:ACCURACY]) if @attacker.stages[:ACCURACY]<0
		miniscore*= (1 + 0.05*@opponent.stages[:EVASION]) if @opponent.stages[:EVASION]>0
	...
	def lockoncode
		return 0 if @opponent.effects[PBEffects::LockOn]>0 || @opponent.effects[PBEffects::Substitute]>0 || @attacker.ability == :NOGUARD && @opponent.ability == :NOGUARD || (@attacker.ability==:FAIRYAURA && @battle.FE == :FAIRYTALE)
		miniscore=1.0
		...
		ministat = (@attacker.stages[:ACCURACY]<0) ? @attacker.stages[:ACCURACY] : 0
		miniscore*=1 + 0.1*ministat
		miniscore*=1 + 0.1*@opponent.stages[:EVASION]
		return miniscore
	end

	def forecode5me #after doing hundreds of these this is how i survive
		return 0 if @opponent.effects[PBEffects::Foresight]
		ministat = (@opponent.stages[:EVASION]>0) ? @opponent.stages[:EVASION] : 0
	...
	def chipcode
		ministat = 0
		ministat+=@opponent.stages[:EVASION] if @opponent.stages[:EVASION]>0
		ministat+=@opponent.stages[:DEFENSE] if @opponent.stages[:DEFENSE]>0
		ministat+=@opponent.stages[:SPECIAL_DEFENSE]   if @opponent.stages[:SPECIAL_DEFENSE]>0
		miniscore=1 + 0.05*ministat
		return miniscore
	end
	...
	def protectcode
		...
		if (@attacker.ability == :SHADOWTAG || @attacker.pbPartner&.ability == :SHADOWTAG || @opponent.effects[PBEffects::MeanLook])
			miniscore*=4 if @opponent.effects[PBEffects::PerishSong]==3
			miniscore*=8 if @opponent.effects[PBEffects::PerishSong]==1
		end
		if @opponent.effects[PBEffects::FutureSight] == 1
			miniscore *= 4
		...
		miniscore/=(@attacker.effects[PBEffects::ProtectRate]*2.0) if @attacker.effects[PBEffects::ProtectRate] > 0
		miniscore*=0.7 if @attacker.effects[PBEffects::ProtectRate] > 0 && @battle.doublebattle
	...
	def mirrorcode(copycat=false)
		if copycat
			...
			mirrmove = Battle::Move.pbFromPBMove(@battle,PBMove.new(@battle.previousMove.intern),@attacker)
		else
			return 0 if !@opponent.lastMoveUsed.is_a?(Symbol)
			mirrmove = Battle::Move.pbFromPBMove(@battle,PBMove.new(@opponent.lastMoveUsed.intern),@attacker)
		end
		return 0 if mirrmove.canMirror?
		miniscore = [pbRoughDamageRejuv(mirrmove) / @opponent.hp.to_f, 100].min
	...
	def tormentcode
		return 0 if @opponent.effects[PBEffects::Torment] || ((@opponent.ability == :AROMAVEIL || @opponent.pbPartner&.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker))
		if @opponent.lastMoveUsed.is_a?(Symbol)
			oldmove = Battle::Move.pbFromPBMove(@battle,PBMove.new(@opponent.lastMoveUsed.intern),@opponent)
		else
			oldmove = -1
		end
		miniscore = 1.0
	...
	def disablecode
		return 0 if @opponent.effects[PBEffects::Disable]>0 || ((@opponent.ability == :AROMAVEIL || @opponent.pbPartner&.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker))
		if @opponent.lastMoveUsed.is_a?(Symbol)
			oldmove = Battle::Move.pbFromPBMove(@battle,PBMove.new(@opponent.lastMoveUsed.intern),@opponent)
		else
			oldmove = -1
	...
	def tauntcode
		return @move.basedamage > 0 ? 1 : 0 if @opponent.effects[PBEffects::Taunt]>0 || ((@opponent.ability == :OBLIVIOUS || @opponent.ability == :AROMAVEIL || @opponent.pbPartner&.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker))
		if @opponent.lastMoveUsed.is_a?(Symbol)
			oldmove = Battle::Move.pbFromPBMove(@battle,PBMove.new(@opponent.lastMoveUsed.intern),@opponent)
		else
			oldmove = -1
	...
	def encorecode
		return 0 if @opponent.effects[PBEffects::Encore]>0 || ((@opponent.ability == :AROMAVEIL || @opponent.pbPartner&.ability == :AROMAVEIL) && !moldBreakerCheck(@attacker))
		...
		miniscore*=0.3 if pbRoughDamageRejuv(oldmove,@opponent,@attacker) > @attacker.hp
		if pbRoughDamageRejuv(oldmove,@opponent,@attacker) * 4 > @attacker.hp
			miniscore*=0.3 
		elsif @opponent.stages[:SPEED]>0
	...
	def beatupcode(score) # only partner else multihit is used
		...
		return 0 if @opponent.ability != :JUSTIFIED || @opponent.stages[:ATTACK]>3 || !@opponent.moves.any? {|moveloop| !moveloop.nil? && moveloop.pbIsPhysical?()} || pbRoughDamageRejuv > @opponent.hp
		score = 100-score
	...
	def hypercode()
		return 2 if !@battle.doublebattle && (!@battle.opponentTrainer.nil? && @battle.opponentTrainer.trainertype==:ZEL)
		miniscore = 1.0
	...
	def twoturncode
		...
		if @opponent.status== :POISON || @opponent.status== :BURN || @opponent.effects[PBEffects::LeechSeed]>=0 || @opponent.effects[PBEffects::MultiTurn]>0 || @opponent.effects[PBEffects::Curse]
			miniscore*=1.2
		else
			miniscore*=0.8 if @battle.pbPokemonCount(@battle.pbPartySingleOwner(@opponent.index))>1
		end
		miniscore*=0.5 if !@attacker.status.nil? || @attacker.effects[PBEffects::Curse] || @attacker.effects[PBEffects::Attract]>-1 || @attacker.effects[PBEffects::Confusion]>0
		miniscore*=hpGainPerTurn()
		miniscore*=0.7 if @attacker.pbOwnSide.screenActive? || @attacker.pbOwnSide.effects[PBEffects::Tailwind]>0 
	...
	def uproarcode
		...
		miniscore*=1.1 if @opponent.pbNonActivePokemonCount==0 || @attacker.ability == :SHADOWTAG || @opponent.effects[PBEffects::MeanLook]>0
	...
	def recovercode(amount=@attacker.totalhp/2.0)
		...
		miniscore*=1.1 if @attacker.status== :PARALYSIS || @attacker.effects[PBEffects::Attract]>=0 || @attacker.effects[PBEffects::Confusion]>0
		if @opponent.status== :POISON || @opponent.status== :BURN || @opponent.effects[PBEffects::LeechSeed]>=0 || @opponent.effects[PBEffects::Curse]
			miniscore*=1.3
			miniscore*=1.3 if @opponent.effects[PBEffects::Toxic]>0
		end
	...
	def aquaringcode
		return 0 if ((@move.function == 0xda || @move.function == 0x187) && @attacker.effects[PBEffects::AquaRing]) || (@move.function == 0xdb && @attacker.effects[PBEffects::Ingrain])
		miniscore = 1.0
	...
	def deathcode
		miniscore = 1.0
		miniscore*=0.7
		miniscore*=0.3 if ((@opponent.effects[PBEffects::Disguise] || (@opponent.effects[PBEffects::IceFace] && (@move.pbIsPhysical? || @battle.FE == :FROZENDIMENSION))) && !moldBreakerCheck(@attacker)) || @opponent.effects[PBEffects::Substitute]>0
	...
	def endurecode
		...
		return 0 if @attacker.status== :POISON || @attacker.status== :BURN || @attacker.effects[PBEffects::LeechSeed]>=0 || @attacker.effects[PBEffects::Curse]
		...
		miniscore*=1.5 if @opponent.status== :POISON || @opponent.status== :BURN || @opponent.effects[PBEffects::LeechSeed]>=0 || @opponent.effects[PBEffects::Curse]
		miniscore/=(@attacker.effects[PBEffects::ProtectRate]*2.0) if @attacker.effects[PBEffects::ProtectRate] > 0
		return miniscore
	end

	def destinycode
		return 0 if @attacker.effects[PBEffects::DestinyRate] && @battle.FE != :HAUNTED
		miniscore=1.0
	...
	def phasecode
		return @move.basedamage > 0 ? 1 : 0 if @opponent.effects[PBEffects::Ingrain] || @opponent.ability == :SUCTIONCUPS || @opponent.pbNonActivePokemonCount==0 || @battle.FE == :COLOSSEUM
		return @move.basedamage > 0 ? 1 : 0 if @opponent.effects[PBEffects::PerishSong]>0 || @opponent.effects[PBEffects::Yawn]>0
		miniscore=1.0
	...
	def pivotcode
		...
		if @move.move == :BATONPASS #Baton Pass
			...
			if pbAIfaster?(@move)
				@opponent.hp -= pbRoughDamageRejuv()
	...
	def meanlookcode
		...
		if @opponent.effects[PBEffects::MeanLook]>=0 || @opponent.effects[PBEffects::Ingrain] ||
			...
			secondaryEffectNegated?() || @opponent.effects[PBEffects::Substitute]>0 || @battle.pbPokemonCount(@battle.pbPartySingleOwner(@opponent.index))==1
	...
	def knockcode
		return @move.basedamage > 0 ? 1 : 0 if @opponent.effects[PBEffects::Substitute]>0
	...
	def covetcode
		return 1 if !((@opponent.ability != :STICKYHOLD || moldBreakerCheck(@attacker)) && @opponent.item && !@battle.pbIsUnlosableItem(@opponent,@opponent.item) && @attacker.item.nil? && @opponent.effects[PBEffects::Substitute]<=0)
		miniscore = 1.2
		case @opponent.item
	...
	def focuscode
		...
		multicheck=getAIMemory().any? {|moveloop| moveloop!=nil && moveloop.pbNumHits(@opponent, @attacker)>1}
	...
	def followcode
		...
		miniscore*=1.5 if notOHKO?(@attacker, @opponent, true)
		if maxdam1 >= @attacker.pbPartner&.hp && pbRoughDamageRejuv(bestmove1,@attacker.pbOpposing1,@attacker) < 0.7*@attacker.hp || 
		   maxdam2 >= @attacker.pbPartner&.hp && pbRoughDamageRejuv(bestmove2,@attacker.pbOpposing2,@attacker) < 0.7*@attacker.hp
		   miniscore*= 1.3
		end
		if @attacker.hp==@attacker.totalhp
			miniscore*=1.2
	...
	def trcode
		...
		if @opponent.pbPartner&.hp > 0
			miniscore*=0.3 if @attacker.pbSpeed<pbRoughStatRejuv(@opponent,:SPEED) && @attacker.pbSpeed>pbRoughStatRejuv(@opponent.pbPartner,:SPEED)
			miniscore*=0.3 if @attacker.pbSpeed>pbRoughStatRejuv(@opponent,:SPEED) && @attacker.pbSpeed<pbRoughStatRejuv(@opponent.pbPartner,:SPEED)
		end
	...
	def wondercode
		...
		if @attacker.attack>@attacker.spatk
			miniscore*= (pbRoughStatRejuv(@opponent,:DEFENSE)>pbRoughStatRejuv(@opponent,:SPECIAL_DEFENSE)) ? 2 : 0.5
		else
			miniscore*= (pbRoughStatRejuv(@opponent,:DEFENSE)>pbRoughStatRejuv(@opponent,:SPECIAL_DEFENSE)) ? 0.5 : 2
		end
		return miniscore
	end
	...
	def pussydeathcode(initialscore)
		...
		if @attacker.ability != :MAGICGUARD && !(@attacker.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
			...
			miniscore*=(1-0.1*@opponent.stages[:EVASION])
			miniscore*=(1+0.1*@opponent.stages[:ACCURACY])
	...
	def psychicterraincode
		...
		if Rejuv && @battle.FE != :INDOOR
			return @move.basedamage > 0 ? 1 : 0 if @battle.state.effects[PBEffects::PSYTERRAIN] > 0
			miniscore = 1.0
	...
	def instructcode # function is only evaluated for the partner, never the opponent
		...
		lastmove = Battle::Move.pbFromPBMove(@battle,PBMove.new(lastmove),@attacker.pbPartner)
		lastmovetarget = @attacker.pbPartner&.lastMoveChoice[3]
		lastmovetarget = firstOpponent() if lastmovetarget == -1
		movescore = pbRoughDamageRejuv(lastmove, @attacker.pbPartner, @battle.battlers[lastmovetarget])
		if movescore == 0
			miniscore*=0
	...
	def antistatcode(stats,beginscore)
		...
		for i in 0...stats.length
			...
			case i
				...
				when :SPEED
					miniscore*=1.1 if @mondata.roles.include?(:TANK)
					miniscore*=0.8 if @attacker.pbSpeed>pbRoughStatRejuv(@opponent,:SPEED)
				when :SPECIAL_ATTACK
					miniscore*=0.5 if beginscore < 100 && checkAIhealing()
					miniscore*=0.8 if @opponent.pbNonActivePokemonCount > 0 && @attacker.pbNonActivePokemonCount==0
				when :SPECIAL_DEFENSE
	...
	def smackcode
		return 1 if @opponent.effects[PBEffects::Ingrain] || @opponent.effects[PBEffects::SmackDown] || @battle.state.effects[PBEffects::Gravity]!=0 || (@mondata.oppitemworks && @opponent.item == :IRONBALL) || @opponent.effects[PBEffects::Substitute]>0
		miniscore=1.0
		if !pbAIfaster?()
	...
	def nightmarecode
		return 0 if @opponent.effects[PBEffects::Nightmare] || (@opponent.status!=:SLEEP && @battle.FE != :INFERNAL) || @opponent.effects[PBEffects::Substitute]>0
		miniscore=1.0
		...
		if @attacker.ability == :SHADOWTAG || @attacker.ability == :ARENATRAP || @opponent.effects[PBEffects::MeanLook]>=0 || @opponent.pbNonActivePokemonCount==0
			miniscore*=1.3
		else
			miniscore*=0.8
		end
	...
	def spoopycode
		return @move.basedamage > 0 ? 1 : 0 if @opponent.effects[PBEffects::Curse] || (((@attacker.hp*2<@attacker.totalhp && @battle.FE != :HAUNTED) || (@attacker.hp*4<@attacker.totalhp)) && @move.function == 0x10d)
		miniscore=0.7
		miniscore*=0.5 if !pbAIfaster?(@move)
	...
	def brickbreakcode(attacker=@attacker)
		...
		miniscore*=1.3 if attacker.pbOpposingSide.effects[PBEffects::AreniteWall]>0
		return miniscore
	end
	...
	def jumpcode(score)
		...
		miniscore*=0.5 if checkAImoves(PBStuff::PROTECTMOVE) && !(@move.contactMove? && @attacker.ability == :UNSEENFIST)
		miniscore*=(1-0.1*@opponent.stages[:EVASION])
		miniscore*=(1+0.1*@attacker.stages[:ACCURACY])
		miniscore*=0.7 if accuracyWeatherAbilityActive?(@opponent)
	...
	def hazardcode
		...
		miniscore*=1.3 if notOHKO?(@attacker, @opponent)
		miniscore*=1.2 if @attacker.turncount<2
		miniscore*=0.9 if @attacker.stages[:ATTACK] > 0 || @attacker.stages[:SPECIAL_ATTACK] > 0 && @move.basedamage == 0
		if @move.basedamage == 0
	...
	def electricterraincode
		...
		if Rejuv && @battle.FE != :INDOOR
			return @move.basedamage > 0 ? 1 : 0 if @battle.state.effects[PBEffects::ELECTERRAIN] > 0
			miniscore = 1
	...
	def grassyterraincode
		...
		if Rejuv && @battle.FE != :INDOOR
			return @move.basedamage > 0 ? 1 : 0 if @battle.state.effects[PBEffects::GRASSY] > 0
			miniscore = 1
			miniscore*=1.5 if @mondata.roles.include?(:PHYSICALWALL) || @mondata.roles.include?(:SPECIALWALL)
	...
	def mistyterraincode
		...
		if Rejuv && @battle.FE != :INDOOR
			return @move.basedamage > 0 ? 1 : 0 if @battle.state.effects[PBEffects::MISTY] > 0
			miniscore = 1
	...
	def arocode(stat)
		...
		miniscore*=0.3 if @opponent.effects[PBEffects::Yawn]>0 || @opponent.effects[PBEffects::LeechSeed]>=0 || @opponent.effects[PBEffects::Attract]>=0 || !@opponent.status.nil?
		if !@battle.pbIsWild?
		...
		miniscore*=1.2 if hpGainPerTurn>1
		miniscore*=0 if @opponent.ability == :CONTRARY
		miniscore*=2 if @battle.FE == :MISTY && stat==:SPECIAL_DEFENSE
	end

	def flowershieldcode(score)
		return 0 unless @battle.doublebattle && @opponent.hasType?(:GRASS) && @opponent.index==@attacker.pbPartner&.index && @opponent.stages[:DEFENSE]!=6
		opp1 = @attacker.pbOppositeOpposing
		opp2 = opp1.pbPartner
		...
		if @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN,2,5)|| @battle.FE == :FAIRYTALE
			score+=20
			miniscore=100
			miniscore*=1.3 if @attacker.effects[PBEffects::Substitute]>0 || ((@attacker.effects[PBEffects::Disguise] || (@attacker.effects[PBEffects::IceFace] && (@opponent.attack > @opponent.spatk || @battle.FE == :FROZENDIMENSION))) && !moldBreakerCheck(@opponent))
			miniscore*=1.3 if @initial_scores.length>0 && hasbadmoves(20)
			...
			miniscore*=1.3 if @attacker.pbHasMove?(:LEECHSEED)
			miniscore*=1.2 if @attacker.pbHasMove?(:PAINSPLIT)
			score*=miniscore if @attacker.stages[:SPECIAL_DEFENSE]!=6 && @attacker.stages[:DEFENSE]!=6
	...
	def rotocode(score)
		...
		if @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN)
			...
			miniscore*=0.5 if opp1.ability == :UNAWARE
			miniscore*=0.3 if @battle.doublebattle
			miniscore*=1+0.05*@opponent.stages[:SPEED] if @opponent.stages[:SPEED]<0
			ministat=@opponent.stages[:ATTACK] + @opponent.stages[:SPEED] + @opponent.stages[:SPECIAL_ATTACK]
			miniscore*=1 -0.05*ministat if ministat > 0
			miniscore*=1.3 if checkAIhealing()
			miniscore*=1.5 if @attacker.pbSpeed>pbRoughStatRejuv(@opponent,:SPEED,@mondata.skill) && @battle.trickroom==0
			miniscore*=1.3 if @mondata.roles.include?(:SWEEPER)
			miniscore*=0.5 if @attacker.status== :PARALYSIS
			miniscore*=0.3 if checkAImoves([:FOULPLAY])
	...
	def flingcode
		return 0 if @attacker.item.nil? || @battle.pbIsUnlosableItem(@attacker,@attacker.item) || @attacker.ability == :KLUTZ || (pbIsBerry?(@attacker.item) && (@opponent.ability == :UNNERVE || @opponent.ability == :ASONE)) || @attacker.effects[PBEffects::Embargo]>0 || @battle.state.effects[PBEffects::MagicRoom]>0
		miniscore=1.0
		case @attacker.item
	...
	def embarcode(opponent=@opponent)
		return 0 if opponent.effects[PBEffects::Embargo]>0  && opponent.effects[PBEffects::Substitute]>0 || opponent.item.nil?
		miniscore = 1.1
	...
	def roastcode
		return 1 if !@opponent.item.nil? && !pbIsBerry?(@opponent.item) && !pbIsTypeGem?(@opponent.item) || @opponent.ability == :STICKYHOLD || @opponent.effects[PBEffects::Substitute] > 0
		miniscore=1.0
	...
	def nomcode
		return 1 if @opponent.effects[PBEffects::Substitute] > 0 || (!@opponent.item.nil? && !pbIsBerry?(@opponent.item))
		miniscore=1.0
		case @opponent.item
			when :LUMBERRY then miniscore*=2 if !@attacker.status.nil?
			when :CHERIBERRY then miniscore*=2 if @attacker.status == :PARALYSIS
	...
	def perishcode
		...
		miniscore*=3 if @attacker.ability == :SHADOWTAG || @attacker.pbPartner&.ability == :SHADOWTAG || @opponent.effects[PBEffects::MeanLook]>0
		...
		miniscore*=0.1 if (@opponent.ability == :SHADOWTAG || @attacker.effects[PBEffects::MeanLook]>0) && !(@attacker.pbHasMove?(:UTURN) || @attacker.pbHasMove?(:VOLTSWITCH) || @attacker.pbHasMove?(:PARTINGSHOT))
	...
	def rolloutcode
		...
		miniscore*=1+0.05*@attacker.stages[:ACCURACY] if @attacker.stages[:ACCURACY]<0
		miniscore*=1+0.05*@attacker.stages[:ATTACK] if @attacker.stages[:ATTACK]<0
		miniscore*=1-0.05*@opponent.stages[:EVASION] if @opponent.stages[:EVASION]>0
	...
	def stupidmovecode
		...
		return 0 if @opponent.stages[:SPEED]==0 && @attacker.stages[:SPEED]==0
		miniscore = 1 + 0.1*@opponent.stages[:SPEED] - 0.1*@attacker.stages[:SPEED]
		miniscore*=0.8 if @battle.doublebattle
		return miniscore
	end
...
######################################################
	...
	def pbMakeFakeBattler(pokemon,batonpass=false)
		...
		battler = Battle::Battler.new(@battle,@index,true)
	...
	def pbRoughStatRejuv(battler,stat)
		return battler.pbSpeed if @mondata.skill>=HIGHSKILL && stat==:SPEED
		stagemul=[2,2,2,2,2,2,2,3,4,5,6,7,8]
		stagediv=[8,7,6,5,4,3,2,2,2,2,2,2,2]
		stage=battler.stages[stat]+6
		value=0
		value=battler.attack if stat==:ATTACK
		value=battler.defense if stat==:DEFENSE
		value=battler.speed if stat==:SPEED
		value=battler.spatk if stat==:SPECIAL_ATTACK
		value=battler.spdef if stat==:SPECIAL_DEFENSE
		return (value*1.0*stagemul[stage]/stagediv[stage]).floor
	end

	def pbRoughAccuracyRejuv(move,attacker,opponent)
		...
		if @mondata.skill>=MEDIUMSKILL
			...
			if @mondata.skill>=HIGHSKILL
				return 100 if (move.function==0x10 || move.move == :BODYSLAM || move.function==0x137 || move.function==0x9B || move.function==0x806) && opponent.effects[PBEffects::Minimize] # Flying Press, Stomp, DRush, Mal. Moonsault
				return 100 if @battle.FE == :MIRROR && (PBFields::BLINDINGMOVES + [:MIRRORSHOT]).include?(move.move)
				return 100 if @battle.FE == :MIRROR && move.basedamage>0 && move.target==:SingleNonUser && !move.contactMove? && move.pbIsSpecial?(move.type) && opponent.stages[:EVASION]>0
			end
		end
		# Get base accuracy
		baseaccuracy=move.accuracy
		...
		# Accuracy stages
		accstage=attacker.stages[:ACCURACY]
		accstage=0 if opponent.ability == :UNAWARE && !moldBreakerCheck(attacker)
		accuracy=(accstage>=0) ? (accstage+3)*100.0/3 : 300.0/(3-accstage)
		evastage=opponent.stages[:EVASION]
		evastage-=2 if @battle.state.effects[PBEffects::Gravity]!=0
		evastage=-6 if evastage<-6
		evastage=0 if opponent.effects[PBEffects::Foresight] || opponent.effects[PBEffects::MiracleEye] || move.function==0xA9 || attacker.ability == :UNAWARE && !moldBreakerCheck(opponent)
		...
		if @mondata.skill>=MEDIUMSKILL
			...
			if @mondata.skill>=BESTSKILL
				...
				evasion*=1.2 if opponent.ability == :TANGLEDFEET && opponent.effects[PBEffects::Confusion]>0 && !moldBreakerCheck(attacker)
	...
	def pbAIfaster?(attackermove=nil, opponentmove=nil, attacker=@attacker, opponent=@opponent)
		...
		return (pbRoughStatRejuv(opponent,:SPEED) < attacker.pbSpeed) ^ (@battle.trickroom!=0) if @mondata.skill < HIGHSKILL
		...
		for battler in [attacker, opponent]
			...
			pri += 1 if battlermove.move == :GRASSYGLIDE && (@battle.FE == :GRASSY || @battle.state.effects[PBEffects::GRASSY] > 0)
	...
	def pbMoveOrderAI #lol it's just pbPriority
		...
		for i in 0..3
			...
			#speed priority
			priorityarray[i][2] = pbRoughStatRejuv(battler,:SPEED) if @battle.trickroom==0
			priorityarray[i][2] = -pbRoughStatRejuv(battler,:SPEED) if @battle.trickroom>0
		end
		priorityarray.sort!
		priorityarray.reverse!
	...
	def hpGainPerTurn(attacker=@attacker)
		...
		if attacker.ability != :MAGICGUARD && !(attacker.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM)
			if (@battle.FE == :BURNING || @battle.FE == :VOLCANIC) && !attacker.isAirborne? && attacker.burningFieldPassiveDamage?
				...
				subscore*=2.0 if attacker.effects[PBEffects::TarShot]
				healing -= subscore
			end
			...
			healing -= 0.0625 if attacker.effects[PBEffects::AquaRing] && @battle.FE == :CORROSIVEMIST && !attacker.hasType?(:STEEL) && !attacker.hasType?(:POISON) || !@battle.pbCheckGlobalAbility(:NEUTRALIZINGGAS)
			healing -= 0.0625 if attacker.effects[PBEffects::Ingrain] && (@battle.FE == :SWAMP || @battle.FE == :CORROSIVE || @battle.FE == :CORRUPTED) && !(attacker.hasType?(:STEEL) || attacker.hasType?(:POISON))
			healing -= 0.0625 if @battle.FE == :HAUNTED && attacker.status == :SLEEP && !attacker.hasType?(:GHOST)
			healing -= 0.0625 if (@battle.FE == :DIMENSIONAL && attacker.effects[PBEffects::HealBlock])
			healing -= 0.125 if @battle.FE == :CORROSIVE && (attacker.ability == :GRASSPELT || attacker.ability == :LEAFGUARD || attacker.ability == :FLOWERVEIL)
			healing -= 0.125 if @battle.FE == :INFERNAL && attacker.effects[PBEffects::Torment]
			...
			healing -= (Rejuv && @battle.FE == :DESERT) ? 0.125 : 0.0625 if @battle.pbWeather == :SANDSTORM && (!(attacker.hasType?(:GROUND) || attacker.hasType?(:ROCK) || attacker.hasType?(:STEEL) || [:SANDFORCE,:SANDRUSH,:SANDVEIL,:MAGICGUARD,:OVERCOAT,:TEMPEST].include?(attacker.ability)) || attacker.effects[PBEffects::DesertsMark])
			...
			healing -= [15,attacker.effects[PBEffects::Toxic]].min / 16.0 if attacker.status == :POISON && attacker.ability != :POISONHEAL && attacker.crested != :ZANGOOSE  && attacker.statusCount > 0
			...
			if attacker.status == :SLEEP || (attacker.ability == :COMATOSE && @battle.FE != :ELECTERRAIN)
				...
				if Rejuv &&  @battle.FE == :SWAMP
					sleepdmg *= 2.0 if attacker.effects[PBEffects::MultiTurn] > 0
				end
			...
			# Other
			healing -= 0.125 if attacker.effects[PBEffects::LeechSeed]>=0 && attacker.ability != :LIQUIDOOZE
			healing -= 0.125 if attacker.status == :PETRIFIED && attacker.ability != :LIQUIDOOZE
			healing -= 0.125 if @battle.FE == :WASTELAND && attacker.effects[PBEffects::LeechSeed]>=0 && attacker.ability != :LIQUIDOOZE
			healing -= (@battle.FE == :DARKNESS3 || @battle.FE == :HAUNTED) ? 0.33 : 0.25 if attacker.effects[PBEffects::Nightmare] && @battle.FE != :RAINBOW && attacker.status== :SLEEP
			healing -= 0.3 if attacker.effects[PBEffects::Curse] && @battle.FE != :HOLY
			if attacker.effects[PBEffects::MultiTurn] > 0
				if attacker.effects[PBEffects::BindingBand]
					healing -= 0.1667
				else
					case attacker.effects[PBEffects::MultiTurnAttack]
		...
		if attacker.effects[PBEffects::Ingrain]
			...
			subscore *= 1.3 if (@attacker.crested == :SHIINOTIC)
			subscore *= 2.0 if (@battle.FE == :FOREST || @battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN) || (Rejuv && @battle.FE == :GRASSY)) || @battle.state.effects[PBEffects::GRASSY] > 0
			subscore *= 2.0 if (@battle.ProgressiveFieldCheck(PBFields::FLOWERGARDEN,4,5))
			healing += subscore
		end
		if attacker.ability == :DRYSKIN
			healing += 0.0625 if ((@battle.FE == :CORROSIVE || @battle.FE == :CORRUPTED) && attacker.hasType?(:POISON) && !attacker.hasType?(:STEEL)) || @battle.pbWeather== :RAINDANCE || [:MISTY,:SWAMP,:WATERSURFACE,:UNDERWATER].include?(@battle.FE) || @battle.state.effects[PBEffects::MISTY] > 0
		end
		...
		healing += 0.0625 if attacker.crested == :VESPIQUEN && attacker.effects[PBEffects::VespiCrest] == false
		...
		healing += 0.0625 if Rejuv && (@battle.FE == :GRASSY || @battle.state.effects[PBEffects::GRASSY] > 0) && attacker.ability == :SAPSIPPER
		healing += 0.0625 if (@battle.FE == :GRASSY || @battle.state.effects[PBEffects::GRASSY] > 0) && !attacker.isAirborne? && !PBStuff::TWOTURNMOVE.include?(attacker.effects[PBEffects::TwoTurnAttack])
		healing += 0.0625 if Rejuv && (@battle.FE == :ELECTERRAIN || @battle.state.effects[PBEffects::ELECTERRAIN ] > 0) && attacker.ability == :VOLTABSORB
		if @battle.FE != :INDOOR
	...
	def pbTypeModNoMessages(type=@move.type,attacker=@attacker,opponent=@opponent,move=@move,skill=@mondata.skill)
		...
		if !moldBreakerCheck(attacker)
			case opponent.ability
				when :SAPSIPPER 					 	then return -1 if type == :GRASS || secondtype==:GRASS
				when :LEVITATE,:SOLARIDOL,:LUNARIDOL 	then return 0 if (type == :GROUND || secondtype==:GROUND) && @battle.FE != :CAVE && !opponent.hasWorkingItem(:IRONBALL) && @battle.state.effects[PBEffects::Gravity]==0
				when :MAGNETPULL,:CONTRARY,:UNAWARE,:OBLIVIOUS 	then return 0 if (type == :GROUND || secondtype==:GROUND) && @battle.FE == :DEEPEARTH && !opponent.hasWorkingItem(:IRONBALL)
				when :STORMDRAIN 						then return -1 if type == :WATER || secondtype==:WATER
				when :LIGHTNINGROD,:MOTORDRIVE			then return -1 if type == :ELECTRIC || secondtype==:ELECTRIC
				when :DRYSKIN 							then return -1 if type == :WATER || secondtype==:WATER && opponent.effects[PBEffects::HealBlock]==0
				when :VOLTABSORB 						then return -1 if type == :ELECTRIC || secondtype==:ELECTRIC && opponent.effects[PBEffects::HealBlock]==0
				when :WATERABSORB 						then return -1 if type == :WATER || secondtype==:WATER && opponent.effects[PBEffects::HealBlock]==0
				when :BULLETPROOF 						then return 0 if (PBStuff::BULLETMOVE).include?(id)
				when :FLASHFIRE 						then return -1 if type == :FIRE || secondtype==:FIRE
				when :MAGMAARMOR 						then return 0 if (type == :FIRE || secondtype==:FIRE) && (@battle.FE == :DRAGONSDEN || @battle.FE == :INFERNAL || @battle.FE == :VOLCANICTOP)
				when :TELEPATHY 						then return 0 if  move.basedamage>0 && opponent.index == attacker.pbPartner&.index
			end
		end
		case opponent.crested
			when :WHISCASH					 		then return -1 if type == :GRASS || secondtype==:GRASS
			when :SKUNTANK 							then return -1 if type == :GROUND || secondtype==:GROUND
			when :DRUDDIGON							then return -1 if type == :FIRE || secondtype==:FIRE
		end
		if @battle.FE == :ROCKY && (opponent.effects[PBEffects::Substitute]>0 || opponent.stages[:EVASION] > 0)
		  	return 0 if (PBStuff::BULLETMOVE).include?(id)
		end
		if (@battle.FE == :WATERSURFACE || @battle.FE == :MURKWATERSURFACE) && (type == :GROUND || secondtype==:GROUND)
		  	return 0
		end
		if @battle.FE == :HOLY && move.basedamage>0 && opponent.index == attacker.pbPartner&.index
			return 0
		end
		if Rejuv && @battle.FE == :DESERT && @battle.pbWeather == :SUNNYDAY && (type == :WATER || secondtype==:WATER) && (opponent.hasType?(:WATER) || opponent.hasType?(:GRASS)) && opponent.effects[PBEffects::HealBlock]==0
			return -1
		end
		if Rejuv && @battle.FE == :GLITCH && opponent.species == :GENESECT
			if opponent.item == :BURNDRIVE && (type == :FIRE || secondtype==:FIRE)
				return -1
			elsif opponent.item == :DOUSEDRIVE && (type == :WATER || secondtype==:WATER)
				return -1
			elsif opponent.item == :CHILLDRIVE && (type == :ICE || secondtype==:ICE)
				return -1
			elsif opponent.item == :SHOCKDRIVE && (type == :ELECTRIC || secondtype==:ELECTRIC)
				return -1
			end
		end
		faintedcount=0
		...
		typemod*=2 if type == :FIRE && opponent.effects[PBEffects::TarShot]
		case opponent.crested
			when :LUXRAY
		...
		if @battle.pbWeather==:STRONGWINDS && ((opponent.hasType?(:FLYING)) && !opponent.effects[PBEffects::Roost]) &&
	...
	def pbChangeMove(move,attacker)
		...
		move = Battle::Move.pbFromPBMove(@battle,PBMove.new(move.move),attacker)
		case move.move
			when :WEATHERBALL
			...
			when :NATUREPOWER
				move = Battle::Move.pbFromPBMove(@battle,PBMove.new(@battle.field.naturePower),attacker)
	...
	def totalHazardDamage(pkmn)
		percentdamage = 0
		if pkmn.pbOwnSide.effects[PBEffects::Spikes]>0 && !pkmn.isAirborne? && !pkmn.ability == :MAGICGUARD && !(pkmn.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM) && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS)
		...
		if pkmn.pbOwnSide.effects[PBEffects::StealthRock] && !pkmn.ability == :MAGICGUARD && !(pkmn.ability == :WONDERGUARD && @battle.FE == :COLOSSEUM) && !pkmn.hasWorkingItem(:HEAVYDUTYBOOTS)
	...
	def getAbilityDisruptScore(attacker,opponent)
		...
		case opponent.ability
			when :SPEEDBOOST
				abilityscore*=1.1
				abilityscore*=1.3 if opponent.stages[:SPEED]<2
			when :SANDVEIL
			...
			when :LIQUIDOOZE
				abilityscore*=2 if opponent.effects[PBEffects::LeechSeed]>=0 || attacker.pbHasMove?(:LEECHSEED)
			...
			when :CONTRARY
				abilityscore*=1.4
				abilityscore*=2 if opponent.stages[:ATTACK]>0 || opponent.stages[:SPECIAL_ATTACK]>0 || opponent.stages[:DEFENSE]>0 || opponent.stages[:SPECIAL_DEFENSE]>0 || opponent.stages[:SPEED]>0
			when :DEFEATIST
				abilityscore*=0.5
			when :MULTISCALE
	...
	def getFieldDisruptScore(attacker=@attacker,opponent=@opponent,fieldeffect=@battle.FE, violent=false)
		...
		case fieldeffect
			...
			when :CORROSIVEMIST # Corromist field
				if violent
					if !PBStuff::INVULEFFECTS.any? {|eff| opponent.effects[eff] } && !(PBStuff::TWOTURNMOVE.include?(opponent.effects[PBEffects::TwoTurnAttack]) && pbAIfaster?(nil,nil,attacker,opponent)) && opponent.ability != :FLASHFIRE 
			...
			when :MIRROR # Mirror field
				if violent
					if opponent.stages[:EVASION] > 0 || (@mondata.oppitemworks && opponent.item == :BRIGHTPOWDER) || 
				...
				fieldscore*=1+0.1*opponent.stages[:ACCURACY]
				fieldscore*=1+0.1*opponent.stages[:EVASION]
				fieldscore*=1-0.1*attacker.stages[:ACCURACY]
				fieldscore*=1-0.1*attacker.stages[:EVASION]
...
################################################################################
	...
	def getItemScore
		...
		#expected damage
		for i in aimem
			tempdam = pbRoughDamageRejuv(i,opponent1,@attacker)
		...
		for i in items
			...
			itemscore*=2 if @attacker.effects[PBEffects::Toxic]>3 && i == :FULLRESTORE
			...
			if @mondata.roles.include?(:SWEEPER)
				minipot+=@attacker.stages[:SPEED]
				minipot+= @attacker.attack>@attacker.spatk ? @attacker.stages[:ATTACK] : @attacker.stages[:SPECIAL_ATTACK]
			end
...
################################################################################
	...
	def pbDefaultChooseNewEnemyRejuv(index,party)
	...
	def getSwitchInScoresParty(hard_switch)
		...
		for partyindex in 0...party.length
			...
			if (i.ability == :IMPOSTER)
				...
				monscore += 20*@opponent.stages[:ATTACK]
				monscore += 20*@opponent.stages[:SPECIAL_ATTACK]
				monscore += 20*@opponent.stages[:SPEED]
			end
			...
			for moveindex in 0...i.moves.length
				...
				next if @move.nil?
				roughdamagearray[0][moveindex] = [(pbRoughDamageRejuv(@move,i,@opponent)*100)/(@opponent.hp.to_f),100].min if @opponent.hp > 0
				if @battle.doublebattle
					roughdamagearray[1][moveindex] = [(pbRoughDamageRejuv(@move,i,@opponent.pbPartner)*100)/(@opponent.pbPartner&.hp.to_f),100].min if @opponent.pbPartner&.hp > 0
					next if @move.target != :AllNonUsers && !PARTNERFUNCTIONS.include?(@move.function)
					roughdamagearray[2][moveindex] = [(pbRoughDamageRejuv(@move,i,@attacker.pbPartner)*100)/(@attacker.pbPartner&.hp.to_f),100].min if @attacker.pbPartner&.hp > 0
				end
			...
			if hard_switch && !@battle.doublebattle && (maxsingulardamage < 1.0 || pbAIfaster?(nil,nil,@opponent,i))
				...
				assumed_damage = pbRoughDamageRejuv(assumed_move,@opponent,nonmegaform)
			...
			if @mondata.skill >= HIGHSKILL
				...
				if theseRoles.include?(:PHYSICALWALL) || theseRoles.include?(:SPECIALWALL)
					...
					rolescore+=30 if @opponent.status== :BURN || @opponent.status== :POISON || @opponent.effects[PBEffects::LeechSeed]>0
					rolescore+=30 if @opponent.pbPartner&.status== :BURN || @opponent.pbPartner&.status== :POISON || @opponent.pbPartner&.effects[PBEffects::LeechSeed]>0
				end
				if theseRoles.include?(:TANK)
					rolescore+=40 if @opponent.status== :PARALYSIS || @opponent.effects[PBEffects::LeechSeed]>0
					rolescore+=40 if @opponent.pbPartner&.status== :PARALYSIS || @opponent.pbPartner&.effects[PBEffects::LeechSeed]>0
				...
				if @attacker.effects[PBEffects::LunarDance] && (@battle.FE == :NEWWORLD || @battle.FE == :DANCEFLOOR)
				...
				if theseRoles.include?(:PHAZER)
					for opp in [@opponent, @opponent.pbPartner]
						next if opp.hp <=0
						rolescore+= (10)*opp.stages[:ATTACK]	if opp.stages[:ATTACK]<0
						rolescore+= (20)*opp.stages[:DEFENSE]	if opp.stages[:DEFENSE]<0
						rolescore+= (10)*opp.stages[:SPECIAL_ATTACK]		if opp.stages[:SPECIAL_ATTACK]<0
						rolescore+= (20)*opp.stages[:SPECIAL_DEFENSE]		if opp.stages[:SPECIAL_DEFENSE]<0
						rolescore+= (10)*opp.stages[:SPEED]		if opp.stages[:SPEED]<0
						rolescore+= (20)*opp.stages[:EVASION]	if opp.stages[:ACCURACY]<0
					end
				end
			...
			if @mondata.skill>=HIGHSKILL
				...
				movesscore+=30 if i.pbHasMove?(:RETALIATE) && @attacker.pbOwnSide.effects[PBEffects::Retaliate]
			...
			if @mondata.skill >= HIGHSKILL
				case i.ability
					...
					when :ICEFACE
						if i.effects[PBEffects::IceFace] && (@opponent.attack > @opponent.spatk || @battle.FE == :FROZENDIMENSION)
							abilityscore+= (10)* statchangecounter(@opponent,1,7,1)
							abilityscore+= (10)* statchangecounter(@opponent.pbPartner,1,7,1)
							abilityscore+= 50 if roughdamagearray[0].max >= 100 || roughdamagearray[1].max >= 100
						end
					...
					when :COMPOUNDEYES
						abilityscore+=25 if (@opponent.item == :LAXINCENSE) || (@opponent.item == :BRIGHTPOWDER) || @opponent.stages[:EVASION]>0 || accuracyWeatherAbilityActive?(@opponent)
						abilityscore+=25 if (@opponent.pbPartner&.item == :LAXINCENSE) || (@opponent.pbPartner&.item == :BRIGHTPOWDER) || @opponent.pbPartner&.stages[:EVASION]>0 || accuracyWeatherAbilityActive?(@opponent.pbPartner)
					when :COMATOSE
					...
					when :SCREENCLEANER
						abilityscore+=10 if @opponent.pbOwnSide.effects[PBEffects::Reflect]>0
						abilityscore+=10 if @opponent.pbOwnSide.effects[PBEffects::LightScreen]>0
						abilityscore+=5 if @opponent.pbOwnSide.effects[PBEffects::Safeguard]>0
						abilityscore+=20 if @opponent.pbOwnSide.effects[PBEffects::AuroraVeil]>0
						abilityscore+=20 if @opponent.pbOwnSide.effects[PBEffects::AreniteWall]>0

						abilityscore-=10 if @attacker.pbOwnSide.effects[PBEffects::Reflect]>0
						abilityscore-=10 if @attacker.pbOwnSide.effects[PBEffects::LightScreen]>0
						abilityscore-=5 if @attacker.pbOwnSide.effects[PBEffects::Safeguard]>0
						abilityscore-=20 if @attacker.pbOwnSide.effects[PBEffects::AuroraVeil]>0
						abilityscore-=20 if @attacker.pbOwnSide.effects[PBEffects::AreniteWall]>0
					...
					when :GALEWINGS
						...
						abilityscore+=50 if @opponent.pbPartner&.hp > 0 && !pbAIfaster?(nil,nil,i,@opponent.pbPartner) && i.hp==i.totalhp && !@attacker.pbOwnSide.effects[PBEffects::StealthRock]
					when :BULLETPROOF
			...
			if @mondata.skill>=HIGHSKILL
				...
				if (i.item == :FOCUSSASH || (@battle.FE == :CHESS && i.pokemon.piece==:PAWN) || i.ability == :STURDY || (@battle.FE == :CHESS && i.ability == :STALWART)) && i.hp == i.totalhp
					if 	(((@battle.weather== :SANDSTORM && !(i.hasType?(:ROCK) || i.hasType?(:GROUND) || i.hasType?(:STEEL)))  || (@battle.weather== :HAIL && !(i.hasType?(:ICE)))) && !((i.ability == :OVERCOAT)))  || @attacker.pbOwnSide.effects[PBEffects::StealthRock] ||
					...
					itemscore+= (30)*@opponent.stages[:ATTACK]
					itemscore+= (30)*@opponent.stages[:SPECIAL_ATTACK]
					itemscore+= (30)*@opponent.stages[:SPEED]
				end
				if (i.item == :SNOWBALL)
			...
			if @attacker.effects[PBEffects::FutureSight] >= 1
				...
				damage = hard_switch ? pbRoughDamageRejuv(move,moveuser,nonmegaform) : pbRoughDamageRejuv(move,moveuser,i)
	...
	def shouldSwitch?
		return -1000 if !@battle.opponentTrainer && @battle.pbIsOpposing?(@attacker.index)
		...
		statusscore+=95 if @attacker.effects[PBEffects::Yawn]>0 && @attacker.status!=:SLEEP
		...
		if @mondata.roles.include?(:SWEEPER)
			statscore+= (-30)*@attacker.stages[:ATTACK] if @attacker.stages[:ATTACK]<0 && physmove
			statscore+= (-30)*@attacker.stages[:SPECIAL_ATTACK] if @attacker.stages[:SPECIAL_ATTACK]<0 && specialmove
			statscore+= (-30)*@attacker.stages[:SPEED] if @attacker.stages[:SPEED]<0
			statscore+= (-30)*@attacker.stages[:ACCURACY] if @attacker.stages[:ACCURACY]<0
		else
			statscore+= (-15)*@attacker.stages[:ATTACK] if @attacker.stages[:ATTACK]<0 && physmove
			statscore+= (-15)*@attacker.stages[:SPECIAL_ATTACK] if @attacker.stages[:SPECIAL_ATTACK]<0 && specialmove
			statscore+= (-15)*@attacker.stages[:SPEED] if @attacker.stages[:SPEED]<0
			statscore+= (-15)*@attacker.stages[:ACCURACY] if @attacker.stages[:ACCURACY]<0
		end
		if @mondata.roles.include?(:PHYSICALWALL)
			statscore+= (-30)*@attacker.stages[:DEFENSE] if @attacker.stages[:DEFENSE]<0
		else
			statscore+= (-15)*@attacker.stages[:DEFENSE] if @attacker.stages[:DEFENSE]<0
		end
		if @mondata.roles.include?(:SPECIALWALL)
			statscore+= (-30)*@attacker.stages[:SPECIAL_DEFENSE] if @attacker.stages[:SPECIAL_DEFENSE]<0
		else
			statscore+= (-15)*@attacker.stages[:SPECIAL_DEFENSE] if @attacker.stages[:SPECIAL_DEFENSE]<0
		end
		...
		for i in @attacker.moves
			...
			for oppmon in [@opponent, @opponent.pbPartner]
				...
				bothimmune = false if i.function == 0xdc && (!noLeechSeed(oppmon) || oppmon.effects[PBEffects::LeechSeed] > -1)
				if i.basedamage > 0
		...
		forcedscore+=30 if @attacker.effects[PBEffects::Torment]
		if @attacker.effects[PBEffects::Encore]>0
			...
			forcedscore+=110 if @attacker.effects[PBEffects::Torment]
		end
		if (@attacker.item == :CHOICEBAND || @attacker.item == :CHOICESPECS || @attacker.item == :CHOICESCARF || @attacker.ability == :GORILLATACTICS) && @attacker.effects[PBEffects::ChoiceBand] != nil
			for i in 0...@attacker.moves.length
				if @attacker.moves[i].move==@attacker.effects[PBEffects::ChoiceBand]
					choiceindex = i
					break
				end
			end
		...
		if @attacker.effects[PBEffects::FutureSight] == 1
			move, moveuser = @attacker.pbFutureSightUserPlusMove
			damage = pbRoughDamageRejuv(move,moveuser,@attacker)
		...
		if !@battle.doublebattle && @opponent.effects[PBEffects::TwoTurnAttack]!=0 #this section really doesn't work in doubles.
			twoturntype = $cache.moves[@opponent.effects[PBEffects::TwoTurnAttack]].type
			for i in @mondata.party
				next if i.nil? || i.hp == 0 || @mondata.party.index(i) == @attacker.pokemonIndex
		...
		for oppmon in [@opponent,@opponent.pbPartner]
			...
			if !pbAIfaster?(nil,nil,@attacker,oppmon) && oppmon.effects[PBEffects::TwoTurnAttack]!=0
				twoturntype = $cache.moves[oppmon.effects[PBEffects::TwoTurnAttack]].type
		...
		if @mondata.roles.include?(:SWEEPER)
			statantiscore += (30)*@attacker.stages[:ATTACK] if @attacker.stages[:ATTACK]>0 && physmove
			statantiscore += (30)*@attacker.stages[:SPECIAL_ATTACK] if @attacker.stages[:SPECIAL_ATTACK]>0 && specialmove
			statantiscore += (30)*@attacker.stages[:SPEED] if @attacker.stages[:SPEED]>0 unless (@mondata.roles.include?(:PHYSICALWALL) || @mondata.roles.include?(:SPECIALWALL) || @mondata.roles.include?(:TANK))
			statantiscore += (30)*@attacker.effects[PBEffects::FocusEnergy]
		else
			statantiscore += (15)*@attacker.stages[:ATTACK] if @attacker.stages[:ATTACK]>0 && physmove
			statantiscore += (15)*@attacker.stages[:SPECIAL_ATTACK] if @attacker.stages[:SPECIAL_ATTACK]>0 && specialmove
			statantiscore += (15)*@attacker.stages[:SPEED] if @attacker.stages[:SPEED]>0 unless (@mondata.roles.include?(:PHYSICALWALL) || @mondata.roles.include?(:SPECIALWALL) || @mondata.roles.include?(:TANK))
			statantiscore += (30)*@attacker.effects[PBEffects::FocusEnergy]
		end
		if @mondata.roles.include?(:PHYSICALWALL)
			statantiscore += (30)*@attacker.stages[:DEFENSE] if @attacker.stages[:DEFENSE]>0
		else
			statantiscore += (15)*@attacker.stages[:DEFENSE] if @attacker.stages[:DEFENSE]>0
		end
		if @mondata.roles.include?(:SPECIALWALL)
			statantiscore += (30)*@attacker.stages[:SPECIAL_DEFENSE] if @attacker.stages[:SPECIAL_DEFENSE]>0
		else
			statantiscore += (15)*@attacker.stages[:SPECIAL_DEFENSE] if @attacker.stages[:SPECIAL_DEFENSE]>0
		end
		statantiscore += (20)*@attacker.stages[:EVASION] if @attacker.stages[:EVASION]>0 && !(checkAIaccuracy(aimem) || checkAIaccuracy(aimem2))
		...
		hazardantiscore+= (15) if (@attacker.pbOwnSide.effects[PBEffects::StickyWeb] && @mondata.roles.include?(:SWEEPER))
		...
		betterswitchscore+=90 if @attacker.effects[PBEffects::PerishSong]==0 && @attacker.pbHasMove?(:BATONPASS)
		...
		noswitchscore += 999 if Reborn && !@battle.doublebattle && @battle.opponentTrainer.name=="Priscilla"
	...
	def pbStatChangingSwitch(mon)
		...
		if mon.pbOwnSide.effects[PBEffects::StickyWeb] && !mon.isAirborne?
			...
			mon.stages[:SPEED]-= drop unless mon.item == :WHITEHERB || mon.ability == :WHITESMOKE || mon.ability == :CLEARBODY || mon.item == :HEAVYDUTYBOOTS
		...
		if mon.item == :IRONBALL && @battle.FE == :DEEPEARTH
			mon.stages[:SPEED] -=2 if !(mon.ability ==:CONTRARY)
			mon.stages[:SPEED] +=2 if (mon.ability ==:CONTRARY)
		end
		# Magnet Deep Earth
		...
		if mon.ability==:INTREPIDSWORD
			...
			mon.stages[:ATTACK]+=boost 
			mon.stages[:SPECIAL_ATTACK]+=1 if (@battle.FE == :FAIRYTALE || @battle.FE == :COLOSSEUM)
		end
		# Dauntless Shield
		if mon.ability==:DAUNTLESSSHIELD
			boost = (@battle.FE == :FAIRYTALE || @battle.FE == :COLOSSEUM) ? 2 : 1
			mon.stages[:DEFENSE]+=boost 
			mon.stages[:SPECIAL_DEFENSE]+=1 if (@battle.FE == :FAIRYTALE || @battle.FE == :COLOSSEUM)
		end
		# Steadfast
		if mon.ability==:STEADFAST && ((Rejuv && @battle.FE == :ELECTERRAIN) || @battle.state.effects[PBEffects::ELECTERRAIN] > 0)
			mon.stages[:SPEED]+=1
		end
		# Light Metal
		...
		if mon.ability==:MAGMAARMOR && (@battle.FE == :DRAGONSDEN || @battle.FE == :VOLCANIC)
			boost = 1
			mon.stages[:DEFENSE]+=boost 
			mon.stages[:SPECIAL_DEFENSE]+=boost if @battle.FE == :DRAGONSDEN
		end
		# Shell Armor
		...
		if @battle.FE == :STARLIGHT
			mon.stages[:SPECIAL_ATTACK]+=2 if mon.ability == :ILLUMINATE
		end
		# Water Compaction
		if @battle.FE == :MISTY || @battle.FE == :CORROSIVEMIST || @battle.state.effects[PBEffects::MISTY] > 0
			mon.stages[:DEFENSE]+=2 if mon.ability == :WATERCOMPACTION
		end
		# Mirror Field Evasion & Accuracy
		if @battle.FE == :MIRROR
			mon.stages[:EVASION]+=1 if [:SANDVEIL,:SNOWCLOAK,:TANGLEDFEET,:MAGICBOUNCE,:COLORCHANGE].include?(mon.ability)
			mon.stages[:EVASION]+=2 if mon.ability == :ILLUSION
			mon.stages[:ACCURACY]+=1 if [:KEENEYE,:COMPOUNDEYES].include?(mon.ability)
		end
		# Rattled 
		if mon.ability == :RATTLED
			mon.stages[:SPEED]+=1 if @battle.FE == :DIMENSIONAL || @battle.FE == :FROZENDIMENSION || @battle.FE == :HAUNTED
		end
		# Psychic Terrain
		if @battle.FE == :PSYTERRAIN || @battle.state.effects[PBEffects::PSYTERRAIN] > 0
			mon.stages[:SPECIAL_ATTACK]+=2 if mon.ability == :FOREWARN || mon.ability == :ANTICIPATION
		end
		# Dimensionals
		if @battle.FE == :DIMENSIONAL || @battle.FE == :FROZENDIMENSION
			mon.stages[:SPECIAL_ATTACK]+=1 if mon.ability == :BERSERK
			mon.stages[:ATTACK]+=1 if mon.ability == :JUSTIFIED || mon.ability == :ANGERPOINT
		end
		# Sky
		if @battle.FE == :SKY
			mon.stages[:DEFENSE]+=1 if mon.ability == :BIGPECKS
			mon.stages[:SPEED]+=1 if mon.ability == :LEVITATE || mon.ability == :SOLARIDOL || mon.ability == :LUNARIDOL
		end 
		# Infernal
		if @battle.FE == :INFERNAL
			mon.stages[:DEFENSE]+=1 if mon.ability == :MAGMAARMOR || mon.ability == :FLAMEBODY || mon.ability == :DESOLATELAND
			mon.stages[:SPECIAL_DEFENSE]+=1 if mon.ability == :MAGMAARMOR || mon.ability == :FLAMEBODY || mon.ability == :DESOLATELAND
		end
		# Colosseum
		if @battle.FE == :COLOSSEUM
			mon.stages[:DEFENSE]+=1 if  (mon.ability == :BATTLEARMOR || mon.ability == :SHELLARMOR)
			mon.stages[:SPECIAL_DEFENSE]+=1 if  (mon.ability == :MIRRORARMOR || mon.ability == :MAGICGUARD)
			mon.stages[:ATTACK]+=1 if mon.ability == :JUSTIFIED || mon.ability == :NOGUARD
			mon.stages[:SPECIAL_ATTACK]+=1 if mon.ability == :JUSTIFIED || mon.ability == :NOGUARD
		end
		# Concert
		if @battle.ProgressiveFieldCheck(PBFields::CONCERT)
			mon.stages[:DEFENSE]+=1 if [:HEAVYMETAL,:SOLIDROCK,:PUNKROCK,:SOUNDPROOF,:ROCKHEAD].include?(mon.ability)
			if  @battle.ProgressiveFieldCheck(PBFields::CONCERT,2,4)
				mon.stages[:SPEED]+=1 if mon.ability == :EMERGENCYEXIT || mon.ability == :RUNAWAY
				if  @battle.ProgressiveFieldCheck(PBFields::CONCERT,3,4)
					mon.stages[:SPEED]+=2 if mon.ability == :RATTLED
				end
			end
		end
		if @battle.FE == :BACKALLEY
			mon.stages[:DEFENSE]+=1 if  mon.ability == :ANTICIPATION || mon.ability == :FOREWARN
			mon.stages[:SPECIAL_DEFENSE]+=1 if  mon.ability == :ANTICIPATION || mon.ability == :FOREWARN
			mon.stages[:ATTACK]+=1 if mon.ability == :PICKPOCKET || mon.ability == :MERCILESS
			mon.stages[:SPECIAL_ATTACK]+=1 if mon.ability == :MAGICIAN
		end
		if @battle.FE == :CITY
			mon.stages[:DEFENSE]+=1 if  mon.ability == :BIGPECKS
			mon.stages[:ATTACK]+=1 if mon.ability == :EARLYBIRD
			mon.stages[:SPEED]+=1 if mon.ability == :PICKUP || mon.ability == :RATTLED
		end
		#Contrary	
		if mon.ability==:CONTRARY
	...
	def shouldHardSwitch?(attacker,switch_in_index)
		...
		assumed_damage = pbRoughDamageRejuv(assumed_move,@opponent,switch_in)
...
################################################################################
	...
	def addMonToMemory(pkmn,index)
		...
		return if !trainer #wild battle
		# @aiMoveMemory[trainer][pkmn.personalID] = [] 
	...
	def getAIMemory(battler=@opponent,inspecting=false)
		...
		trainer = @battle.pbGetOwner(battler.index)
		return [] if !trainer
		if (@mondata.index==battler.index || @mondata.index==battler.pbPartner&.index) && battler.is_a?(Battle::Battler) && inspecting!=true
		...
		elsif battler.is_a?(Battle::Battler) || inspecting == true
	...
	def checkAIMovePlusDamage(opponent=@opponent, attacker=@attacker, memory=nil, wholearray: false)
		...
		if @mondata.skill >= HIGHSKILL && memory.length < opponent.moves.count {|move| !move.nil?}
			unless memory.any? {|moveloop| moveloop!=nil && moveloop.pbType(opponent)==opponent.type1 && moveloop.betterCategory != :status}
				stabmove1 = Battle::Move_FFF.new(@battle,opponent, opponent.type1)
				damagearray.push(pbRoughDamageRejuv(stabmove1,opponent,attacker))
			...
			unless memory.any? {|moveloop| moveloop!=nil && moveloop.pbType(opponent)==opponent.type2 && moveloop.betterCategory != :status} || opponent.type2.nil?
				stabmove2 = Battle::Move_FFF.new(@battle,opponent, opponent.type2)
				damagearray.push(pbRoughDamageRejuv(stabmove2,opponent,attacker))
				movearray.push(stabmove2)
			end
		end
		for j in memory
			damagearray.push(pbRoughDamageRejuv(j,opponent,attacker))
			movearray.push(j)
		end
...
# AI Damage Calc
######################################################
	def pbRoughDamageRejuv(move=@move,attacker=@attacker,opponent=@opponent)
		...
		return 0 if opponent.totalhp == 1 && (opponent.ability == :STURDY || (opponent.ability == :STALWART && @battle.FE == :COLOSSEUM)) && move.pbNumHits(attacker, opponent)==1 && !attacker.effects[PBEffects::ParentalBond] && !attacker.effects[PBEffects::TyphBond] && !move.pbIsMultiHit && !moldBreakerCheck(attacker)
		if @mondata.skill>=MEDIUMSKILL
		  basedamage = pbBetterBaseDamage(move,attacker,opponent)
		end
		return 0 if move.zmove && ((opponent.effects[PBEffects::Disguise] || (opponent.effects[PBEffects::IceFace] && (move.pbIsPhysical? || @battle.FE == :FROZENDIMENSION))) && !moldBreakerCheck(opponent))
		...
		basedamage*=1.25 if (attacker.effects[PBEffects::ParentalBond] || attacker.effects[PBEffects::TyphBond]) && move.pbNumHits(attacker, opponent)==1
		...
		atk=attacker.attack
		atk=attacker.spatk if attacker.crested == :REUNICLUS && ((attacker.type1 != :FIGHTING && type == :FIGHTING) || (attacker.type1 == :FIGHTING && type == :PSYCHIC))
		atkstage=attacker.stages[:ATTACK]+6
		if attacker.species==:AEGISLASH
			originalform = attacker.form
			dummymon = pbAegislashStats(attacker)
			dummymon.pbUpdate
			atk=dummymon.attack
			atkstage=dummymon.stages[:ATTACK]+6
		...
		if move.function==0x121 # Foul Play
			atk=opponent.attack
			atkstage=opponent.stages[:ATTACK]+6
		end
		if move.pbIsSpecial?(type)
			atk=attacker.spatk
			atk=attacker.attack if attacker.crested == :REUNICLUS && ((attacker.type1 != :FIGHTING && type == :FIGHTING) || (attacker.type1 == :FIGHTING && type == :PSYCHIC))
			atkstage=attacker.stages[:SPECIAL_ATTACK]+6
			if attacker.species==:AEGISLASH
				originalform = attacker.form
				dummymon = pbAegislashStats(attacker)
				dummymon.pbUpdate
				atk=dummymon.spatk
				atkstage=dummymon.stages[:SPECIAL_ATTACK]+6
				dummymon.form = originalform
				dummymon.pbUpdate
			end
			if move.function==0x121 # Foul Play
				atk=opponent.spatk
				atkstage=opponent.stages[:SPECIAL_ATTACK]+6
			end
			if @battle.FE == :GLITCH
		...
		case attacker.crested
			when :CLAYDOL then atkstage=attacker.stages[:DEFENSE]+6 if move.pbIsSpecial?(type)
			when :DEDENNE then atkstage=attacker.stages[:SPEED]+6 if !move.pbIsSpecial?(type)
		end
		...
		if @mondata.skill>=BESTSKILL && @battle.FE != :INDOOR
			...
			when :CHESS
				...
				if (CHESSMOVES).include?(move.move)
					...
					if (opponent.ability == :OBLIVIOUS) || (opponent.ability == :KLUTZ) || (opponent.ability == :UNAWARE) || (opponent.ability == :SIMPLE) || (Rejuv && opponent.ability == :DEFEATIST) || opponent.effects[PBEffects::Confusion]>0
				...
				if Rejuv && attacker.effects[PBEffects::Illusion]
					basedamage = (basedamage*1.2).round
				end
			...
			when :SHORTCIRCUIT
				if type == :ELECTRIC
					damageroll = @battle.field.getRoll(update_roll: false, maximize_roll: (@battle.state.effects[PBEffects::ELECTERRAIN] > 0))
			...
			when :MIRROR
				if (PBFields::MIRRORMOVES).include?(move.move) && opponent.stages[:EVASION]>0
					provimult=2.0
		...
		if type == :POISON && (opponent.ability == :PASTELVEIL || opponent.pbPartner&.ability == :PASTELVEIL) && ([:MISTY,:RAINBOW].include?(@battle.FE) || @battle.state.effects[PBEffects::MISTY] > 0)
			basedamage = (basedamage*0.5).round
		end
		for terrain in [PBEffects::ELECTERRAIN,PBEffects::GRASSY,PBEffects::MISTY,PBEffects::PSYTERRAIN]
		...
		if @mondata.skill>=MEDIUMSKILL
		  ############ ATTACKER ABILITY CHECKS ############
			...
			#Plus/Minus
				...
				if move.pbIsSpecial?(type)
					...
					if partner.ability == :PLUS || partner.ability == :MINUS
						atk=(atk*1.5).round
					elsif (@battle.FE == :SHORTCIRCUIT || (Rejuv && @battle.FE == :ELECTERRAIN) || @battle.state.effects[PBEffects::ELECTERRAIN] > 0) && @mondata.skill>=BESTSKILL
						atk=(atk*1.5).round
					end
				end
			#Defeatist
			...
			#Pure/Huge Power
				...
				if @mondata.skill>=BESTSKILL
					if attacker.ability == :PUREPOWER && (@battle.FE == :PSYTERRAIN || @battle.state.effects[PBEffects::PSYTERRAIN] > 0)
			...
			#Flash Fire
			elsif attacker.effects[PBEffects::FlashFire]
				if type == :FIRE
					atk=(atk*1.5).round
				end
			#Slow Start
			...
			# Type Changing Abilities
				...
				# Galvanize
					...
					if @mondata.skill>=BESTSKILL
						if @battle.FE == :ELECTERRAIN || @battle.FE == :FACTORY || @battle.state.effects[PBEffects::ELECTERRAIN] > 0 # Electric or Factory Fields
							basedamage=(basedamage*1.5).round
				...
				# Pixilate
					...
					if @mondata.skill>=BESTSKILL
						if @battle.FE == :MISTY && @battle.state.effects[PBEffects::MISTY] > 0
			...
			# Quark Drive (offense boost)
				...
				basedamage=(basedamage*1.3).round if (attacker.effects[PBEffects::Quarkdrive][0] == :ATTACK && move.pbIsPhysical?(type)) || (attacker.effects[PBEffects::Quarkdrive][0] == :SPECIAL_ATTACK && move.pbIsSpecial?(type))
			# Execution
		  ...
		  ############ OPPONENT ABILITY CHECKS ############
			...
			############ MISC CHECKS ############
			# Charge
			if attacker.effects[PBEffects::Charge]>0 && type == :ELECTRIC
				basedamage=(basedamage*2.0).round
			end
			# Helping Hand
		...
		# Pinch Abilities
		if @mondata.skill>=BESTSKILL
			if ([:BURNING,:VOLCANIC,:INFERNAL].include?(@battle.FE) || attacker.effects[PBEffects::Blazed]) && attacker.ability == :BLAZE && type == :FIRE
				atk=(atk*1.5).round
		...
		defstage=opponent.stages[:DEFENSE]+6
		applysandstorm=false
		if move.pbHitsSpecialStat?(type)
			defense=opponent.spdef
			defstage=opponent.stages[:SPECIAL_DEFENSE]+6
			applysandstorm=true
			if @battle.FE == :GLITCH
		...
		if @mondata.skill>=MEDIUMSKILL
			...
			if opponent.ability == :MARVELSCALE
				if move.pbIsPhysical?(type)
					...
					elsif ([:MISTY,:RAINBOW,:FAIRYTALE,:DRAGONSDEN,:STARLIGHT].include?(@battle.FE) || @battle.state.effects[PBEffects::MISTY] > 0) && @mondata.skill>=BESTSKILL
						defense=(defense*1.5).round
					end
				end
			elsif opponent.ability == :GRASSPELT
				defense=(defense*1.5).round if move.pbIsPhysical?(type) && (@battle.FE == :GRASSY || @battle.FE == :FOREST || @battle.state.effects[PBEffects::GRASSY] > 0) # Grassy Field
				...
				defense=(defense*1.3).round if (opponent.effects[PBEffects::Quarkdrive][0] == :DEFENSE && move.pbIsPhysical?(type)) || (opponent.effects[PBEffects::Quarkdrive][0] == :SPECIAL_DEFENSE && move.pbIsSpecial?(type))
			end
		...
		if @mondata.skill>=HIGHSKILL && @battle.FE != 24 && oppitemworks
			...
			when :METALPOWDER
				defense=(defense*2.0).round if (opponent.pokemon.species == :DITTO) && !opponent.effects[PBEffects::Transform] && move.pbIsPhysical?(type)
			#when :EEVIUMZ
		...
		if @mondata.skill>=MEDIUMSKILL
			case @battle.pbWeather
				when :SUNNYDAY
					if @battle.state.effects[PBEffects::HarshSunlight] && type == :WATER
						damage=0
					end
					if type == :FIRE
				...
				when :RAINDANCE
					if @battle.state.effects[PBEffects::HeavyRain] && type == :FIRE
						damage=0
					end
					if type == :FIRE
						damage=(damage*0.5).round
		...
		if @mondata.skill>=MEDIUMSKILL
			...
			if (attacker.hasType?(type) && (!attacker.effects[PBEffects::DesertsMark])) || attacker.ability == :PROTEAN || attacker.ability == :LIBERO || typecrest==true
		...
		# Shelter
		if @mondata.skill>=MEDIUMSKILL
			if opponent.effects[PBEffects::Shelter] && @battle.FE != :INDOOR && (type == @battle.field.mimicry || move.getSecondaryType(attacker) == @battle.field.mimicry)
				damage=(damage*0.5).round
			end
		end
		# Screens
		...
		# pbModifyDamage - TODO
		if opponent.effects[PBEffects::Minimize] && (move.move == :BODYSLAM || move.function==0x10 ||
		...
		#Substitute damage
		if opponent.effects[PBEffects::Substitute] > 0 && attacker.ability != :INFILTRATOR && !move.isSoundBased? && 
		...
		case move.function
			...
			when 0x71 # Counter
				...
				for j in aimem
					...
					tempdam = pbRoughDamageRejuv(j,opponent,attacker)*2
			...
			when 0x75, 0x12D # Surf, Shadow Storm
				return basedamage*2 if !$cache.moves[opponent.effects[PBEffects::TwoTurnAttack]].nil? &&
				$cache.moves[opponent.effects[PBEffects::TwoTurnAttack]].function == 0xCB # Dive
			when 0x76 # Earthquake
				return basedamage*2 if !$cache.moves[opponent.effects[PBEffects::TwoTurnAttack]].nil? &&
				$cache.moves[opponent.effects[PBEffects::TwoTurnAttack]].function == 0xCA # Dig
			when 0x77, 0x78 # Gust, Twister
				return basedamage*2 if !$cache.moves[opponent.effects[PBEffects::TwoTurnAttack]].nil? &&
				($cache.moves[opponent.effects[PBEffects::TwoTurnAttack]].function == 0xC9 ||# Fly
				$cache.moves[opponent.effects[PBEffects::TwoTurnAttack]].function == 0xCC ||# Bounce
				$cache.moves[opponent.effects[PBEffects::TwoTurnAttack]].function == 0xCE )# Sky Drop
			...
			when 0x7C # SmellingSalt
				return basedamage*2 if opponent.status== :PARALYSIS  && opponent.effects[PBEffects::Substitute]<=0
			when 0x7D # Wake-Up Slap
				return basedamage*2 if opponent.status== :SLEEP && opponent.effects[PBEffects::Substitute]<=0
			when 0x7E # Facade
			...
			when 0x85 # Retaliate
				return basedamage*2 if attacker.pbOwnSide.effects[PBEffects::Retaliate]
			...
			when 0x8D # Gyro Ball
				...
				ospeed=pbRoughStatRejuv(opponent,:SPEED)
				aspeed=pbRoughStatRejuv(attacker,:SPEED)
				return [[(25*ospeed/aspeed).floor,150].min,1].max
			when 0x8E # Stored Power
				mult=0
				for i in [:ATTACK,:DEFENSE,:SPEED,
						:SPECIAL_ATTACK,:SPECIAL_DEFENSE,:ACCURACY,:EVASION]
			...
			when 0x92 # Echoed Voice
				return basedamage*(attacker.effects[PBEffects::EchoedVoice]+1)
			when 0x94 # Present
				return [attacker.happiness,250].min if attacker.crested == :LUVDISC
				return 50
			when 0x95 # Magnitude
				...
				damage *= 2 if !$cache.moves[opponent.effects[PBEffects::TwoTurnAttack]].nil? && $cache.moves[opponent.effects[PBEffects::TwoTurnAttack]].function==0xCA # Dig
				return damage
			when 0x96 # Natural Gift
				return [attacker.happiness,250].min if attacker.crested == :LUVDISC
			...
			when 0xD3 # Rollout
				if @mondata.skill>=MEDIUMSKILL
					return basedamage*2 if attacker.effects[PBEffects::DefenseCurl]
				end
			when 0xD4 # Bide
			...
			when 0x161 # First Impression
				...
				return 0
			when 0x171 # Stomping Tantrum
				return basedamage*2 if attacker.effects[PBEffects::Tantrum]
			...
			when 0x30A # Misty explosion
				return basedamage*1.5 if (@battle.FE == :MISTY || @battle.state.effects[PBEffects::MISTY] > 0)
			when 0x311 # Rising Voltage
				return basedamage*2 if (@battle.FE == :ELECTERRAIN || @battle.state.effects[PBEffects::ELECTERRAIN] > 0) && !opponent.isAirborne?
			when 0x314 # Lash Out
				...
				return basedamage*2 if attacker.effects[PBEffects::LashOut]
			when 0x319 # Surging Strikes
				return basedamage*4.5
			when 0x321 # Expanding Force
				return basedamage*1.5 if (@battle.FE == :PSYTERRAIN || @battle.state.effects[PBEffects::PSYTERRAIN] > 0) && !attacker.isAirborne?
			when 0x502 # Barb Barrage
				if opponent.status== :POISON
					return basedamage*2
				elsif @mondata.skill>=BESTSKILL
	...
	def pbRoughDamageRejuvAfterBoosts(move=@move,attacker=@attacker,opponent=@opponent,oppboosts: {},attboosts:{})
		...
		damage = pbRoughDamageRejuv(move,attacker,opponent)
	...
	def mirrorNeverMiss
		return (@attacker.stages[:ACCURACY] < 0 || @opponent.stages[:EVASION] > 0 || @opponent.item == :BRIGHTPOWDER || 
	...
	def ignitecheck
		return @battle.state.effects[PBEffects::WaterSport] <= 0 && @battle.pbWeather != :RAINDANCE
	end
	...
	def pbAegislashStats(aegi)
		if aegi.form==1
			...
			bladecheck.form = 1
			bladecheck.stages[:ATTACK] += 1 if @battle.FE == :FAIRYTALE && bladecheck.stages[:ATTACK]<6
			return bladecheck
		end
	end
...
#####################################################
	...
	def canGroundMoveHit?(battler)
		...
		return false if battler.hasType?(:FLYING) && battler.effects[PBEffects::Roost]==false && @battle.FE != :INVERSE
		return false if [:LEVITATE,:SOLARDIOL,:LUNARIDOL].include?(battler.ability)
		return false if battler.item == :AIRBALLOON && battler.itemWorks? && !battler.effects[PBEffects::DesertsMark]
...
class Battle::Move_FFF < Battle::Move	#Fake move used by AI to determine damage if no damaging AI memory move
...
class PokeBattle_AI_Info #info per battler for debuglogging
	...
	def reset(battler)
		...
		@battler_item								= battler.nil? || battler.item.nil? ? "" : battler.itemName
		@battler_ability							= battler.nil? || battler.ability.nil? ? "" : battler.abilityName
