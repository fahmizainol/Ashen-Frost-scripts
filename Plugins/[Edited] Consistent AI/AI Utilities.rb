...
class Battle::AI
	...
	def pbRoughDamage(move, user, target, skill, baseDmg=0)
		...
		elsif move.function == "UseUserBaseSpDefInsteadOfUserBaseSpAtk"   # Holy Ward # Changed by Jos 2022-10-07
		...
		# Changed 13-Oct-2023 by DemICE i have a feeling it was still not working properly as it was before.
		if (move.specialMove?(type) || move.function == "UseTargetSpDefInsteadOfTargetDefense") && # Mixed Waves, Alkaline Bomb
		...
		if skill >= PBTrainerAI.mediumSkill &&
			...
			(@battle.pbCheckGlobalAbility(:LIGHTAURA) && type == :LIGHT) || # Changed by Jos 2022-10-28 for new ability
		...
		# Changed by DemICE for AI to play around Ethereal.
		if skill >= PBTrainerAI.mediumSkill && target.hasActiveAbility?(:ETHEREAL) && 
		  target.effects[PBEffects::EtherealBroken]==false && move.contactMove?
		...
		if skill >= PBTrainerAI.mediumSkill
			...
			# Changed by Jos 2022-09-25 for Blackout
			if type == :LIGHT
				if @battle.allBattlers.any? { |b| b.effects[PBEffects::Blackout] }
				multipliers[:base_damage_multiplier] /= 3
				end
				if @battle.field.effects[PBEffects::BlackoutField] > 0
				multipliers[:base_damage_multiplier] /= 3
				end
			end
		end
		# Changed by DemICE 13-Sep-2024 Dev Battle specifics.
		if type == :LIGHT && @battle.backdrop == "demiceblackhole"
		  multipliers[:base_damage_multiplier] *= 0
		end
		if type == :DARK && @battle.backdrop == "demicesun"
		  multipliers[:base_damage_multiplier] *= 0
		end
		if type == :ICE && @battle.backdrop == "demiceouterspace"
		...
		# Terrain moves
		if skill >= PBTrainerAI.mediumSkill
			#Changed by DemICE 24-Oct-2023 buffed terrains
		...
		if Effectiveness.super_effective?(typeMod)
			...
			if target.itemActive?
				...
				when :OLIBERRY
					multipliers[:final_damage_multiplier]*=0.5 if type==:COSMMIC
				when :PATOTOBERRY
					multipliers[:final_damage_multiplier]*=0.5 if type==:LIGHT
				when :AVOCABERRY
					multipliers[:final_damage_multiplier]*=0.5 if type==:SOUND
				end
			end  
		end
		
		# Changed by DemICE 24-Sep-2023 making AI know about type shields.
		if target.itemActive?
			case target.item_id
			when :SHIELDNORMAL, :CHILANBERRY
				multipliers[:final_damage_multiplier]*=0.5 if type==:NORMAL
			when :SHIELDSTEEL
				multipliers[:final_damage_multiplier]*=0.5 if type==:STEEL
			when :SHIELDGROUND
				multipliers[:final_damage_multiplier]*=0.5 if type==:GROUND
			...
			when :SHIELDICE
				multipliers[:final_damage_multiplier]*=0.5 if type==:ICE
			when :SHIELDCOSMIC
				multipliers[:final_damage_multiplier]*=0.5 if type==:COSMIC
			when :SHIELDLIGHT
				multipliers[:final_damage_multiplier]*=0.5 if type==:LIGHT
			when :SHIELDSOUND
				multipliers[:final_damage_multiplier]*=0.5 if type==:SOUND
		...
		if skill >= PBTrainerAI.mediumSkill
			...
			when :Hail 
					...
					if target.pbHasType?(:ICE) && #Settings::HAIL_WEATHER_TYPE > 0 &&  # Changed by DemICE soulstones 2 Hail
						(move.physicalMove?(type) ||
						move.function == "UseTargetDefenseInsteadOfTargetSpDef")   # Psyshock
						multipliers[:defense_multiplier] *= 1.5
					end
				#end
		...
		# multipliers[:final_damage_multiplier] *=0.9 if $PokemonSystem.damage_variance==1  # Changed by DemICE 23-Sep-2023
    multipliers[:final_damage_multiplier] *= 0.9 if $PokemonGlobal.damage_variance == 1  # DemICE
	...
	def pbCheckMoveImmunity(score, move, user, target, skill)
		...
		if move.function == "FlinchTargetFailsIfNotUserFirstTurn" && user.turnCount > 0
			return true
		end
		...
		# Changed by Jos 2022-09-27 for new Cacophony ability
		# Changed by PDM 2024-12-18 for Headphone fix
		when :SOUND
			return true if target.hasActiveAbility?(:CACOPHONY,false,mold_broken) || target.hasActiveItem?(:HEADPHONES)
		# Changed by DemICE 2022-09-27 for new Opaqueness ability
		when :LIGHT
			return true if target.hasActiveAbility?(:OPAQUENESS,false,mold_broken)
		# Changed by DemICE 2022-09-27 for new improved Immunity
		when :POISON
			return true if target.hasActiveAbility?(:IMMUNITY,false,mold_broken)
		end
	...
	def pbEnemyItemToUse(idxBattler)
		...
		xItems = {
			# Changed by Jos 2022-10-10 for new vials
			:OFFENSEVIAL  		=> [:ATTACK, 1] 		&& [:SPEED, 1],
			:INTELLECTVIAL   	=> [:SPECIAL_ATTACK, 1] && [:SPECIAL_DEFENSE, 1],
			:PROTECTIONVIAL  	=> [:DEFENSE, 1] 		&& [:SPECIAL_DEFENSE, 1],
			:WISDOMVIAL   		=> [:SPECIAL_ATTACK, 1] && [:SPEED, 1],
			:TOUGHNESSVIAL   	=> [:ATTACK, 1] 		&& [:DEFENSE, 1],
			# End of addition
			...
			if chosenhpitem
				...
				if !targetSurvivesMove(maxmove,maxidx,maxattacker,battler)
					...
					if !fastermon
						...
						echo("\n")
						hpitemscore*=1.5 if battler.hasActiveAbility?(:FORTIFICATION)   # Changed by DemICE 30-Aug-2023 Soulstones 2 Specifics
		...
		if usableXItems.length>0
			...
			usableXItems.each do |i|
				...
				if user.hasActiveAbility?(:CONTRARY)
					...
					case i[0]
					#Changed by DemICE 30-Aug-2023 Soulstones 2 Specific vials section
					when :OFFENSEVIAL # Dragon Dance
						bestmove=bestMoveVsTarget(target,user,skill) # [maxdam,maxmove,maxprio,physorspec]
						maxdam=bestmove[0] 
						maxmove=bestmove[1]
						maxidx=bestmove[4]
						...
						if targetSurvivesMove(maxmove,maxidx,target,attacker,maxprio) || (target.status == :SLEEP && target.statusCount>1)
							...
							if skill>=PBTrainerAI.highSkill
								aspeed*=1.5 if user.hasActiveAbility?(:SPEEDBOOST)
								ospeed*=1.5 if target.hasActiveAbility?(:SPEEDBOOST)
								xitemscore += 80 if ((aspeed<ospeed) ^ (@battle.field.effects[PBEffects::TrickRoom]>1)) && ((aspeed*1.5>ospeed) ^ (@battle.field.effects[PBEffects::TrickRoom]>1))
							end
							xitemscore += 20 if halfhealth>maxdam
							xitemscore += 40 if thirdhealth>maxdam
						end
						...
						if user.statStageAtMax?(:ATTACK) &&
							...
							xitemscore -= user.stages[:ATTACK]*10
					...
					when :WISDOMVIAL # Macabre Dance
						...
						if user.statStageAtMax?(:SPECIAL_ATTACK) &&
							...
							xitemscore -= user.stages[:SPECIAL_ATTACK]*10
							xitemscore -= user.stages[:SPEED]*10
							if skill>=PBTrainerAI.mediumSkill
								hasSpecialAttack = false
								user.eachMove do |m|
					...
					when :TOUGHNESSVIAL # Bulk Up
						...
						mult=1.0
						mult=mult/1.5 if maxphys
						if targetSurvivesMove(maxmove,maxidx,target,attacker,maxprio,mult)|| (target.status == :SLEEP && target.statusCount>1)
							xitemscore += 40
							if maxphys
								xitemscore+=20 
								xitemscore+=20 if user.hasActiveAbility?(:FORTIFICATION)   # Changed by DemICE 30-Aug-2023 Soulstones 2 Specifics
							end
						...
						if user.statStageAtMax?(:ATTACK) &&
							...
							xitemscore -= user.stages[:ATTACK]*10
							xitemscore -= user.stages[:DEFENSE]*10
							if skill>=PBTrainerAI.mediumSkill
								hasPhysicalAttack = false
								user.eachMove do |m|
									next if !m.physicalMove?(m.type)
					...
					when :INTELLECTVIAL # Calm Mind
						...
						if user.statStageAtMax?(:SPECIAL_ATTACK) &&
							...
							xitemscore -= user.stages[:SPECIAL_ATTACK]*10
							xitemscore -= user.stages[:SPECIAL_DEFENSE]*10
							if skill>=PBTrainerAI.mediumSkill
					...
					when :PROTECTIONVIAL # Cosmic Power
						bestmove=bestMoveVsTarget(target,user,skill) # [maxdam,maxmove,maxprio,physorspec]
						maxdam=bestmove[0] 
						maxmove=bestmove[1]
						maxidx=bestmove[4]
						...
						if targetSurvivesMove(maxmove,maxidx,target,attacker,maxprio,mult) || (target.status == :SLEEP && target.statusCount>1)
							...
							if skill>=PBTrainerAI.highSkill
								aspeed*=1.5 if user.hasActiveAbility?(:SPEEDBOOST)
								ospeed*=1.5 if target.hasActiveAbility?(:SPEEDBOOST)
								xitemscore -= 90 if ((aspeed<ospeed) ^ (@battle.field.effects[PBEffects::TrickRoom]>0)) && maxdam>thirdhealth
							end
						end 
			...
			user.moves.each_with_index do |move,i|
				...
				for j in 0..@battle.battlers.length - 1
					...
					if move.function == "DoublePowerIfTargetNotActed"
						...
						maxidx=0
						maxmove=nil
						bestmove=bestMoveVsTarget(b,user,100,true) # [maxdam,maxmove,maxprio,physorspec]
						maxdam=bestmove[0] 
						maxidx=bestmove[4]
						maxmove=bestmove[1]
						maxprio=bestmove[2]
						priod=false
						priod=true if priorityAI(b,maxmove)>0 || (maxprio > maxdam*0.6)
    ...
    def targetSurvivesMove(move,idxmove,attacker,opponent,priodamage=0,mult=1,priomult=1,priotype=:NONE)
		...
		if !mold_broken && opponent.hasActiveAbility?(:TELEFACE) && move.physicalMove? && opponent.form==0
			if ["HitTwoToFiveTimes", "HitTwoTimes", "HitThreeTimes" ,"HitTwoTimesFlinchTarget", "HitThreeTimesPowersUpWithEachHit", "HitTenTimes"].include?(move.function)
				damage*=0.6
			else
				damage=1
		...
		# Changed by DemICE for AI to play around Ethereal.
		if !mold_broken && opponent.hasActiveAbility?(:ETHEREAL) && opponent.turnCount==0	
			if ["HitTwoToFiveTimes", "HitTwoTimes", "HitThreeTimes" ,"HitTwoTimesFlinchTarget", "HitThreeTimesPowersUpWithEachHit", "HitTenTimesPopulationBomb"].include?(move.function)
				damage*=0.6
			else
				damage=1
			end
		end	
		if !mold_broken && opponent.hasActiveAbility?(:ETHEREAL) && 
			opponent.effects[PBEffects::EtherealBroken]==false && move.contactMove?	
		#   if ["HitTwoToFiveTimes", "HitTwoTimes", "HitThreeTimes" ,"HitTwoTimesFlinchTarget", "HitThreeTimesPowersUpWithEachHit", "HitTenTimesPopulationBomb", "HitThreeTimesAlwaysCriticalHit"].include?(move.function)
		# 	  damage*=0.6
		#   else
			  damage=1
		  #end
	...
	def checkWeatherBenefit(user)
		...
		ownparty.each_with_index do |pkmn, idxParty|
			...
			if [:Hail].include?(@battle.pbWeather)
				...
				#if PluginManager.installed?("Generation 9 Pack")
					sum+=10 if pkmn.hasType?(:ICE) && @battle.pbWeather==:Hail #&& Settings::HAIL_WEATHER_TYPE > 0   # Changed by DemICE ss2 hail
				#end
				sum-=5 if pkmn.pbHasMoveFunction?("HealUserDependingOnWeather", "RaiseUserAtkSpAtk1Or2InSun", "TwoTurnAttackOneTurnInSun") && @battle.field.weather == :Sun
	...
	def priorityAI(user,move,switchin=false)
		...
		# Changed by DemICE 09-Oct-2023 Soulstones 2 Specifics   
		pri +=1 if (user.hasActiveAbility?(:LEADERSHIP) && turncount==0) || (user.hasActiveAbility?(:POUNCE) && turncount==0)  
