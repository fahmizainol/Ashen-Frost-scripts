...
class PokemonSaveScreen
  ...
  def doSave(slot)
    if $player.save_slot
      ...
      for i in 0..SaveData::MANUAL_SLOTS.length
        if SaveData::MANUAL_SLOTS[i]==$player.save_slot
          save_slot=i
        end  
      end  
      
      #Create the backup slots array for not new save files.
      if !$player.inf_backup_slots
        $player.inf_backup_slots=[0,0,0,0,0,0,0,0]
      end  

      #We increase the backup count for our current save slot
      $player.inf_backup_slots[save_slot]+=1
      
      #We piece together the backup file's name
      if $backupName!=""
        backup_slot=$player.save_slot + ' - ' + $backupName
      else  
        backup_slot = $player.save_slot + ' ' + $player.inf_backup_slots[save_slot].to_s + ' ' + "Case" + ' ' + $PokemonGlobal.case_num.to_s + ' ' + hour.to_s + 'h' + ' ' + min.to_s + 'm' + ' ' + mapname + + ' ' + $player.name
      end
    ...
    if backup_doSave(slot)    
      ...
      Game.save(backup_slot,true) if $player.save_slot
      return true
    end 
    return false 
  end
  ...
  def pbSaveScreen(exiting=false)
    ...
    if !$player.save_slot
      ...
      if opt == 0
        ...
        pbPlayDecisionSE
        $backupName = pbMessageFreeText(_INTL("Backup name:"),"",false,999,500)
        choices = [
          _INTL("Save to #{$player.save_slot}"),
          _INTL("Save to another file"),
          exiting ? _INTL("Quit without saving") : _INTL("Cancel")
  ...
  def pbSaveScreenJoiplay(exiting=false)
    ret = false
    @scene.pbStartScreen
    if !$player.save_slot
      # New Game - must select slot
