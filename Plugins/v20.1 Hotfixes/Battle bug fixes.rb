    ...
    if pbPlayer.party_full? && (@sendToBoxes == 0 || @sendToBoxes == 2)   # Ask/must add to party
      ...
      loop do
        ...
        when 0   # Add to your party
          ...
          # Account for instances where the Pokemon cannot be stored
          if pbPlayer.party[party_index]&.cannot_store
            pbMessage(_INTL("{1} refuses to go into storage!", pbPlayer.party[party_index].name)) 
            next false
          end 
    ...
    if !singleBattle?
      ...
      2.times do |side|
        ...
        allSameSideBattlers(side).each do |battler|
          ...
          next if pbGetOwnerIndexFromBattlerIndex(pos) != idxOwner && !(pbPlayerBattlerCount == 0 && pbOwnedByPlayer?(pos) && idxOwner != 0)
...
class Battle::Battler
  def pbCanInflictStatus?(newStatus, user, showMessages, move = nil, ignoreStatus = false)
    ...
    # Weather immunity
    if newStatus == :FROZEN && [:Sun, :HarshSun, :HolyInferno].include?(effectiveWeather)
      @battle.pbDisplay(_INTL("It doesn't affect {1}...", pbThis(true))) if showMessages
      return false
    end
    # Terrains immunity
