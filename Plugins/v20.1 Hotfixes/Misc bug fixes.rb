...
class PokemonEvolutionScene
  def pbEvolutionSuccess
    ...
    pbGetRecallableMoves(@pokemon) # DemICE Move Remember
...
class DayCare
  module EggGenerator
    ...
    def inherit_ability(egg, mother, father)
      ...
      # 50% chance to inherit the parent's ability
      # Otherwise, 25% chance to inherit one of the other two abilities
      if rand(100) < 50
        egg.ability_index = parent.ability_index 
      else
        egg.ability_index = ([0, 1, 2] - [parent.ability_index]).sample
      end
    end
  end
end
