...
#                                                                              #
# This Script is meant for the default Essentials battle system.                #
...
#------------------------------------------------------------------------------#
#                                                                              #
# Start a new save (Not nescessary, but just to be on the safe side)           #
#------------------------------------------------------------------------------#
...
alias dialogue_title pbCallTitle
def pbCallTitle
...
class PokemonTemp
  ...
  @dialogueInstances={}
  @orderData={}
  @nextturnmoves=[nil,nil,nil,nil,nil,nil,nil]
  end
  ...
  def nextturnmoves
  @nextturnmoves=[nil,nil,nil,nil,nil,nil,nil] if !@nextturnmoves
...
class Battle::DamageState
  ...
  attr_accessor :lowHP   # triggers at 25% maxhp
  attr_accessor :halfHP   # triggers at 50% maxhp
  attr_accessor :highHP  # triggers at 75% maxhp
...
# For Start Battle Messages
  def pbStartBattleSendOut(sendOuts)
    ...
    if wildBattle?
      ...
      when 1
        DiscordRPC.single_wild(foeParty[0]) if defined?(DiscordRPC) && $PokemonSystem.discordrpc == 0
        # Edited
      ...
      when 2
        DiscordRPC.double_wild(foeParty[0], foeParty[1]) if defined?(DiscordRPC) && $PokemonSystem.discordrpc == 0 
        case TrainerDialogue.eval("battleStart")
        when -1
      ...
      when 3
        DiscordRPC.triple_wild(foeParty[0], foeParty[1], foeParty[2]) if defined?(DiscordRPC) && 
        $PokemonSystem.discordrpc == 0
        case TrainerDialogue.eval("battleStart")
        when -1
      ...
      when 1
        DiscordRPC.single_trainer(@opponent[0]) if defined?(DiscordRPC) && $PokemonSystem.discordrpc == 0
        # Edited
        case TrainerDialogue.eval("battleStart")
        when -1
        ...
        when 0
          battleStart= TrainerDialogue.get("battleStart")
          pbDisplayPaused(_INTL(battleStart,@opponent[0].full_name))
        when 1
          battleStart= TrainerDialogue.get("battleStart")
          pbBGMPlay(battleStart["bgm"])
          pbDisplayPaused(_INTL(battleStart["text"],@opponent[0].full_name))
        when 2
          battleStart= TrainerDialogue.get("battleStart")
          battleStart.call(self)
        when 3
          battleStart= TrainerDialogue.get("battleStart")
          for i in 0...battleStart.length
            pbDisplayPaused(_INTL(battleStart[i],@opponent[0].full_name))
          end
        end
      when 2
        DiscordRPC.double_trainer(@opponent[0], @opponent[1]) if defined?(DiscordRPC) && $PokemonSystem.discordrpc == 0 
        case TrainerDialogue.eval("battleStart")
        when -1
          pbDisplayPaused(_INTL("You are challenged by {1} and {2}!",@opponent[0].full_name,@opponent[1].full_name))
        when 0
          battleStart= TrainerDialogue.get("battleStart")
          pbDisplayPaused(_INTL(battleStart,@opponent[0].full_name,@opponent[1].full_name))
        when 1
          battleStart= TrainerDialogue.get("battleStart")
          pbBGMPlay(battleStart["bgm"])
          pbDisplayPaused(_INTL(battleStart["text"],@opponent[0].full_name,@opponent[1].full_name))
        when 2
          battleStart= TrainerDialogue.get("battleStart")
          battleStart.call(self)
        when 3
          battleStart= TrainerDialogue.get("battleStart")
          for i in 0...battleStart.length
            pbDisplayPaused(_INTL(battleStart[i],@opponent[0].full_name,@opponent[1].full_name))
          end
        end
      when 3
        DiscordRPC.triple_trainer(@opponent[0], @opponent[1], @opponent[2]) if defined?(DiscordRPC) && 
        $PokemonSystem.discordrpc == 0
        case TrainerDialogue.eval("battleStart")
        when -1
        ...
        when 0
          battleStart= TrainerDialogue.get("battleStart")
          pbDisplayPaused(_INTL(battleStart,@opponent[0].full_name,@opponent[1].full_name,@opponent[2].full_name))
        when 1
          battleStart= TrainerDialogue.get("battleStart")
          pbBGMPlay(battleStart["bgm"])
          pbDisplayPaused(_INTL(battleStart,@opponent[0].full_name,@opponent[1].full_name,@opponent[2].full_name))
        ...
        when 3
          battleStart= TrainerDialogue.get("battleStart")
          for i in 0...battleStart.length
            pbDisplayPaused(_INTL(battleStart[i],@opponent[0].full_name,@opponent[1].full_name,@opponent[2].full_name))
          end
        end
  ...
  def pbUseItemOnBattler(item,idxParty,userBattler)
    ...
    if battler
      if ItemHandlers.triggerCanUseInBattle(item,battler.pokemon,battler,ch[3],true,self,@scene,false)
        ItemHandlers.triggerBattleUseOnBattler(item,battler,@scene,self)
        ...
        if !pbOwnedByPlayer?(idxBattler)   # Opponent/ally is switching in
          ...
          if @internalBattle && @switchStyle && trainerBattle? && pbSideSize(0)==1 &&
            ...
            if pbDisplayConfirm(_INTL("{1} is about to send in {2}. Will you switch your Pokémon?",
               opponent.full_name, GameData::Species.get(enemyParty[idxPartyForName].species).name)) # DemICE: Changed to display species name instead of nickname
               $ShiftSwitch=false
...
# Loss and Win Dialogue
  def pbEndOfBattle
    ...
    pbParty(0).each_with_index do |pkmn,i|
      ...
      pkmn.item = @defeatItems[0][i]  if [2,5,6].include?(@decision) # DemICE attempt to recover items upon defeat/forfeit
    end
    return @decision
  end
end
...
# HP Reduction Dialogue
  ...
  elsif self.hp < (self.totalhp*0.75).floor && self.hp > (self.totalhp*0.5).floor && !self.damageState.highHP
      self.damageState.highHP = true
      if !opposes?
        TrainerDialogue.display("highHP",@battle,@battle.scene)
      else
        TrainerDialogue.display("highHPOpp",@battle,@battle.scene)
      end
    end
    return amt
  end
...
# Big,Small and Low, Mid HP Dialogue Dialogue
  def pbEndureKOMessage(target)
    ...
    if target.hp < (target.totalhp*0.25).floor && target.hp>0
      if !target.damageState.lowHP
        ...
        target.damageState.halfHP = true
    target.damageState.highHP = true
        if !target.opposes?
          TrainerDialogue.display("lowHP",@battle,@battle.scene)
        else
          TrainerDialogue.display("lowHPOpp",@battle,@battle.scene)
  ...
  elsif target.hp < (target.totalhp*0.75).floor && target.hp>0
      if !target.damageState.highHP
        target.damageState.highHP = true
        if !target.opposes?
          TrainerDialogue.display("highHP",@battle,@battle.scene)
        else
...
# Sendout Dialogue
  ...
  def pbSendOutBattlers(sendOuts,startBattle=false)
    ...
    TrainerDialogue.forceSet("highHP#{side[b[0]]},#{len2-1}")
    ...
    TrainerDialogue.forceSet("fainted#{side[b[0]]},#{len2-1}")
      end
...
module TrainerDialogue
  ...
  $PokemonTemp.nextturnmoves=[nil,nil,nil,nil,nil,nil,nil]
  end
  ...
  def self.changeTrainerSprite(name,scene,delay=2)
    if name.is_a?(String)
      scene.sprites["trainer_1"].setBitmap(GameData::TrainerType.front_sprite_filename(name))
    elsif name.is_a?(Array)
      for i in 0...name.length
        Graphics.update
        pbWait(delay-1)
        scene.sprites["trainer_1"].setBitmap(GameData::TrainerType.front_sprite_filename(name))#"Graphics/Trainers/#{name[i]}")
      end
    end
  end

  def self.setInstance(parameter)
    noIncrement = ["lowHP","lowHPOpp","halfHP","halfHPOpp","highHP","highHPOpp","bigDamage","bigDamageOpp","smlDamage",
      "smlDamageOpp","attack","attackOpp","superEff","superEffOpp","notEff","notEffOpp","fainted","faintedOpp","roomOff"]
...
class Battle::Scene
  ...
  def pbInitSprites
    mbd_initSprites
    if @battle.wildBattle?
      trainerfile = "Graphics/Pokemon/Front/000"
      spriteX, spriteY = Battle::Scene.pbTrainerPosition(1,1,1)
  ...
  def pbShowOpponent(idxTrainer,priority=false)
    ...
    appearAnim = Battle::Scene::Animation::TrainerAppear.new(@sprites,@viewport,idxTrainer)
...
class DataboxFadeAnimation < Battle::Scene::Animation
...
class DataboxUnfadeAnimation < Battle::Scene::Animation
  def initialize(sprites,viewport,battlers,delay=nil,specific=nil)
    @battlers = battlers
    @delay = delay
    @specific = specific
...
class BlackBarAppearAnimation < Battle::Scene::Animation
...
class BlackBarDisappearAnimation < Battle::Scene::Animation
  def initialize(sprites,viewport,battlers)
    @battlers = battlers
    super(sprites,viewport)
  end
...
class Battle
  ...
  def pbForceMove(idxBattler, idxMove,target)
    battler = @battlers[idxBattler]
    move = battler.moves[idxMove]
    @choices[idxBattler][0] = :UseMove   # "Use move"
    @choices[idxBattler][1] = idxMove    # Index of move to be used
    @choices[idxBattler][2] = move       # Battle::Move object
    @choices[idxBattler][3] = target         # Target index
  end
  
  alias dialogue_attack pbAttackPhase
  def pbAttackPhase
    #forcing moves even if it can't use it
  if $PokemonTemp.nextturnmoves[6]==@turnCount
    (0..maxBattlerIndex).each do |i|
      if $PokemonTemp.nextturnmoves[i]!=nil
        @choices[i][0]==:UseMove
        pbForceMove(i,$PokemonTemp.nextturnmoves[i][0],$PokemonTemp.nextturnmoves[i][1])
      end
    end
  end
  dialogue_attack
