...
class Battle::Scene::PokemonDataBox < SpriteWrapper
  def initializeDataBoxGraphic(sideSize)
    ...
    case sideSize
    when 2
      @spriteX += (PluginManager.installed?("Modular Battler Scene")) ? [0,0,0,0][@battler.index] : [0, 12, 12, 0][@battler.index]
      @spriteY += [-20, -34, 34, 20][@battler.index]
    when 3
      @spriteX += (PluginManager.installed?("Modular Battler Scene")) ? [0,0,0,0,0,0][@battler.index] : [0, 12, 6, 6, 12, 0][@battler.index]
