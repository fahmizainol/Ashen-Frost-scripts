class PBAI
  ...
  class ScoreHandler
    ...
    def self.trigger_general(score, ai, user, target, move)
      trigger(@@GeneralCode, score, ai, user, target, move)
    end

    def self.trigger_status_moves(score, ai, user, target, move)
      trigger(@@StatusCode, score, ai, user, target, move)
    end

    def self.trigger_damaging_moves(score, ai, user, target, move)
      trigger(@@DamagingCode, score, ai, user, target, move)
    end

    def self.trigger_final(score, ai, user, target, move)
      trigger(@@FinalCode, score, ai, user, target, move)
    end

    def self.trigger_move(move, score, ai, user, target)
      id = move.id
      id = move.function unless @@MoveCode[id]
      trigger(@@MoveCode[id], score, ai, user, target, move)
    end
  end
end
...
PBAI::ScoreHandler.add do |score, ai, user, target, move|
  ...
  if user.hasActiveItem?(:THROATSPRAY)
    ...
    if user.has_role?(%i[SETUPSWEEPER WINCON SPECIALBREAKER]) # .include?(roles)
      score += 1
...
PBAI::ScoreHandler.add do |score, ai, user, target, move|
  if !move.is_a?(Battle::Move::FixedDamageMove) || move.function == 'OHKO' || move.function == 'MultiTurnAttackBideThenReturnDoubleDamage'
    next
  end
...
PBAI::ScoreHandler.add do |score, ai, user, target, move|
  if move.pbContactMove?(user) && user.discourage_making_contact_with?(target)
...
PBAI::ScoreHandler.add_damaging do |score, ai, user, target, move|
  next unless move.pbCouldBeCritical?(user.battler, target.battler)
...
PBAI::ScoreHandler.add_damaging do |score, ai, user, target, move|
  if move.is_a?(Battle::Move::RecoilMove) && !user.hasActiveAbility?(%i[ROCKHEAD MAGICGUARD])
...
# TODO: Add another ScoreHandler for setupping if target can't harm ai
PBAI::ScoreHandler.add do |score, ai, user, target, move|
  next unless PBAI::AI_Move.offense_setup_move?(move)
  ...
  for j in target.moves
    ...
    threat += 1 if user.get_calc(target, j) >= user.hp / 3
  end
  if threat == 0 && user.set_up_score < 3
    add = 9 - user.set_up_score
    score += add
    PBAI.log_score("+ #{add} to encourage setup when target cant kill in 3 turns")
  end
end
#
...
PBAI::ScoreHandler.add do |score, ai, user, target, move|
  next unless PBAI::AI_Move.defense_setup_move?(move)
...
PBAI::ScoreHandler.add do |score, ai, user, target, move|
  next unless PBAI::AI_Move.speed_setup_move?(move)
...
PBAI::ScoreHandler.add_status do |score, ai, user, target, move|
  next unless PBAI::AI_Move.status_condition_move?(move)
  ...
  ability_list = %i[MAGICBOUNCE GOODASGOLD PURIFYINGSALT GUTS COMATOSE FAIRYBUBBLE MARVELSCALE QUICKFEET]
  can_status = true
  case move.id
  when :WILLOWISP
    flag = :burn
    ability_list = %i[MAGICBOUNCE GOODASGOLD PURIFYINGSALT WATERVEIL WATERBUBBLE GUTS COMATOSE FAIRYBUBBLE
                      FLAREBOOST MARVELSCALE WELLBAKEDBODY STEAMENGINE FLASHFIRE QUICKFEET]
    can_status = target.can_burn?(user, move)
  when :DEEPFREEZE
    flag = :frostbite
    ability_list = %i[MAGICBOUNCE GOODASGOLD PURIFYINGSALT MAGMAARMOR GUTS COMATOSE FAIRYBUBBLE MARVELSCALE
                      QUICKFEET]
    can_status = target.can_freeze?(user, move)
  when :THUNDERWAVE, :GLARE, :STUNSPORE
    flag = :paralysis
    ability_list = %i[MAGICBOUNCE GOODASGOLD PURIFYINGSALT LIMBER GUTS COMATOSE FAIRYBUBBLE MARVELSCALE
                      QUICKFEET]
    can_status = target.can_paralyze?(user, move)
  when :POISONGAS, :TOXIC, :POISONPOWDER, :TOXICTHREAD
    flag = :poison
    ability_list = %i[MAGICBOUNCE GOODASGOLD PURIFYINGSALT IMMUNITY TOXICBOOST POISONHEAL GUTS QUICKFEET
                      COMATOSE FAIRYBUBBLE MARVELSCALE PASTELVEIL QUICKFEET]
    can_status = target.can_poison?(user, move)
  when :SPORE, :SING, :SLEEPPOWDER, :YAWN, :HYPNOSIS, :DARKVOID
    flag = :sleep
    ability_list = %i[MAGICBOUNCE GOODASGOLD PURIFYINGSALT INSOMNIA SWEETVEIL VITALSPIRIT FAIRYBUBBLE GUTS
                      COMATOSE MARVELSCALE QUICKFEET]
  ...
  if (flag == :paralysis) && user.has_role?(:SPEEDCONTROL)
    score += 1
    PBAI.log_score('+ 1')
  end
  if (flag == :poison) && user.has_role?(:TOXICSTALLER)
    score += 2
...
PBAI::ScoreHandler.add('GiveUserStatusToTarget') do |score, ai, user, target, move|
  if user.has_non_volatile_status?
    ...
    unless target.has_non_volatile_status?
...
PBAI::ScoreHandler.add('AddSpikesToFoeSide', 'AddToxicSpikesToFoeSide', 'AddStealthRocksToFoeSide',
                       'AddStickyWebToFoeSide') do |score, ai, user, target, move|
  ded = 0
  mov = []
  ...
  if ded > 0
    PBAI.log_score("Skip scoring for hazards because #{target.pokemon.name} does #{user.get_calc(target,
                                                                                                 mov[0])} damage with #{mov[0].name}")
    next
  end
  ...
  if move.function == 'AddSpikesToFoeSide' && user.opposing_side.effects[PBEffects::Spikes] >= 3 ||
    ...
    removal = 0
    target.moves.each { |move| removal += 1 if %i[RAPIDSPIN MORTALSPIN DEFOG TIDYUP].include?(move.id) }
    if removal > 0
      score -= 20
...
PBAI::ScoreHandler.add('UserFaintsExplosive',
                       'UserFaintsPowersUpInMistyTerrainExplosive') do |score, ai, user, target, move|
  unless user.can_switch?
    score -= 10
...
PBAI::ScoreHandler.add('StartLeechSeedTarget') do |score, ai, user, target, move|
  ...
  if !user.underdog?(target) && !target.has_type?(:GRASS) && target.effects[PBEffects::LeechSeed] == 0
    ...
    if user.has_role?(%i[PHYSICALWALL SPECIALWALL DEFENSIVEPIVOT]) # .include?(user.role)
      score += 3
      PBAI.log_score('+ 3')
    end
  end
...
PBAI::ScoreHandler.add('SwitchOutTargetStatusMove', 'SwitchOutTargetDamagingMove', 'SwitchOutUserDamagingMove',
                       'LowerTargetAtkSpAtk1SwitchOutUser', 'SwitchOutUserStartHailWeather') do |score, ai, user, target, move|
  if user.bad_against?(target) && !target.has_ability?(:SUCTIONCUPS) && !target.effects[PBEffects::Ingrain] && !%w[
    SwitchOutUserDamagingMove LowerTargetAtkSpAtk1SwitchOutUser SwitchOutUserStartHailWeather
  ].include?(move.function)
    score += 1
    PBAI.log_score("+ 1 for forcing our target to switch and we're bad against our target")
    o_boost = 0
    faint = 0
    GameData::Stat.each_battle { |s| o_boost += target.stages[s] unless target.stages[s].nil? }
  ...
  elsif %w[SwitchOutUserDamagingMove LowerTargetAtkSpAtk1SwitchOutUser
           SwitchOutUserStartHailWeather].include?(move.function)
    roles = []
    for i in user.roles
      roles.push(i)
    end
    if user.has_role?(%i[DEFENSIVEPIVOT OFFENSIVEPIVOT LEAD]) # .include?(roles)
      score += 1 if user.can_switch?
      PBAI.log_score('+ 1 for being a Pivot or Lead')
    end
    boosts = 0
    o_boost = 0
    GameData::Stat.each_battle { |s| boosts += user.stages[s] unless user.stages[s].nil? }
    boosts *= -1
    score += boosts
    GameData::Stat.each_battle { |s| o_boost += target.stages[s] unless target.stages[s].nil? }
    if boosts > 0
  ...
  if target.hasActiveAbility?(%i[MAGICBOUNCE GOODASGOLD]) && move.statusMove?
    score -= 20
...
PBAI::ScoreHandler.add('HealUserHalfOfTotalHP', 'HealUserHalfOfTotalHPLoseFlyingTypeThisTurn',
                       'HealUserPositionNextTurn') do |score, ai, user, target, move|
  factor = 1 - user.hp / user.totalhp.to_f
  # At full hp, factor is 0 (thus not encouraging this move)
  # At half hp, factor is 0.5 (thus slightly encouraging this move)
  # At 1 hp, factor is about 1.0 (thus encouraging this move)
  ...
  if user.flags[:will_be_healed] && ai.battle.pbSideSize(0) == 2
    ...
    if user.is_healing_pointless?(0.50)
      ...
      if user.has_role?(%i[PHYSICALWALL SPECIALWALL TOXICSTALLER DEFENSIVEPIVOT OFFENSIVEPIVOT CLERIC]) # .include?(roles)
        score += 1
  ...
  if user.has_role?(:CLERIC) && move.function == 'HealUserPositionNextTurn'
    PBAI.log_score('+ 1  and potentially passing a Wish')
  end
...
PBAI::ScoreHandler.add('HealUserDependingOnWeather') do |score, ai, user, target, move|
  heal_factor = 0.5
  heal_factor = case ai.battle.pbWeather
                when :Sun, :HarshSun
                  2.0 / 3.0
...
PBAI::ScoreHandler.add('HealUserDependingOnSandstorm') do |score, ai, user, target, move|
  heal_factor = 0.5
  heal_factor = 2.0 / 3.0 if ai.battle.pbWeather == :Sandstorm
  factor = 1 - user.hp / user.totalhp.to_f
  # At full hp, factor is 0 (thus not encouraging this move)
  # At half hp, factor is 0.5 (thus slightly encouraging this move)
  # At 1 hp, factor is about 1.0 (thus encouraging this move)
...
PBAI::ScoreHandler.add('DisableTargetStatusMoves') do |score, ai, user, target, move|
  if target.flags[:will_be_taunted] && ai.battle.pbSideSize(0) == 2
    ...
    setup_moves = %i[SWORDSDANCE WORKUP NASTYPLOT GROWTH HOWL BULKUP CALMMIND TAILGLOW AGILITY ROCKPOLISH AUTOTOMIZE
    ...
    for i in target.moves
      setup = true if setup_moves.include?(i.id)
    end
    if setup == true
      score += 2
  ...
  if target.hasActiveAbility?(%i[MAGICBOUNCE GOODASGOLD AROMAVEIL OBLIVIOUS])
    score -= 20
...
PBAI::ScoreHandler.add('ResetAllBattlersStatStages') do |score, ai, user, target, move|
  ...
  user.side.battlers.each do |proj|
    ...
    GameData::Stat.each_battle { |s| net -= proj.stages[s] unless proj.stages[s].nil? }
  end
  ...
  if net < 0
    ...
    if user.has_role?(%i[STALLBREAKER PHAZER]) # #.include?(roles)
      score += 1
...
PBAI::ScoreHandler.add('RaiseUserSpDef1PowerUpElectricMove') do |score, ai, user, target, move|
  if (target.types.include?(:GROUND) || target.hasActiveAbility?(%i[LIGHTNINGROD MOTORDRIVE
                                                                    VOLTABSORB])) || (user.effects[PBEffects::Charge] != 0 || user.charge != 0)
    score -= 10
...
PBAI::ScoreHandler.add('ProtectUser', 'ProtectUserFromDamagingMovesKingsShield') do |score, ai, user, target, move|
  ...
  if user.effects[PBEffects::Substitute] > 0 && user.effects[PBEffects::ProtectRate] == 1
    ...
    if target.effects[PBEffects::LeechSeed] || %i[POISON BURN FROZEN].include?(target.status)
      score += 1
      PBAI.log_score('+ 1 for forcing opponent to take residual damage')
    end
  end
  if (user.hasActiveItem?(:FLAMEORB) && user.status == :NONE && user.hasActiveAbility?(%i[GUTS
                                                                                          MARVELSCALE])) || ((user.hasActiveItem?(:TOXICORB) || ai.battle.field.terrain == :Poison) && user.hasActiveAbility?(%i[
                                                                                                                                                                                                                TOXICBOOST POISONHEAL GUTS
                                                                                                                                                                                                              ]) && user.affectedByTerrain? && user.status == :NONE)
    score += 25
    PBAI.log_score('+ 25 for getting a status to benefit their ability')
  end
  if %i[POISON BURN FROZEN].include?(target.status)
  ...
  PBAI.log_score('- 2 for predicting the switch') if user.predict_switch?(target)
  ...
  elsif user.turnCount == 0 && user.hasActiveAbility?(:SPEEDBOOST)
    score += 3
...
PBAI::ScoreHandler.add('SwitchOutUserStatusMove') do |score, ai, user, target, move|
  ...
  if user.has_role?(%i[PHYSICALWALL SPECIALWALL DEFENSIVEPIVOT OFFENSIVEPIVOT TOXICSTALLER LEAD])
    score += 1
    PBAI.log_score('+ 1 ')
  end
  fnt = 0
...
PBAI::ScoreHandler.add('RedirectAllMovesToUser', 'UserSwapsPositionsWithAlly') do |score, ai, user, target, move|
  if ai.battle.pbSideSize(0) == 2
    ...
    if ally
      ...
      if !$chosen_move.nil? && ($chosen_move.id == :PROTECT)
        score = 0
...
PBAI::ScoreHandler.add('PowerUpAllyMove') do |score, ai, user, target, move|
  if ai.battle.pbSideSize(0) == 2
    ...
    if ally
      ...
      mon = user.side.battlers.find { |proj| proj && proj != self && !proj.fainted? }
      ally_kill = # the 1.5x multiplier here is to consider the Helping Hand boost
        mon.moves.any? do |m|
          (mon.get_calc_self(target, m) * 1.5) >= target.hp
        end
...
PBAI::ScoreHandler.add('TargetActsNext') do |score, ai, user, target, move|
  if ai.battle.pbSideSize(0) == 2
    ...
    if ally
      ...
      ally_kill = # the 1.5x multiplier here is to consider the Helping Hand boost
        mon.moves.any? do |m|
          mon.get_calc_self(target, m) >= target.hp
        end
      target_kill_ally = target.moves.any? { |m2| mon.get_calc(target, m2) >= mon.hp }
...
PBAI::ScoreHandler.add('AttackerFaintsIfUserFaints') do |score, ai, user, target, move|
  ...
  prio_kill = target.moves.any? do |mo|
    user.get_calc(target, mo) >= user.hp && mo.priority > 0
  end && !user.priority_blocking?
  fast_kill = user.faster_than?(target) && user.moves.any? do |mv|
    user.get_calc_self(target, mv) >= target.hp
  end && !prio_kill
  if dmg && !fast_kill
    score += 50
    PBAI.log_score('+ 50 to go for Destiny Bond')
    unless target.moves.any? { |m| m.statusMove? }
      score += 50
...
# Belly Drum
PBAI::ScoreHandler.add('MaxUserAttackLoseHalfOfTotalHP',
                       'RaiseUserAtk2SpAtk2Speed2LoseHalfOfTotalHP') do |score, ai, user, target, move|
  ded = 0
  me_ded = 0
  for i in user.moves
    next if i.statusMove?
...
PBAI::ScoreHandler.add('FailsIfNotUserFirstTurn') do |score, ai, user, target, move|
  if user.turnCount == 0 && ai.battle.field.terrain != :Psychic && !target.hasActiveAbility?(%i[ARMORTAIL DAZZLING
                                                                                                QUEENLYMAJESTY])
    score += 4
...
PBAI::ScoreHandler.add('DoublePowerIfTargetStatusProblem', 'DoublePowerIfTargetPoisonedPoisonTarget',
                       'DoublePowerIfTargetStatusProblemBurnTarget', 'DoublePowerIfTargetStatusProblemFrostbiteTarget') do |score, ai, user, target, move|
...
PBAI::ScoreHandler.add('FailsIfUserHasUnusedMove') do |score, ai, user, target, move|
  ...
  used.each do |move2|
    unused_moves = true unless user.movesUsed.include?(move2)
  end
...
PBAI::ScoreHandler.add('ParalyzeTargetIfNotTypeImmune') do |score, ai, user, target, move|
  if move.statusMove? && !user.target_is_immune?(move,
                                                 target) && !target.hasActiveAbility?(%i[LIMBER COMATOSE VOLTABSORB GOODASGOLD
                                                                                         HOPEFULTOLL]) && user.has_role?(:SPEEDCONTROL)
    score += 2
  ...
  if ((target.status != :NONE || target.pbHasType?(:GROUND) || target.pbHasType?(:ELECTRIC) || target.hasActiveAbility?(%i[
                                                                                                                          LIMBER COMATOSE VOLTABSORB GOODASGOLD
                                                                                                                        ])) && move.id == :THUNDERWAVE) ||
     (target.status != :NONE || target.pbHasType?(:ELECTRIC) || target.hasActiveAbility?(%i[LIMBER COMATOSE VOLTABSORB
                                                                                            GOODASGOLD HOPEFULTOLL]))
    score -= 20
...
PBAI::ScoreHandler.add do |score, ai, user, target, move|
  ...
  next unless PBAI::AI_Move.weather_terrain_move?(move)

  weather = %i[Sun Rain Snow Sandstorm Electric Grassy Misty Psychic]
  setter = [[:SUNNYDAY], [:RAINDANCE], %i[HAIL SNOWSCAPE CHILLYRECEPTION], [:SANDSTORM], [:ELECTRICTERRAIN],
            [:GRASSYTERRAIN], [:MISTYTERRAIN], [:PSYCHICTERRAIN]]
  ability = [
    %i[SOLARPOWER CHLOROPHYLL PROTOSYNTHESIS FLOWERGIFT HARVEST FORECAST STEAMPOWERED],
    %i[SWIFTSWIM RAINDISH DRYSKIN FORECAST STEAMPOWERED],
    %i[ICEBODY SLUSHRUSH SNOWCLOAK ICEFACE FORECAST],
    %i[SANDRUSH SANDVEIL SANDFORCE FORECAST],
    %i[SURGESURFER QUARKDRIVE],
    [:MEADOWRUSH],
    [:NOCTEMBOOST],
    [:BRAINBLAST]
  ]
  ...
  if weather[idx] != ai.battle.pbWeather
    if user.has_role?(:WEATHERTERRAIN)
      mod = party.any? do |pkmn|
        !pkmn.fainted? && pkmn.has_role?(:WEATHERTERRAINABUSER) && ability[idx].include?(pkmn.ability_id)
      end
      add = mod ? 8 : 5
      score += add
...
PBAI::ScoreHandler.add_final do |score, ai, user, target, move|
  ...
  next unless ai.battle.doublebattle

  # If the move is a status move, we can assume it has a positive effect and thus would be good for our ally too.
  if (target.side != user.side) && !move.statusMove?
    target_type = move.pbTarget(user)
    # If the move also targets our ally
    # See if we have an ally
    if %i[AllNearOthers AllBattlers BothSides].include?(target_type) && (ally = user.side.battlers.find do |proj|
      proj && proj != user && !proj.fainted?
    end)
...
PBAI::ScoreHandler.add_final do |score, ai, user, target, move|
  ...
  next score unless PBAI::AI_Move.setup_move?(move)
...
PBAI::ScoreHandler.add_final do |score, ai, user, target, move|
  ...
  next if user.hasActiveAbility?(%i[AIRLOCK CLOUDNINE])
...
PBAI::ScoreHandler.add_final do |score, ai, user, target, move|
  next score if %i[FAKEOUT FIRSTIMPRESSION].include?(move.id) && ai.battle.turnCount == 0
  ...
  inflict_status = %w[BurnTarget FrostbiteTarget ParalyzeTargetIfNotTypeImmune PoisonTarget BadPoisonTarget
                      SleepTarget SleepTarget SleepTargetIfUserDarkrai AttackerFaintsIfUserFaints UserFaintsLowerTargetAtkSpAtk2]
