...
class Battle::Move::FixedDamageMove < Battle::Move
  ...
  def pbCalcDamage(user, target, numTargets = 1)
    ...
    target.damageState.calcDamage = self.function == "UserFaintsFixedDamageUserHP" ? user.hp : pbFixedDamage(user, target)
...
PBAI::AbilityEffects::ModifyMoveBaseType.add(:ENTYMATE,
  proc { |ability, user, move, type|
...
PBAI::AbilityEffects::SpeedCalc.add(:BACKDRAFT,
  proc { |ability, battler, mult|
    next mult * 2 if [:StrongWinds].include?(battler.effectiveWeather)
  }
)
