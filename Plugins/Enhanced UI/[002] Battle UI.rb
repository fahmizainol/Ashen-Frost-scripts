...
class Battle::Scene
  ...
  # White text.
  #-----------------------------------------------------------------------------
  BASE_LIGHT     = MESSAGE_BASE_COLOR
  SHADOW_LIGHT   = Color.new(72, 72, 72)
  #-----------------------------------------------------------------------------
  # Black text.
  #-----------------------------------------------------------------------------
  BASE_DARK      = Color.new(72, 72, 72)
  ...
  def pbFightMenu_EnhancedUI(battler, cw)
    # Keycode for M
    if (!$joiplay && Input.trigger?(Input::D)) || ($joiplay && Input.trigger?(Input::JUMPUP))
      pbHideBattleInfo
      pbHideFocusPanel
      pbToggleMoveInfo(battler, cw.index)
    # Keycode for N
    elsif (!$joiplay && Input.trigger?(Input::E)) || ($joiplay && Input.trigger?(Input::JUMPDOWN))
      pbHideMoveInfo
      pbHideFocusPanel
      pbToggleBattleInfo
    end
...
#===============================================================================
  ...
  def pbUpdateMoveInfoWindow(battler, index)
    ...
    @dmg_base   = @acc_base   = @eff_base   = BASE_DARK
    @dmg_shadow = @acc_shadow = @eff_shadow = SHADOW_DARK
    ...
    textPos.push(
      [move.name,       xpos + 10,            ypos + 8,  0, BASE_DARK, SHADOW_DARK],
      [_INTL("Pow:"),   Graphics.width - 86,  ypos + 10, 2, BASE_DARK, SHADOW_DARK],
      [_INTL("Acc:"),   Graphics.width - 86,  ypos + 38, 2, BASE_DARK, SHADOW_DARK],
      [_INTL("Effct:"), xpos + 286,           ypos + 38, 0, BASE_DARK, SHADOW_DARK],
      [power,           Graphics.width - 34,  ypos + 10, 2, @dmg_base, @dmg_shadow],
      [accuracy,        Graphics.width - 34,  ypos + 38, 2, @acc_base, @acc_shadow],
      [effectrate,      Graphics.width - 144, ypos + 38, 2, @eff_base, @eff_shadow]
    )
    pbDrawTextPositions(@moveUIOverlay, textPos)
    drawTextEx(@moveUIOverlay, xpos + 10, ypos + 70, Graphics.width - 10, 2, GameData::Move.get(move.id).description, BASE_DARK, SHADOW_DARK)
  end
...
#===============================================================================
  ...
  def pbSelectBattlerInfo
    ...
    @sprites["infoselect"].x = (@battle.pbSideBattlerCount(0) == 2) ? 68 : 174
    ...
    loop do
      ...
      oldSide = idxSide
      oldPoke = idxPoke
      break if Input.trigger?(Input::BACK) || (!$joiplay && Input.trigger?(Input::E)) || ($joiplay && Input.trigger?(Input::JUMPDOWN))
      if Input.trigger?(Input::USE)
        ...
        pbPlayCursorSE
      elsif cw.visible && (!$joiplay && Input.trigger?(Input::D)) || ($joiplay && Input.trigger?(Input::JUMPUP))
        switchUI = 1
        break
      ...
      if oldSide != idxSide || oldPoke != idxPoke
        ...
        when 1 then @sprites["infoselect"].x = 174
        when 2 then @sprites["infoselect"].x = 68 + (208 * idxPoke)
        when 3 then @sprites["infoselect"].x = 4 + (168 * idxPoke)
        end
        battler = battlers[idxSide][idxPoke]
        @battle.allBattlers.each do |b|
    ...
    2.times do |side|
      ...
      when 0
        @battle.allSameSideBattlers.each_with_index do |b, i|
          case count
          when 1 then iconX, bgX = 178, 174
          when 2 then iconX, bgX = 72 + (208 * i), 68 + (208 * i)
          when 3 then iconX, bgX = 32 + (168 * i), 4 + (168 * i)
          end
          iconY = ypos + 114
          nameX = iconX + 96
          if index == [side, i]
          ...
          @sprites["battler_icon#{b.index}"].y = iconY + 4
          pbSetWithOutline("battler_icon#{b.index}", [iconX, iconY + 4, 400])
          imagePos1.push([@path + "Battle Info/select", bgX, iconY - 28, 0, 0, 166, 52])
          imagePos2.push([@path + "Battle Info/panel_owner", bgX + 36, iconY + 10],
                         [@path + "Battle Info/battler_gender", bgX + 146, iconY - 36, b.gender * 22, 0, 22, 20])
          textPos.push([_INTL("{1}", b.pokemon.name), nameX, iconY - 14, 2, base, shadow],
                       [@battle.pbGetOwnerFromBattlerIndex(b.index).name, nameX - 10, iconY + 12, 2, BASE_LIGHT, SHADOW_LIGHT])
        end
        trainers = []
        @battle.player.each { |p| trainers.push(p) if p.able_pokemon_count > 0 }
        ballY = ypos + 154
        ballXFirst = 36
        ballXLast = Graphics.width - (16 * NUM_BALLS) - 36
        ballOffset = 2
      #-------------------------------------------------------------------------
      # Opponent's side.
      #-------------------------------------------------------------------------
      when 1
        @battle.allOtherSideBattlers.reverse.each_with_index do |b, i|
          ...
          if @battle.trainerBattle?
            imagePos2.push([@path + "Battle Info/panel_owner", bgX + 36, iconY + 12])
            textPos.push([@battle.pbGetOwnerFromBattlerIndex(b.index).name, nameX - 10, iconY + 16, 2, BASE_LIGHT, SHADOW_LIGHT])
          end
          imagePos2.push([@path + "Battle Info/battler_gender", bgX + 146, iconY - 36, b.displayPokemon.gender * 22, 0, 22, 20])
        end
        trainers = []
        @battle.opponent.each { |p| trainers.push(p) if p.able_pokemon_count > 0 } if @battle.opponent
        ballY = ypos - 18
        ballXFirst = Graphics.width - (16 * NUM_BALLS) - 36
        ballXLast = 36
        ballOffset = 2
      end
      #-------------------------------------------------------------------------
      ...
      trainers.each do |trainer|
        ...
        NUM_BALLS.times do |slot|
          ...
          imagePos2.push([@path + "Battle Info/party", ballX + (slot * 16), ballY, idx * 16, 0, 16, 16])
        end
      end
    end
    pbUpdateBattlerIcons
  ...
  def pbOpenBattlerInfo(battler, battlers)
    ...
    loop do
      ...
      if Input.trigger?(Input::LEFT)
        ...
        ret = 1
        break
      elsif PluginManager.installed?("Focus Meter System") && Input.trigger?(Settings::FOCUS_PANEL_KEY)
        ret = 2
        break
      elsif Input.trigger?(Input::USE) || (!$joiplay && Input.trigger?(Input::E)) || ($joiplay && Input.trigger?(Input::JUMPDOWN))
        ret = [0, 0]
        if battler.opposes?
          ret[0] = 1
  ...
  def pbUpdateBattlerInfo(battler)
    ...
    imagePos = [[@path + "Battle Info/info_bg", 0, 0],
                ...
                [@path + "Battle Info/battler_gender", xpos + 146, ypos + 22, poke.gender * 22, 0, 22, 20]]
    textPos  = [[_INTL("{1}", poke.name), iconX + 94, iconY - 16, 2, BASE_DARK, SHADOW_DARK],
                [_INTL("Lv. {1}", battler.level), xpos + 18, ypos + 106, 0, BASE_LIGHT, SHADOW_LIGHT],
    ...
    if battler.hp > 0
      ...
      imagePos.push(["Graphics/Pictures/Battle/overlay_hp", 86, 90, 0, hpzone * 6, w, 6])
    end
    # Battler status.
    if battler.status != :NONE
      iconPos = GameData::Status.get(battler.status).icon_position
      imagePos.push(["Graphics/Pictures/statuses", xpos + 86, ypos + 106, 0, iconPos * 16, 44, 16])
    end
    # Shininess
    imagePos.push(["Graphics/Pictures/shiny", xpos + 142, ypos + 106]) if poke.shiny?
    # Owner
    if !battler.wild?
      imagePos.push([@path + "Battle Info/panel_owner", xpos - 28, ypos + 4])
      textPos.push([@battle.pbGetOwnerFromBattlerIndex(battler.index).name, xpos + 28, ypos + 6, 2, BASE_LIGHT, SHADOW_LIGHT])
    end
    ...
    if battler.pbOwnedByPlayer?
      imagePos.push(
        [@path + "Battle Info/panel_owner", xpos + 36, iconY + 12],
        [@path + "Battle Info/panel_effects", panelX, 66, 0, 0, 218, 24],
        [@path + "Battle Info/panel_effects", panelX, 88, 0, 0, 218, 24]
      )
      textPos.push(
        [_INTL("Abil."), xpos + 272, ypos + 44, 2, BASE_LIGHT, SHADOW_LIGHT],
        [_INTL("Item"), xpos + 272, ypos + 66, 2, BASE_LIGHT, SHADOW_LIGHT],
        [_INTL("{1}", battler.abilityName), xpos + 376, ypos + 44, 2, BASE_DARK, SHADOW_DARK],
        [_INTL("{1}", battler.itemName), xpos + 376, ypos + 66, 2, BASE_DARK, SHADOW_DARK],
        [sprintf("%d/%d", battler.hp, battler.totalhp), iconX + 74, iconY + 16, 2, BASE_LIGHT, SHADOW_LIGHT]
      )
    end
    #---------------------------------------------------------------------------
  ...
  def pbAddTypesDisplay(xpos, ypos, battler, poke)
    ...
    displayTypes.each_with_index do |type, i|
      ...
      @infoUIOverlay1.blt(xpos + 172, typeY + (i * 30), typebitmap.bitmap, type_rect)
    end
    #---------------------------------------------------------------------------
    # Tera Types
    if PluginManager.installed?("Terastal Phenomenon")
      poke = battler if !illusion
      pbDisplayTeraType(poke, @infoUIOverlay1, xpos + 188, ypos + 98)
    end
  end
    ...
    [[:ATTACK,          _INTL("Attack")],
     ...
     _INTL("Crit. Hit")
      ...
      if stat.is_a?(Array)
        ...
        addText.push([stat[1], xpos + 18, ypos + 138 + (i * 24), 0, BASE_LIGHT, color])
        stage = battler.stages[stat[0]]
      else
        addText.push([stat, xpos + 18, ypos + 138 + (i * 24), 0, BASE_LIGHT, SHADOW_LIGHT])
        stage = [battler.effects[PBEffects::FocusEnergy] + battler.effects[PBEffects::CriticalBoost], 4].min
      end
      arrow = (stage > 0) ? 0 : 1
      stage.abs.times { |t| images.push([@path + "Battle Info/battler_stats", xpos + 106 + (t * 18), ypos + 138 + (i * 24), arrow * 18, 0, 18, 18]) }
    end
  ...
  def pbAddEffectsDisplay(xpos, ypos, panelX, battler)
    ...
    field_effects = {
      ...
      PBEffects::ClearFlute      => [_INTL("Clear Flute"),  3],  #Ashen Frost Exclusive
    ...
    effects.each_with_index do |effect, i|
      ...
      addText.push([effect[0], xpos + 320, ypos + 140 + (i * 24), 2, BASE_DARK, SHADOW_DARK],
                   [effect[1], xpos + 426, ypos + 140 + (i * 24), 2, BASE_LIGHT, SHADOW_LIGHT])
    end
    return images, addText
  end
end