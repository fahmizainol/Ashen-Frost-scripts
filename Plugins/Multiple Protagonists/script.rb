...
def pbSwitchCharacter(id, name = nil, outfit = 0)
  ...
  backupslots=$player.inf_backup_slots
  tutorlist=$Trainer.tutorlist
  tutornet=$Trainer.tutornet
  tutornet_old_user=$Trainer.tutornet_old_user
  ...
  # Fix potential outfit problem
  $player.name = name if !name.nil?
  $player.outfit = outfit if outfit != 0
  # Fix save slot transfer
  $player.save_slot = $PokemonGlobal.mainCharacters[1][0].save_slot
  $player.inf_backup_slots=backupslots
  $player.tutorlist=tutorlist
  $player.tutornet=tutornet
  $player.tutornet_old_user=tutornet_old_user
  # Unlocks national dex
  $player.pokedex.unlock(-1)  if !$player.pokedex.unlocked?(-1) 
