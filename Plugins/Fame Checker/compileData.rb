module FameChecker
  ...
  def self.compile(mustCompile = false)
    ...
    if refresh
      Compiler.compile_pbs_file_message_start("PBS/fame_targets.txt")
  ...
  def self.ensureInfoSizeData(currentFame, infoHash, name)
    ...
    infoHash[:IsAnimated] = true if not infoHash[:IsAnimated] #Ekat note: Set to true by default to save myself from having to edit the pbs
