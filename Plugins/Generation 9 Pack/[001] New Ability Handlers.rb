...
Battle::AbilityEffects::DamageCalcFromUser.add(:ORICHALCUMPULSE,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:attack_multiplier] *= 4 / 3.0 if move.physicalMove? && [:Sun, :HarshSun, :HolyInferno].include?(user.effectiveWeather)
  }
)
...
#===============================================================================
# Thermal Exchange on Holy Inferno
#===============================================================================
Battle::AbilityEffects::OnSwitchIn.add(:THERMALEXCHANGE,
  proc { |ability, battler, battle, switch_in|
    next if battle.field.weather != :HolyInferno
    battler.pbRaiseStatStageByAbility(:ATTACK, 1, battler)
  }
)

#===============================================================================
# Well-Baked Body on Holy Inferno 
#===============================================================================
Battle::AbilityEffects::OnSwitchIn.add(:WELLBAKEDBODY,
  proc { |ability, battler, battle, switch_in|
    next if battle.field.weather != :HolyInferno
...
#===============================================================================
# Flame Body on Holy Inferno
#===============================================================================
Battle::AbilityEffects::OnSwitchIn.add(:FLAMEBODY,
  proc { |ability, battler, battle, switch_in|
    next if battle.field.weather != :HolyInferno
    battle.pbShowAbilitySplash(battler)
    battle.pbDisplay(_INTL("{1}'s {2} ignited!",
      battler.pbThis(true), battler.abilityName))
    if !battler.effects[PBEffects::FlashFire]
      battler.effects[PBEffects::FlashFire] = true
      if Battle::Scene::USE_ABILITY_SPLASH
...
Battle::AbilityEffects::OnSwitchIn.add(:PROTOSYNTHESIS,
  proc { |ability, battler, battle, switch_in|
    case ability
    when :PROTOSYNTHESIS then field_check = [:Sun, :HarshSun, :HolyInferno].include?(battle.field.weather)
    when :QUARKDRIVE     then field_check = (battle.field.terrain == :Electric || battle.field.weather == :FrozenStorm)
    end
