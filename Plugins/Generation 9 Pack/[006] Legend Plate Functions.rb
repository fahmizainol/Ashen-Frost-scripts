...
class Battle
  ...
  def pbGetBestTypeJudgment(user, target, move = nil, check_type = nil)
    ...
    GameData::Type.each do |type_data|
      ...
      baseMult = [multipliers[:base_damage_multiplier].round,  1].max
  ...
  def pbCalcTypeMultsJudgment(user, target, move, type, target_types, multipliers)
    ...
    case type
    when :FIRE
      multipliers[:base_damage_multiplier] /= 3 if @field.effects[PBEffects::WaterSportField] > 0
    when :ELECTRIC
      multipliers[:base_damage_multiplier] *= 2 if user.effects[PBEffects::Charge] > 0
      multipliers[:base_damage_multiplier] /= 3 if @field.effects[PBEffects::MudSportField] > 0
    end
    if user.affectedByTerrain?
      case @field.terrain
      when :Electric then multipliers[:base_damage_multiplier] *= 1.3 if type == :ELECTRIC
      when :Grassy   then multipliers[:base_damage_multiplier] *= 1.3 if type == :GRASS
      when :Psychic  then multipliers[:base_damage_multiplier] *= 1.3 if type == :PSYCHIC
      when :Misty    then multipliers[:base_damage_multiplier] /= 2   if type == :DRAGON
      end
    end
