...
EventHandlers.add(:on_player_step_taken, :auto_save, proc {
  ...
  if $player.autosave_steps >= 1000 && !$game_system.save_disabled && #Ekat note: I hate this autosave when testing, it always screws me over by saving at the wrong time. I always remember to save, unlike some people who never save and screw themselves over. 
...
class PokemonLoadScreen
  ...
  def pbStartLoadScreen
    pbBGMStop #Ekat Note: I added this because if you exit to the title the bgm keeps playing.
    ...
    #  commands[cmd_controls = commands.length]  = _INTL('Controls') unless $joiplay
      commands[cmd_options = commands.length]   = _INTL('Options')
      commands[cmd_language = commands.length]  = _INTL('Language') if Settings::LANGUAGES.length >= 2
      commands[cmd_debug = commands.length]     = _INTL('Debug') if $DEBUG
      commands[cmd_quit = commands.length]      = _INTL('Quit Game')
      # Move left and right/show arrows
      ...
      @scene.pbStartScene(commands, show_continue, @save_data[:player],
                          @save_data[:frame_count] || 0, @save_data[:stats], @save_data[:global_metadata],
                          map_id)
...
class PokemonSaveScreen
  ...
  def getSaveInfoBoxContents(slot)
    ...
    # Character name
    name_str = _INTL("Character:<r><c3=0070F8,78B8E8>{1}</c3><br>", temp_save_data[:player].name)
    ...
    min = totalsec / 60 % 60
    if hour > 0
      elapsed_str = _INTL("Time:<r><c3=0070F8,78B8E8>{1}h {2}m</c3><br>", hour, min)
    else
      elapsed_str = _INTL("Time:<r><c3=0070F8,78B8E8>{1}m</c3><br>", min)
    end

    # Case Number 
    case_str = _INTL("Case:<r><c3=0070F8,78B8E8>{1}</c3><br>", temp_save_data[:global_metadata].case_num)
