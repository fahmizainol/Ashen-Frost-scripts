...
class EVAllocationSprite < Sprite
  ...
  def refresh
    ...
    if PluginManager.installed?("BW Summary Screen")   
      ...
      self.x=160
      self.y=56
      self.y=56+(self.index*32) if self.index>0
    end  
...
class EVAllocationSprite2 < Sprite
  ...
  def refresh
    ...
    if PluginManager.installed?("BW Summary Screen")   
      ...
      self.x=220
      self.y=58
      self.y=58+(self.index*32.floor) if self.index>0
    end  
...
if PluginManager.installed?("BW Summary Screen")
  ...
  if Settings::I_EDITED_MY_SUMMARY
    ...
    def drawPageThree
      ...
      stat_base = Color.new(255, 255, 255)
      stat_shadow = Color.new(82, 82, 82)
      # Determine which stats are boosted and lowered by the Pokémon's nature
      statshadows = {}
      GameData::Stat.each_main { |s| statshadows[s.id] = stat_shadow }
      if !@pokemon.shadowPokemon? || @pokemon.heartStage <= 3
        @pokemon.nature_for_stats.stat_changes.each do |change|
          statshadows[change[0]] = Color.new(136, 96, 72) if change[1] > 0
          statshadows[change[0]] = Color.new(64, 120, 152) if change[1] < 0
      ...
      # Write various bits of text
      textpos = [
      [_INTL("HP"), 262, baseY, 2, stat_base, statshadows[:HP]],
      [sprintf("%d/%d", @pokemon.hp, @pokemon.totalhp), baseX, baseY, 2, base, shadow],
      [sprintf("%d", @pokemon.iv[:HP]), ivX, baseY, 2, base, shadow],
      [sprintf("%d", @pokemon.ev[:HP]), evX, baseY, 2, base, shadow],
      [_INTL("ATTACK"), 232, baseY + 32, 0, stat_base, statshadows[:ATTACK]],
      [sprintf("%d", @pokemon.attack), baseX, baseY + 32, 2, base, shadow],
      [sprintf("%d", @pokemon.iv[:ATTACK]), ivX, baseY + 32, 2, base, shadow],
      [sprintf("%d", @pokemon.ev[:ATTACK]), evX, baseY + 32, 2, base, shadow],
      [_INTL("DEFENSE"), 232, baseY + 64, 0, stat_base, statshadows[:DEFENSE]],
      [sprintf("%d", @pokemon.defense), baseX, baseY + 64, 2, base, shadow],
      [sprintf("%d", @pokemon.iv[:DEFENSE]), ivX, baseY + 64, 2, base, shadow],
      [sprintf("%d", @pokemon.ev[:DEFENSE]), evX, baseY + 64, 2, base, shadow],
      [_INTL("SP. ATK"), 232, baseY + 96, 0, stat_base, statshadows[:SPECIAL_ATTACK]],
      [sprintf("%d", @pokemon.spatk), baseX, baseY + 96, 2, base, shadow],
      [sprintf("%d", @pokemon.iv[:SPECIAL_ATTACK]), ivX, baseY + 96, 2, base, shadow], 
      [sprintf("%d", @pokemon.ev[spatk]), evX, baseY + 96, 2, base, shadow], # DemICE edit
      [_INTL("SP. DEF"), 232, baseY + 128, 0, stat_base, statshadows[:SPECIAL_DEFENSE]],
      [sprintf("%d", @pokemon.spdef), baseX, baseY + 128, 2, base, shadow],
      [sprintf("%d", @pokemon.iv[:SPECIAL_DEFENSE]), ivX, baseY + 128, 2, base, shadow],
      [sprintf("%d", @pokemon.ev[:SPECIAL_DEFENSE]), evX, baseY + 128, 2, base, shadow],
      [_INTL("SPEED"), 232, baseY + 160, 0, stat_base, statshadows[:SPEED]],
      [sprintf("%d", @pokemon.speed), baseX, baseY + 160, 2, base, shadow], #, #DemICE edit
      [sprintf("%d", @pokemon.iv[:SPEED]), ivX, baseY + 160, 2, base, shadow],
      [sprintf("%d", @pokemon.ev[:SPEED]), evX, baseY + 160, 2, base, shadow]#,
      #[_INTL("Ability"), 224, 290, 0, base, shadow]  #DemICE edit
      ]    
      ...
      if $evalloc
        ...
        textpos.push([_INTL("EV ALLOC."), 250, 250, 0, Color.new(255, 255, 255), Color.new(112, 104, 96)])
        textpos.push(["EV Pool:", 226, 276, 0, Color.new(248, 248, 248), Color.new(96, 96, 96)])
        textpos.push([sprintf("%d", evpool), 326, 276, 1, Color.new(248, 248, 248), Color.new(96, 96, 96)])
        textpos.push(["[S] resets EVs",362, 276, 0, base, shadow])
        drawTextEx(overlay, 226, 306, 278, 2,"When EV is 0: [<-] to max.  \nWhen EV is max: [->] to 0.", base, shadow)
      else
        # Draw ability name and description
        textpos.push([_INTL("ABILITY"), 250, 250, 0, Color.new(255, 255, 255), Color.new(112, 104, 96)])
        ability = @pokemon.ability
        if ability
  ...
  def pbScene
    ...
    loop do
      ...
      pbUpdate
      dorefresh = false
    # Changed by DemICE 16-Dec-2023 adding tutorials to the game.
   if @page == 3 && !pbSeenTipCard?(:EVALLOC1)
    pbShowTipCardsGrouped(:EVALLOC)
   end
   if @page == 4 && !pbSeenTipCard?(:MOVEREMIND)
     pbShowTipCard(:MOVEREMIND)
   end
   # DemICE Move Remember
      if Input.trigger?(Input::ACTION)
    if @page == 4 && !@inbattle && !$donteditEVs
      pbPlayDecisionSE
