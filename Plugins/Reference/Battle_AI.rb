...
#All functions here return a modifier to the original score, similar to miniscore
	...
	def protectcode
		...
		if (@attacker.ability == :SHADOWTAG || @attacker.pbPartner.ability == :SHADOWTAG || @opponent.effects[:MeanLook])
	...
	def psychicterraincode
		...
		if Rejuv && @battle.FE != :INDOOR
			return @move.basedamage > 0 ? 1 : 0 if @battle.state.effects[:PSYTERRAIN] > 0
			miniscore = 1.0
	...
	def electricterraincode
		...
		if Rejuv && @battle.FE != :INDOOR
			return @move.basedamage > 0 ? 1 : 0 if @battle.state.effects[:ELECTERRAIN] > 0
			miniscore = 1
	...
	def grassyterraincode
		...
		if Rejuv && @battle.FE != :INDOOR
			return @move.basedamage > 0 ? 1 : 0 if @battle.state.effects[:GRASSY] > 0
			miniscore = 1
			miniscore*=1.5 if @mondata.roles.include?(:PHYSICALWALL) || @mondata.roles.include?(:SPECIALWALL)
	...
	def mistyterraincode
		...
		if Rejuv && @battle.FE != :INDOOR
			return @move.basedamage > 0 ? 1 : 0 if @battle.state.effects[:MISTY] > 0
			miniscore = 1
	...
	def perishcode
		...
		miniscore*=3 if @attacker.ability == :SHADOWTAG || @attacker.pbPartner.ability == :SHADOWTAG || @opponent.effects[:MeanLook]>0
	...
	def pbTypeModNoMessages(type=@move.type,attacker=@attacker,opponent=@opponent,move=@move,skill=@mondata.skill)
		...
		if !moldBreakerCheck(attacker)
			case opponent.ability
				...
				when :LEVITATE,:SOLARIDOL,:LUNARIDOL 	then return 0 if (type == :GROUND || secondtype==:GROUND) && @battle.FE != :CAVE && !opponent.hasWorkingItem(:IRONBALL) && @battle.state.effects[:Gravity]==0
				...
				when :DRYSKIN 							then return -1 if type == :WATER || secondtype==:WATER && opponent.effects[:HealBlock]==0
				when :VOLTABSORB 						then return -1 if type == :ELECTRIC || secondtype==:ELECTRIC && opponent.effects[:HealBlock]==0
				when :WATERABSORB 						then return -1 if type == :WATER || secondtype==:WATER && opponent.effects[:HealBlock]==0
		...
		if Rejuv && @battle.FE == :DESERT && @battle.pbWeather == :SUNNYDAY && (type == :WATER || secondtype==:WATER) && (opponent.hasType?(:WATER) || opponent.hasType?(:GRASS)) && opponent.effects[:HealBlock]==0
			return -1
		end
...
######################################################
	def pbRoughDamage(move=@move,attacker=@attacker,opponent=@opponent)
		...
		# Shelter
		if @mondata.skill>=MEDIUMSKILL
			if opponent.effects[:Shelter] && @battle.FE != :INDOOR && (type == @battle.field.mimicry || move.getSecondaryType(attacker) == @battle.field.mimicry)
				damage=(damage*0.5).round
			end
		end
		# Screens
