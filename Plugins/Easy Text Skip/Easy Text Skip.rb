...
# Added because Josie likes this binding
TEXT_SKIP_BUTTON = ($joiplay || $DEBUG && pbGetUserName == "Josep")  ? Input::BACK : Input::F
# Possible Inputs:
...
# This overwrites pbMessageDisplay, so if you have any changes in that method you could change them here instead
#-------------------------------------------------------------------------------
...
def pbMessageDisplay(msgwindow, message, letterbyletter = true, commandProc = nil)
  ...
  textchunks = []
  controls = []
  while text[/(?:\\([Xn][Nn]|[Dd][Xx][Nn]|[Xn][Nn][Aa]|[Xn][Nn][Bb]|[Xn][Nn][Cc]|f|ff|ts|cl|me|se|wt|wtnp|ch)\[([^\]]*)\]|\\(g|cn|pt|dp|wd|wm|op|cl|wu|\.|\||\!|\^))/i]
  ...
  # Collect text for text log
  begin
    text = text.gsub(colortag, DEFAULTCOLOR)
    text = changeSkinColor(text)
    if namewindow != nil # Mr. Gela
      name = namewindow.text
    #  name_base = namewindow.colorBase
    #  name_shadow = namewindow.colorShadow
      if name[/(<c3=.*>)(<ac>)(.*)(<\/ac>)/]
        name = "<c3=FFFFFF,4A4A4A>" + $3 + ": \n"
      else
        name = name + ": \n"
      end
      str = name + text
  ...
  if $scene.is_a?(Scene_Map) && !$game_temp.in_battle && !$game_temp.in_menu
