...
ItemHandlers::UseOnPokemon.copy(:MAXPOTION, :SILVERSYRINGE)
...
ItemHandlers::UseOnPokemon.copy(:MOOMOOMILK, :CHEESEBURGER)

ItemHandlers::UseOnPokemon.add(:BURGER, proc { |item, qty, pkmn, scene|
  next pbHPItem(pkmn, 45, scene)
})

ItemHandlers::UseOnPokemon.add(:HOTCHOCOLATE, proc { |item, qty, pkmn, scene|
  next pbHPItem(pkmn, 75, scene)
})

ItemHandlers::UseOnPokemon.add(:SARSAPARILLA, proc { |item, qty, pkmn, scene|
  next pbHPItem(pkmn, 175, scene)
})

ItemHandlers::UseOnPokemon.add(:REDSYRINGE, proc { |item, qty, pkmn, scene|
...
ItemHandlers::UseOnPokemon.copy(:BURNHEAL, :RAWSTBERRY, :MILKSHAKE)
...
ItemHandlers::UseOnPokemon.copy(:FULLRESTORE, :GOLDSYRINGE)

ItemHandlers::UseOnPokemon.add(:GREENSYRINGE, proc { |item, qty, pkmn, scene|
  if pkmn.fainted? || (pkmn.hp == pkmn.totalhp && pkmn.status == :NONE)
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
  hpgain = pbItemRestoreHP(pkmn, (pkmn.totalhp / 2).floor)
  pkmn.heal_status
  scene.pbRefresh
  if hpgain > 0
    scene.pbDisplay(_INTL("{1}'s HP was restored by {2} points.", pkmn.name, hpgain))
...
ItemHandlers::UseOnPokemon.add(:COFFEE, proc { |item, qty, pkmn, scene|
  if pkmn.fainted? || (pkmn.hp == pkmn.totalhp && pkmn.status == :NONE)
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
  hpgain = pbItemRestoreHP(pkmn, (pkmn.totalhp / 4).floor)
  pkmn.heal_status
  scene.pbRefresh
  if hpgain > 0
    scene.pbDisplay(_INTL("{1}'s HP was restored by {2} points.", pkmn.name, hpgain))
...
ItemHandlers::UseOnPokemon.copy(:MAXREVIVE, :MAXHONEY, :APPLECIDER)

ItemHandlers::UseOnPokemon.add(:FIRSTAIDKIT, proc { |item, qty, pkmn, scene|
  if !pkmn.fainted?
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
...
ItemHandlers::UseOnPokemon.add(:MEDICALBAG, proc { |item, qty, pkmn, scene|
  if !pkmn.fainted?
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
  pkmn.hp = (pkmn.totalhp / 4 * 3).floor
  pkmn.hp = 1 if pkmn.hp <= 0
  pkmn.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1}'s HP was restored.", pkmn.name))
...
ItemHandlers::UseOnPokemon.copy(:HEALPOWDER, :SKETCHYFRIES)
...
ItemHandlers::UseOnPokemon.add(:RAMEN, proc { |item, qty, pkmn, scene|
  if pbHPItem(pkmn, 30, scene)
...
ItemHandlers::UseOnPokemon.add(:SKETCHYRAMEN, proc { |item, qty, pkmn, scene|
  if pbHPItem(pkmn, 30, scene)
    pkmn.changeHappiness("energyroot")
    next true
  end
...
ItemHandlers::UseOnPokemon.add(:IMPUREWATER, proc { |item, qty, pkmn, scene|
...
# DemICE Changed vitamins/feathers
ItemHandlers::UseOnPokemonMaximum.add(:HPUP, proc { |item, pkmn|
...
ItemHandlers::UseOnPokemon.add(:RARECANDY, proc { |item, qty, pkmn, scene|
  if pkmn.shadowPokemon? || ($PokemonGlobal.levelcap == 1 && pkmn.level >= $player.level_cap)
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
...
ItemHandlers::UseOnPokemon.add(:ABILITYCAPSULE, proc{ |item, qty, pkmn, scene|
  ...
  next false if abil_cmd < 0 || pkmn.ability_index == abils[abil_cmd][1]
...
# Ashen Frost Reverse Candy
ItemHandlers::UseOnPokemon.add(:REVERSECANDY, proc { |item, qty, pkmn, scene|
  if pkmn.shadowPokemon? || pkmn.level <= 1
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
  # Level down
...
# Ashen Frost Regional Evolutions
ItemHandlers::UseOnPokemon.add(:STRANGESOUVENIR, proc { |item, qty, pkmn, scene|
  # Alolan split evos
  if [:PIKACHU, :CUBONE, :EXEGGCUTE].include?(pkmn.species) && pkmn.form == 0
    pkmn.setForm(1)
  end
  if pkmn.shadowPokemon?
    scene.pbDisplay(_INTL("It won't have any effect."))
...
ItemHandlers::UseOnPokemon.add(:WISHINGSTAR, proc { |item, qty, pkmn, scene|
  # Galarian split evos
  if [:KOFFING, :MIMEJR].include?(pkmn.species) && pkmn.form == 0
    pkmn.setForm(1)
  end
  if pkmn.shadowPokemon?
    scene.pbDisplay(_INTL("It won't have any effect."))
...
ItemHandlers::UseOnPokemon.add(:JUBILIFEMUFFIN, proc { |item, qty, pkmn, scene|
  # Hisiuan split evos
  if [:QUILAVA, :DEWOTT, :DARTRIX, :PETILIL, :RUFFLET, :GOOMY, :BERGMITE].include?(pkmn.species) && pkmn.form == 0
    pkmn.setForm(1)
  end
  if pkmn.shadowPokemon?
    scene.pbDisplay(_INTL("It won't have any effect."))
