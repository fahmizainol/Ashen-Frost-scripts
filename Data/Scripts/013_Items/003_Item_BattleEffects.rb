...
ItemHandlers::CanUseInBattle.copy(:POTION,
   ...
   :ENERGYROOT, :BURGER, :HOTCHOCOLATE, :CHEESEBURGER, :SARSAPARILLA, :REDSYRINGE, 
   :SILVERSYRINGE, :RAMEN)
...
ItemHandlers::CanUseInBattle.copy(:BURNHEAL, :RAWSTBERRY, :MILKSHAKE)
...
ItemHandlers::CanUseInBattle.copy(:FULLHEAL,
   ...
   :BIGMALASADA, :PEWTERCRUNCHIES, :LUMBERRY, :HEALPOWDER, :FRIES, :SKETCHYFRIES)
...
ItemHandlers::CanUseInBattle.copy(:FULLRESTORE, :GREENSYRINGE, :GOLDSYRINGE, :COFFEE)
...
ItemHandlers::CanUseInBattle.copy(:REVIVE, :MAXREVIVE, :REVIVALHERB, :MAXHONEY, :APPLECIDER, :FIRSTAIDKIT, :MEDICALBAG)
...
ItemHandlers::CanUseInBattle.add(:CLEARFLUTE, proc { |item, pokemon, battler, move, firstAction, battle, scene, showMessages|
  if [:None, :HarshSun, :HeavyRain, :StrongWinds, :FrozenStorm, :HolyInferno].include?(battle.field.weather) || battle.field.effects[PBEffects::ClearFlute] != 0 
    scene.pbDisplay(_INTL("It won't have any effect.")) if showMessages
    next false
  end
  next true
...
ItemHandlers::BattleUseOnPokemon.copy(:MAXPOTION, :SILVERSYRINGE)
...
ItemHandlers::BattleUseOnPokemon.copy(:MOOMOOMILK, :CHEESEBURGER)

ItemHandlers::BattleUseOnPokemon.add(:BURGER, proc { |item, pokemon, battler, choices, scene|
  pbBattleHPItem(pokemon, battler, 45, scene)
})

ItemHandlers::BattleUseOnPokemon.add(:HOTCHOCOLATE, proc { |item, pokemon, battler, choices, scene|
  pbBattleHPItem(pokemon, battler, 75, scene)
})

ItemHandlers::BattleUseOnPokemon.add(:SARSAPARILLA, proc { |item, pokemon, battler, choices, scene|
  pbBattleHPItem(pokemon, battler, 175, scene)
})

ItemHandlers::BattleUseOnPokemon.add(:REDSYRINGE, proc { |item, pokemon, battler, choices, scene|
...
ItemHandlers::BattleUseOnPokemon.copy(:BURNHEAL, :RAWSTBERRY, :MILKSHAKE)
...
ItemHandlers::BattleUseOnPokemon.copy(:FULLHEAL,
   :LAVACOOKIE, :OLDGATEAU, :CASTELIACONE, :LUMIOSEGALETTE, :SHALOURSABLE,
   :BIGMALASADA, :PEWTERCRUNCHIES, :LUMBERRY, :FRIES, :BLUESYRINGE)
...
ItemHandlers::BattleUseOnPokemon.copy(:FULLRESTORE, :GOLDSYRINGE)

ItemHandlers::BattleUseOnPokemon.add(:GREENSYRINGE, proc { |item, pokemon, battler, choices, scene|
  pokemon.heal_status
  battler&.pbCureStatus(false)
  battler&.pbCureConfusion
  name = (battler) ? battler.pbThis : pokemon.name
...
ItemHandlers::BattleUseOnPokemon.copy(:MAXREVIVE, :MAXHONEY, :APPLECIDER)

ItemHandlers::BattleUseOnPokemon.add(:FIRSTAIDKIT, proc { |item, pokemon, battler, choices, scene|
...
ItemHandlers::BattleUseOnPokemon.add(:MEDICALBAG, proc { |item, pokemon, battler, choices, scene|
  pokemon.hp = pokemon.totalhp / 4 * 3
  pokemon.hp = 1 if pokemon.hp <= 0
  pokemon.heal_status
  scene.pbRefresh
  scene.pbDisplay(_INTL("{1} recovered from fainting!", pokemon.name))
...
ItemHandlers::BattleUseOnPokemon.copy(:HEALPOWDER, :SKETCHYFRIES)
...
ItemHandlers::BattleUseOnPokemon.add(:RAMEN, proc { |item, pokemon, battler, choices, scene|
  if pbBattleHPItem(pokemon, battler, 30, scene)
    pokemon.changeHappiness("groom")
  end
})

ItemHandlers::BattleUseOnPokemon.add(:SKETCHYRAMEN, proc { |item, pokemon, battler, choices, scene|
  if pbBattleHPItem(pokemon, battler, 30, scene)
    pokemon.changeHappiness("energyroot")
  end
})

ItemHandlers::BattleUseOnPokemon.add(:IMPUREWATER, proc { |item, pokemon, battler, choices, scene|
...
#Ashen Frost Exclusive
ItemHandlers::BattleUseOnBattler.add(:CLEARFLUTE, proc { |item, battler, scene, battle|
  battle.field.effects[PBEffects::ClearFlute] = 3
  scene.pbDisplay(_INTL("Weather has been temporarily cleared.", battler.pbThis))
})
