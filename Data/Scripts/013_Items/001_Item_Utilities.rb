...
module ItemHandlers
  ...
  # Returns whether item was used
  def self.triggerBattleUseOnBattler(item, battler, scene, battle=nil)
    return false if !BattleUseOnBattler[item]
    return BattleUseOnBattler.trigger(item, battler, scene, battle)
  end
...
def pbChangeExp(pkmn, new_exp, scene)
  ...
  if pkmn.exp > new_exp   # Loses Exp
    ...
    movelist.each do |i|
      next if i[0] <= old_level || i[0] > pkmn.level
      pbLearnMove(pkmn, i[1], true) { scene.pbUpdate }
      pkmn.add_first_move(i[1])  # DemICE Move Remember
    end
    # Check for evolution
    new_species = pkmn.check_evolution_on_level_up
    if new_species
...
def pbGainExpFromExpCandy(pkmn, base_amt, qty, scene)
  if pkmn.level >= GameData::GrowthRate.max_level || pkmn.shadowPokemon? || pkmn.level >= $player.level_cap
  ...
  exp = pkmn.exp + (base_amt * qty)
  # Add adjustment for hard cap
  level_cap_gap = pkmn.growth_rate.exp_values[$player.level_cap] - pkmn.exp
  if pkmn.level < $player.level_cap && (base_amt * qty) >= level_cap_gap && $PokemonGlobal.levelcap == 1
    exp = pkmn.exp + level_cap_gap 
  end
  pbChangeExp(pkmn, exp, scene)
  scene.pbHardRefresh
  return true
end
...
#===============================================================================
def pbLearnMove(pkmn, move, ignore_if_known = false, by_machine = false, rotom = false, &block)
  ...
  if pkmn.species == :ROTOM
    rotom = true if  [:OVERHEAT,:HYDROPUMP,:LEAFSTORM,:BLIZZARD,:AIRSLASH].include?(move)
  end
  pkmn.add_first_move(move) if !rotom # DemICE Move Remember
  ...
  if pbConfirmMessage(_INTL("Should {1} forget a move to learn {2}?", pkmn_name, move_name), &block)
    loop do
      move_index = pbForgetMove(pkmn, move, rotom)
      if move_index >= 0
...
def pbForgetMove(pkmn, moveToLearn, rotom = false)
  ret = -1
  pbFadeOutIn {
    scene = PokemonSummary_Scene.new
    screen = PokemonSummaryScreen.new(scene)
    ret = screen.pbStartForgetScreen([pkmn], 0, moveToLearn, rotom)
  }
  return ret
end
