  ...
  def update_command_new
    ...
    unless pbMapInterpreterRunning? || $game_temp.message_window_showing ||
      ...
      if @moved_last_frame ||
        ...
        # Cordelia check
        if $game_variables[270] != 0
          event = $game_map.events[$game_variables[270]]
          case dir
          when 2 then event.move_down
          when 4 then event.move_right
          when 6 then event.move_left
...
def pbGetPlayerCharset(charset, trainer = nil, force = false)
  ...
  # Check for Sylvester bike - Piloswine or Mamoswine
  if ret == "Syl_bike_4"
    truffles = trainer.has_species?(:MAMOSWINE) ? "Mamoswine" : "Piloswine"
    ret = ret + "_" + truffles
  end
  return ret
end
