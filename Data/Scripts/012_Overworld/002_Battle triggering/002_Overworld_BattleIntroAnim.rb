...
#===============================================================================
# Play the "LonardoGangster" battle transition animation for any trainer battle
# involving a Lonardo Gangster. Is lower priority than the Vs. animation above.
#===============================================================================
SpecialBattleIntroAnimations.register("lonardo_gangster_animation", 40,   # Priority 40
  proc { |battle_type, foe, location|   # Condition
    next false unless [1, 3].include?(battle_type)   # Only if a trainer battle
    trainer_types = [:GANGSTER_1, :GANGSTER_2, :GANGSTER_3, :GANGSTER_4, 
                     :MAFIOSO_1, :MAFIOSO_2, :MAFIOSO_3, :MAFIOSO_4, 
                     :LUE, :FRASER]
    next foe.any? { |f| trainer_types.include?(f.trainer_type) }
  },
  proc { |viewport, battle_type, foe, location|   # Animation
...
#===============================================================================
# Play the "Serenity" battle transition animation for any trainer battle
# involving a Serenity member. Is lower priority than the Vs. animation above.
#===============================================================================
SpecialBattleIntroAnimations.register("serenity_animation", 40,   # Priority 40
  proc { |battle_type, foe, location|   # Condition
    next false unless [1, 3].include?(battle_type)   # Only if a trainer battle
    trainer_types = [:SERENITY_1, :SERENITY_2, :SERENITY_3, :SERENITY_Argon,
                     :SERENITY_Astatine, :SERENITY_Enforcer, :SERENITY_Radon, 
                     :SERENITY_Trainer]
    next foe.any? { |f| trainer_types.include?(f.trainer_type) }
  },
  proc { |viewport, battle_type, foe, location|   # Animation
...
#===============================================================================
# Play the "Ghost" battle transition animation for any trainer battle
# involving a Ghost. Is lower priority than the Vs. animation above.
#===============================================================================
SpecialBattleIntroAnimations.register("ghost_animation", 40,   # Priority 40
  proc { |battle_type, foe, location|   # Condition
    next false unless [1, 3].include?(battle_type)   # Only if a trainer battle
    trainer_types = [:GHOST_1, :GHOST_2, :GHOST_3, :GHOST_4,
                     :GHOST_Ivy, :GHOST_Silva, :GHOST_Syl_II,
                     :FRANCIS]
    next foe.any? { |f| trainer_types.include?(f.trainer_type) }
  },
  proc { |viewport, battle_type, foe, location|   # Animation
...
#===============================================================================
# Play the "Lafayette" battle transition animation for any trainer battle
# involving a Lafayette. Is lower priority than the Vs. animation above.
# Sean is not included in this list because she needs a black and white screen.
#===============================================================================
SpecialBattleIntroAnimations.register("lafayette_animation", 40,   # Priority 40
  proc { |battle_type, foe, location|   # Condition
    next false unless [1, 3].include?(battle_type)   # Only if a trainer battle
    trainer_types = [:DITTO_Sylvester, :ELOISE, :GANGSTER_Lafayette,
                     :MAISY, :LUMBERJACK_1, :LUMBERJACK_2,
                     :SAWYER, :SYLVESTER_II_1, :SYLVESTER_II_2,
                     :SEAN]
    next foe.any? { |f| trainer_types.include?(f.trainer_type) }
  },
  proc { |viewport, battle_type, foe, location|   # Animation
...
#===============================================================================
# Play the "Flores" battle transition animation for any trainer battle
# involving a Flores. Is lower priority than the Vs. animation above.
#===============================================================================
SpecialBattleIntroAnimations.register("flores_animation", 40,   # Priority 40
  proc { |battle_type, foe, location|   # Condition
    next false unless [1, 3].include?(battle_type)   # Only if a trainer battle
    trainer_types = [:GANGSTER_Flores_1, :GANGSTER_Flores_2, :GANGSTER_Flores_3,
                     :MONTAGUE, :ARTURO]
    next foe.any? { |f| trainer_types.include?(f.trainer_type) }
  },
  proc { |viewport, battle_type, foe, location|   # Animation
...
#===============================================================================
# Play the "Renegade (Past)" battle transition animation for any trainer battle
# involving a high-ranking Lonardo in the past. Is lower priority than the Vs. 
# animation above.
#===============================================================================
SpecialBattleIntroAnimations.register("renegade_past_animation", 50,   # Priority 50
  proc { |battle_type, foe, location|   # Condition
    next false unless [1, 3].include?(battle_type)   # Only if a trainer battle
    next false unless $PokemonGlobal.case_num == 20 && [5, 7].include?($player.character_ID)
    trainer_types = [:LUSTROUS,:ADAMANT, :LEO_1]
    next foe.any? { |f| trainer_types.include?(f.trainer_type) }
  },
  proc { |viewport, battle_type, foe, location|   # Animation"
    pbCommonEvent(182)
...
# Sean is not included in this list because she needs a black and white screen.
#===============================================================================
SpecialBattleIntroAnimations.register("lafayette_past_animation", 50,   # Priority 40
  proc { |battle_type, foe, location|   # Condition
    next false unless [1, 3].include?(battle_type)   # Only if a trainer battle
    trainer_types = [:SEAN]
    next foe.any? { |f| trainer_types.include?(f.trainer_type) }
...
#===============================================================================
# Play the "LucileRenegade" battle transition animation for any trainer battle
# involving Lucile/a Renegade. Is lower priority than the Vs. animation above.
#===============================================================================
SpecialBattleIntroAnimations.register("lucile_renegade_animation", 50,   # Priority 50
  proc { |battle_type, foe, location|   # Condition
    next false unless [1, 3].include?(battle_type)   # Only if a trainer battle
    trainer_types = [:LUCILE_1, :LUCILE_2, :LUCILE_3, :DOMINIC, :LUSTROUS, :ADAMANT,
                     :BLADE, :SCYTHE, :BLADE_SCYTHE, :LEO_1]
    next foe.any? { |f| trainer_types.include?(f.trainer_type) }
  },
  proc { |viewport, battle_type, foe, location|   # Animation
    pbCommonEvent(173)
