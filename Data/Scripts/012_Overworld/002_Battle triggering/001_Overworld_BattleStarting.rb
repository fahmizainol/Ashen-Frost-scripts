...
class Game_Temp
  ...
  def add_battle_rule(rule, var = nil)
    ...
    when "noforfeit"              then rules["noforfeit"]           = true
    when "alwayslosemoney"        then rules["alwayslosemoney"]     = true
    ...
    when "canscale"               then rules["canScale"]            = true
  ...
  args.each do |arg|
    if r
      ...
      when "terrain", "weather", "environment", "environ", "backdrop",
           "battleback", "base", "outcome", "outcomevar", "foreground"
        r = arg
        next
      end
...
module BattleCreationHelperMethods
  ...
  def skip_battle?
    ...
    return true if $game_switches[408]
    return false
  end

  def skip_battle(outcome_variable, trainer_battle = false)
    pbMessage(_INTL("SKIPPING BATTLE...")) if !trainer_battle && $player.pokemon_count > 0 && !$game_switches[408]
  ...
  def prepare_battle(battle)
    ...
    # Whether the player will lose for real even if they forfeit (default: false)
    battle.noforfeit = battleRules["noforfeit"] if !battleRules["noforfeit"].nil?
    # Whether the player will lose money even if they forfeit (default: false)
    battle.alwayslosemoney = battleRules["alwayslosemoney"] if !battleRules["alwayslosemoney"].nil?
    ...
    battle.switchStyle = ($PokemonSystem.battlestyle == 0 && $PokemonGlobal.difficulty != 2)
    ...
    # Whether trainer's Pokemon scale to your level
    battle.canScale  = battleRules["canScale"] if !battleRules["canScale"].nil?
    # Terrain
    ...
    if battleRules["defaultWeather"].nil?
      ...
      when :FreezingRain
        battle.defaultWeather = :FreezingRain
      when :FrozenStorm
        battle.defaultWeather = :FrozenStorm
      when :HolyInferno
        battle.defaultWeather = :HolyInferno
      when :SolarWinds
        battle.defaultWeather = :SolarWinds
      end
    else
  ...
  def after_battle(outcome, can_lose)
    ...
    if $PokemonGlobal.partner
      $player.heal_party if !$game_switches[149]
    ...
    if [2, 5, 6].include?(outcome) && can_lose   # if loss or draw
    ...
    if !$game_temp.waiting_trainer && pbMapInterpreterRunning? && pbCanDoubleBattle?
      ...
      if otherEvent.length == 1 || otherEvent.length == 2
        ...
        # to battle alongside the other trainer
          ...
          @participants.push([trainers[0], thisEvent.id])
          return false
        #end
      end
    end
    ...
    if $game_temp.waiting_trainer
      if @trainercount > 0
        ...
        for i in 0..@participants.length-1
          new_args.push(@participants[i][0])
        end
  ...
  proc { |decision, canLose|
    ...
    when 1, 4   # Win, capture
      ...
      $player.pokemon_party.each do |pkmn|
        #alive=true if !pkmn.fainted? && !alive
        pbPickup(pkmn)
        pbHoneyGather(pkmn)
      end
      # Dem trying to make you blackout if you run out of pokemon but your partner wins
      # if !alive && !canLose
      #   $game_system.bgm_unpause
      #   $game_system.bgs_unpause
      #   pbStartOver
      # end
    when 2, 5, 6   # Lose, draw, forfeit
      if !canLose
        $game_system.bgm_unpause
        $game_system.bgs_unpause
        pbStartOver
...
PICKUP_COMMON_ITEMS = [
  ...
  :SUPERREPEL,    # Levels 1-10, 11-20, 21-30, 31-40, 41-50, 51-60
