...
EventHandlers.add(:on_frame_update, :pokerus_counter,
  proc {
    ...
    # now = pbGetTimeNow
      ...
      $player.pokemon_party.each { |pkmn| pkmn.curePokerus }
...
=begin
# Added to address chance of dying with partner who can't heal
EventHandlers.add(:on_frame_update, :faint_with_partner,
  proc { |handled|
    # handled is an array: [nil]. If [true], a transfer has happened because of
    # this event, so don't do anything that might cause another one
    next if handled[0]
    next if !($PokemonGlobal.partner && $game_switches[149])
    if $player.able_pokemon_count == 0
      handled[0] = true
      pbCheckAllFainted
    end
...
EventHandlers.add(:on_enter_map, :setup_new_map,
  proc { |old_map_id|   # previous map ID, is 0 if no map ID
    ...
    power = new_weather[0] == :Drip ? 3 : 9
    $game_screen.weather(new_weather[0], power, 0) if rand(100) < new_weather[1]
  }
)
...
def pbSetPokemonCenter(mapid=$game_map.map_id,x=$game_player.x,y=$game_player.y,dir= $game_player.direction)
  $PokemonGlobal.pokecenterMapId     = mapid#$game_map.map_id
  $PokemonGlobal.pokecenterX         = x#$game_player.x
  $PokemonGlobal.pokecenterY         = y#$game_player.y
  $PokemonGlobal.pokecenterDirection = dir#$game_player.direction
end
...
def pbItemBall(item, quantity = 1)
  ...
  if $bag.add(item, quantity)   # If item can be picked up
    ...
    if item.is_machine?
      meName = "TM get"
    elsif item.is_key_item?
      meName = "sfx-realization"
    elsif item.is_plate?
      meName = "Azure Flute Short"
    end
    if item == :LEFTOVERS
    ...
    pbItemPrinterAdd(itemid) if item.is_evolution_stone? # DemICE item printer
    return true
  end
  # Can't add the item
  if item == :LEFTOVERS
...
def pbReceiveItem(item, quantity = 1)
  ...
  if item == :LEFTOVERS
    ...
    pbMessage(_INTL("\\me[{1}]You obtained the \\c[1]{2}\\c[0]!\\wtnp[30]", meName, itemname))
  else
    pbMessage(_INTL("\\me[{1}]You obtained the \\c[1]{2}\\c[0]!\\wtnp[30]", meName, itemname))
  end
