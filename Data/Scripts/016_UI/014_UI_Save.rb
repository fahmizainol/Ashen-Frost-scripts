def pbEmergencySave
  ...
  if Game.save
    pbMessage(_INTL("\\se[]The game was saved.\\me[Typing_Bell] The previous save file has been backed up.\\wtnp[30]"))
  else
...
class PokemonSaveScreen
  ...
  def pbSaveScreen
    ...
    if pbConfirmMessage(_INTL("Would you like to save the game?"))
      ...
      pbSEPlay("GUI save choice")
      if Game.save
        pbMessage(_INTL("\\se[]{1} saved the game.\\me[Typing_Bell]\\wtnp[30]", $player.name))
        ret = true
      else
        pbMessage(_INTL("\\se[]Save failed.\\wtnp[30]"))
        ret = false
...
def pbSaveScreen
  ...
  ret = $joiplay ? screen.pbSaveScreenJoiplay : screen.pbSaveScreen
  return ret
end
