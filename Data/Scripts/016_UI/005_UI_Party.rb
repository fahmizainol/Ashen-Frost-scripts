...
class PokemonPartyPanel < Sprite
  ...
  def draw_name
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[@pokemon.name, 88, 22, 0, TEXT_BASE_COLOR, TEXT_SHADOW_COLOR]])
  end
  ...
  def draw_gender
    ...
    pbDrawTextPositions(@overlaysprite.bitmap,
                        [[gender_text, 232, 22, 0, base_color, shadow_color]])
  end
  ...
  def draw_shiny_icon
    ...
    pbDrawImagePositions(@overlaysprite.bitmap,
                         [["Graphics/Pictures/shiny", 72, 48, 0, 0, 18, 16]])
  end
...
def pbChooseNonEggPokemon(variableNumber, nameVarNumber)
  pbChoosePokemon(variableNumber, nameVarNumber, proc { |pkmn| !pkmn.egg? && !(pkmn.species == :SANDSLASH && pkmn.form == 0) && ![:SWINUB, :PILOSWINE, :MAMOSWINE, :ZAPDOS].include?(pkmn.species) })
end
