...
class PokemonMartAdapter
  ...
  def showQuantity?(item)
    return !GameData::Item.get(item).is_important? && !GameData::Item.get(item).is_powerful?
  end
  ...
  def canSell?(item)
    return getPrice(item, true) > 0 && !GameData::Item.get(item).is_important? && !GameData::Item.get(item).is_powerful?
  end
...
class PokemonMartScreen
  ...
  def pbBuyScreen(place="")
    ...
    loop do
      ...
      if GameData::Item.get(item).is_important? || GameData::Item.get(item).is_powerful?
      ...
      if added == quantity
        ...
        pbDisplayPaused(_INTL("Here you are! Thank you!")) { pbSEPlay("Slots coin") }
        pbItemPrinterAdd(item) if GameData::Item.get(item).is_evolution_stone? # DemICE item printer
        case place
        when "grotto"
            if item == :PWCONVERTER
              $game_self_switches[[593, 4, "B"]] = true 
              @stock.delete_if { |item| item == :PWCONVERTER}
            end
            if item == :ENCONVERTER
              $game_self_switches[[593, 4, "C"]] = true 
              @stock.delete_if { |item| item == :ENCONVERTER}
            end
            $map_factory.getMap(593, false).need_refresh = true
          when "easte"
              if item == :FLAMEORB
                $game_self_switches[[375, 43, "A"]] = true 
                @stock.delete_if { |item| item == :FLAMEORB}
              end
              $map_factory.getMap(375, false).need_refresh = true
        end
    ...
    loop do
      ...
      if pbConfirm(_INTL("I can pay ${1}.\nWould that be OK?", price.to_s_formatted))
        ...
        pbDisplayPaused(_INTL("You turned over the {1} and got ${2}.",
                              sold_item_name, price.to_s_formatted)) { pbSEPlay("Slots coin") }
        @scene.pbRefresh
      end
      @scene.pbHideMoney
    end
...
#
#===============================================================================
def pbPokemonMart(stock, speech = [], cantsell = false)
  stock.delete_if { |item| GameData::Item.get(item).is_important? && $bag.has?(item) }
  commands = []
  cmdBuy  = -1
  cmdSell = -1
  ...
  cmd = pbMessage(speech[0] || _INTL("Welcome! How may I help you?"), commands, cmdQuit + 1)
  loop do
    if cmdBuy >= 0 && cmd == cmdBuy
      ...
      pbMessage(speech[2] || _INTL("Stay safe!"))
      break
    end
    cmd = pbMessage(speech[1] || _INTL("Is there anything else I can do for you?"), commands, cmdQuit + 1)
  end
  $game_temp.clear_mart_prices
end

#===============================================================================
# Event specific pokemarts.
#===============================================================================
def pbSpecialPokemonMart(place, stock, speech = [], cantsell = false)
  stock.delete_if { |item| GameData::Item.get(item).is_important? && $bag.has?(item) }
  commands = []
  cmdBuy  = -1
  cmdSell = -1
  ...
  loop do
    if cmdBuy >= 0 && cmd == cmdBuy
      scene = PokemonMart_Scene.new
      screen = PokemonMartScreen.new(scene, stock)
      screen.pbBuyScreen(place)
    elsif cmdSell >= 0 && cmd == cmdSell
      scene = PokemonMart_Scene.new
      screen = PokemonMartScreen.new(scene, stock)
      screen.pbSellScreen
    else
      pbMessage(speech[2] || _INTL("Do come again!"))
      break
    end
    cmd = pbMessage(speech[1] || _INTL("Is there anything else I can do for you?"), commands, cmdQuit + 1)
  end
