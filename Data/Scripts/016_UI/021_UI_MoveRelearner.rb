...
class MoveRelearner_Scene
  ...
  def pbDrawMoveList
    ...
    textpos = [
      [_INTL("Teach which move?"), 16, 6, 0, Color.new(64, 64, 64), Color.new(176, 176, 176)]
    ]
    imagepos = []
    yPos = 88
...
class MoveRelearnerScreen
  ...
  def pbGetRelearnableMoves(pkmn)
    ...
    pkmn.getMoveList.each do |m|
      ...
      next if m[0] != 1 || pkmn.hasMove?(m[1]) || pkmn.first_moves.include?(m[1])
      ...
      if move
        if @scene.pbConfirm(_INTL("Teach {1}?", GameData::Move.get(move).name))
          if pbLearnMove(pkmn, move)
            pkmn.add_first_move(move) # DemICE Move Remember
