...
class PokemonSummary_Scene
  ...
  def drawPage(page)
    ...
    if status >= 0
      imagepos.push(["Graphics/Pictures/statuses", 90, 348, 0, 16 * status, 44, 16])
    end
  ...
  def drawPageOne
    ...
    if @pokemon.owner.name.empty?
      ...
      textpos.push(["?????", 467, 68, 2, base, shadow])
    else
  ...
  def drawPageOneEgg
    ...
    if mapname && mapname != ""
      memo += _INTL("<c3=000000,D0D0C8>A mysterious Pokémon Egg received from <c3=E00808,F8B870>{1}<c3=000000,D0D0C8>.\n", mapname)
    else
  ...
  def drawMemo
    ...
    mettext = [_INTL("Met at Lv. {1}.", @pokemon.obtain_level),
               ...
               "",
               _INTL("Fateful encounter at Lv. {1}.", @pokemon.obtain_level)][@pokemon.obtain_method]
  ...
  def drawPageTwo
    ...
    # Write happiness
    textpos.push([sprintf("%d", @pokemon.happiness), 442, 68, 2, base, shadow])
  ...
  def drawPageFive
    ...
    # Write various bits of text
    textpos = [
      [@pokemon.numRibbons.to_s, 296, 318, 1, Color.new(72, 72, 72), Color.new(214, 214, 206)]
    ]
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
    # Show all ribbons
  ...
  def drawSelectedMove(move_to_learn, selected_move)
    ...
    imagepos = [["Graphics/Pictures/category", 136, 340, 0, selected_move.display_category(@pokemon) * 28, 64, 28]]
    pbDrawImagePositions(overlay, imagepos)
  ...
  def pbMoveSelection
    ...
    loop do
      ...
      elsif Input.trigger?(Input::ACTION) && !@inbattle && !$donteditEVs
            pbPlayDecisionSE
            @sprites["movepresel"].visible = false
            switching = false
    ...
    loop do
      ...
      if redraw
        ...
        textpos = [
          ...
          [_INTL("OK"), 366, 254, 2, Color.new(72, 72, 72), Color.new(214, 214, 206)],
          [_INTL("Cancel"), 366, 304, 2, Color.new(72, 72, 72), Color.new(214, 214, 206)]
        ]
...
class PokemonSummaryScreen
  ...
  def pbStartForgetScreen(party, partyindex, move_to_learn, rotom = false)
    ret = -1
    ...
    @scene.pbEndScene
    if party[partyindex].species == :ROTOM
      rotom = true if  [:OVERHEAT,:HYDROPUMP,:LEAFSTORM,:BLIZZARD,:AIRSLASH].include?(party[partyindex].moves[ret].id)
    end
    party[partyindex].add_first_move(party[partyindex].moves[ret].id) if !rotom # DemICE Move Remember
    return ret
  end
