...
class MapBottomSprite < Sprite
  ...
  TEXT_MAIN_COLOR   = Color.new(234, 234, 234)
  TEXT_SHADOW_COLOR = Color.new(82, 82, 82)
...
class PokemonRegionMap_Scene
  ...
  def pbStartScene(as_editor = false, fly_map = false)
    ...
    (LEFT..RIGHT).each do |i|
      (TOP..BOTTOM).each do |j|
        ...
        # Checks for all cases where the map should be unavailable
        next if !healspot || !$PokemonGlobal.visitedMaps[healspot[0]] || (healspot[0] == 600 && !$game_switches[231]) ||
        (healspot[0] == 446 && $game_switches[177]) || (healspot[0] == 444 && !$game_switches[177]) || 
        (healspot[0] == 42 && $player.character_ID == 2) #Michael Note: Prevents Mordecai from warping to the Café District
        # Change icon based on location
        fly_icon = "mapFly"
        if find_region_group(@current_region).include?(5)
          fly_icon = "mapFlyUnderground"
        elsif find_region_group(@current_region).include?(6) || find_region_group(@current_region).include?(7)
          fly_icon = "mapFlyTrebel"
        elsif find_region_group(@current_region).include?(8)
          fly_icon = "mapFlyFogdale"
        end
        @sprites["point#{k}"] = AnimatedSprite.create("Graphics/Pictures/" + fly_icon, 2, 16)
    ...
    @sprites["cursor"].play
    # Added for showing if map can be scrolled 
    if find_region_group(@current_region).length > 1
      @sprites["switcher"] = IconSprite.new(0, 0, @viewport)
      @sprites["switcher"].setBitmap("Graphics/Pictures/mapSwitch")
      @sprites["switcher"].x = 282
      @sprites["switcher"].y = 4
    end
    @sprites["help"] = BitmapSprite.new(Graphics.width, 32, @viewport)
    pbSetSystemFont(@sprites["help"].bitmap)
   # refresh_fly_screen
   refresh_fast_travel_screen
    @changed = false
  ...
  def refresh_fly_screen
    return if @fly_map || !Settings::CAN_FLY_FROM_TOWN_MAP || !pbCanFly?  # Can we use this somehow?
    @sprites["help"].bitmap.clear
    text = (@mode == 0) ? _INTL("") : _INTL("") #Ekat note: text = (@mode == 0) ? _INTL("ACTION: Fly") : _INTL("ACTION: Cancel Fly")
    pbDrawTextPositions(
      @sprites["help"].bitmap,
  ...
  def refresh_fast_travel_screen
    return if @fly_map || !Settings::CAN_FLY_FROM_TOWN_MAP || !pbCanFastTravel?  # Can we use this somehow?
    @sprites["help"].bitmap.clear
    text = (@mode == 0) ? _INTL("") : _INTL("") #Ekat note: text = (@mode == 0) ? _INTL("ACTION: Fly") : _INTL("ACTION: Cancel Fly")
    pbDrawTextPositions(
      @sprites["help"].bitmap,
  ...
  def swap_maps(map_index, starting_region, playerX, playerY)
    # Added for map switching
    xval = @sprites["cursor"].x
    yval = @sprites["cursor"].y
    @map     = @map_data[map_index]
=begin
    mapsize = $game_map.metadata.town_map_size
    if mapsize && mapsize[0] && mapsize[0] > 0
      sqwidth  = mapsize[0]
      sqheight = (mapsize[1].length.to_f / mapsize[0]).ceil
    end
=end
    @current_region = map_index
    addBackgroundOrColoredPlane(@sprites, "background", "mapbg", Color.new(0, 0, 0), @viewport)
    @sprites["map"] = IconSprite.new(0, 0, @viewport)
    @sprites["map"].setBitmap("Graphics/Pictures/#{@map[1]}")
    @sprites["map"].x += (Graphics.width - @sprites["map"].bitmap.width) / 2
    @sprites["map"].y += (Graphics.height - @sprites["map"].bitmap.height) / 2
    Settings::REGION_MAP_EXTRAS.each do |graphic|
      next if graphic[0] != map_index || !location_shown?(graphic)
      if !@sprites["map2"]
        @sprites["map2"] = BitmapSprite.new(480, 320, @viewport)
        @sprites["map2"].x = @sprites["map"].x
        @sprites["map2"].y = @sprites["map"].y
    ...
    @sprites["mapbottom"] = MapBottomSprite.new(@viewport)
    @sprites["mapbottom"].mapname     = pbGetMessage(MessageTypes::RegionNames, map_index)
    @sprites["mapbottom"].maplocation = pbGetMapLocation(@map_x, @map_y)
    @sprites["mapbottom"].mapdetails  = pbGetMapDetails(@map_x, @map_y)
    draw_roaming_position(map_index)
    if @sprites["player"] && map_index == starting_region
      @sprites["player"] = IconSprite.new(0, 0, @viewport)
      @sprites["player"].setBitmap(GameData::TrainerType.player_map_icon_filename($player.trainer_type))
      @sprites["player"].x = playerX
      @sprites["player"].y = playerY
    end
    k = 0
    (LEFT..RIGHT).each do |i|
      (TOP..BOTTOM).each do |j|
    ...
    @sprites["cursor"] = AnimatedSprite.create("Graphics/Pictures/mapCursor", 2, 5)
    @sprites["cursor"].viewport = @viewport
    @sprites["cursor"].x        = xval
    @sprites["cursor"].y        = yval
    @sprites["cursor"].play
    # Added for showing if map can be scrolled 
    if find_region_group(@current_region).length > 1
      @sprites["switcher"] = IconSprite.new(0, 0, @viewport)
  ...
  # Function to define possible regions nased on player's starting area
  def find_region_group(starting_region)
    case starting_region
    when 0, 1, 3, 4
      map_list = [0]
      map_list.push(1) if $game_switches[201]
      map_list.push(3) if $game_switches[155]
      map_list.push(4) if $game_switches[168]
      return map_list
    else
  ...
  def pbMapScene
    ...
    if @sprites["player"]
      playerX = @sprites["player"].x
      playerY = @sprites["player"].y
    end
    x_offset = 0
    y_offset = 0
    new_x    = 0
    ...
    loop do
      ...
      if Input.trigger?(Input::BACK)
        ...
        # Get possible maps to swap to
        available_maps = find_region_group(starting_region)
        region_index = available_maps.find_index(current_region)
        # Get index of current region map and set next region
        new_region = (available_maps[(region_index - 1) % available_maps.length] ) 
        swap_maps(new_region, starting_region, playerX, playerY) unless available_maps.length == 1
...
=end
        ...
        if healspot && ($PokemonGlobal.visitedMaps[healspot[0]] ||
          ...
          return healspot if @fly_map
          name = pbGetMapNameFromId(healspot[0])
          return healspot if pbConfirmMessage(_INTL("Would you like to fast travel to {1}?", name)) { pbUpdate }
        end
    ...
    # elsif Input.trigger?(Input::ACTION) && !@wallmap && !@fly_map && pbCanFly?
      elsif Input.trigger?(Input::ACTION) && !@wallmap && !@fly_map && pbCanFastTravel?
        pbPlayDecisionSE
        @mode = (@mode == 1) ? 0 : 1
      # refresh_fly_screen
        refresh_fast_travel_screen
...
class PokemonRegionMapScreen
  ...
  def pbStartFlyScreen2
    @scene.pbStartScene(false, true)
    ret = @scene.pbMapScene
    if ret
      $game_temp.fly_destination = ret 
      @scene.pbFastTravelToNewLocation
    else
      @scene.pbEndScene
    end
    return ret
