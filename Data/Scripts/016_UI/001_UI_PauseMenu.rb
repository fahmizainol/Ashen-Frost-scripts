...
class PokemonPauseMenu_Scene
  def pbStartScene
    ...
    @sprites["infowindow"] = Window_AdvancedTextPokemon.newWithSize("", 0, 0, 32, 32, @viewport)
    @sprites["infowindow"].visible = false
    @sprites["helpwindow"] = Window_AdvancedTextPokemon.newWithSize("", 0, 0, 32, 32, @viewport)
  ...
  def pbShowInfo(text)
    @sprites["infowindow"].resizeToFit(text, 300)
  ...
  def pbShowCommands(commands)
    ...
    cmdwindow.index    =  $game_temp.menu_last_choice <= (cmdwindow.commands.length - 1) ? $game_temp.menu_last_choice : 0
    ...
    loop do
      ...
      Input.update
      pbUpdateSceneMap
      pbShowInfo(_INTL("<ac>{1}</ac> Money: <r>${2}<br> Level Cap: <r>{3}<br>", $game_map.name, $player.money.to_s_formatted,
      $player.level_cap))
...
MenuHandlers.add(:pause_menu, :case_menu, {
  "name"      => _INTL("Case File"),
  "order"     => 20,
  "condition" => proc { next ["Sylvester", "Mordecai", "Arturo", "Scottsborough", "Lucile", "Luciano"].any?($player.name)  },
  "effect"    => proc { |menu|
    pbPlayDecisionSE
    pbFadeOutIn {
      CaseUI.new
...
MenuHandlers.add(:pause_menu, :bag, {
  "name"      => _INTL("Bag"),
  "order"     => 30,
  "condition" => proc { next !pbInBugContest? && ![3, 4, 10].any?($player.character_ID) },
  "effect"    => proc { |menu|
    pbPlayDecisionSE
    item = nil
    pbFadeOutIn {
...
#Ekat Note: This is for making my life easier while testing the game, this menu will actually be nested in the phone on release. 
...
MenuHandlers.add(:pause_menu, :save, {
  ...
  "effect"    => proc { |menu|
    ...
    if $joiplay ? screen.pbSaveScreenJoiplay : screen.pbSaveScreen
      menu.pbEndScene
      next true
    end
    menu.pbRefresh
...
MenuHandlers.add(:pause_menu, :controls, {
  "name"      => _INTL("Controls"),
  "order"     => 70,
  "condition" => proc { next !$joiplay && !Input::Controller.connected? },
  "effect"    => proc { |menu|
    pbPlayDecisionSE
    # Added ability to remap controller in script
    if Input::Controller.connected?
      menu.pbHideMenu
      cmd = pbMessage(_INTL('Which control scheme would you like to remap?'), ["Keyboard Controls", "Gamepad Controls", "Cancel"], 3)
      case cmd
      when 0
        open_set_controls_ui(menu)
      when 1
        open_set_controls_ui(menu, true)
      else
        pbPlayCancelSE
        menu.pbShowMenu
      end
...
MenuHandlers.add(:pause_menu, :quit_game, {
  ...
  "effect"    => proc { |menu|
    ...
    if pbConfirmMessage(_INTL("Are you sure you want to quit the game?"))
      scene = PokemonSave_Scene.new
      screen = PokemonSaveScreen.new(scene)
      $joiplay ? screen.pbSaveScreenJoiplay(true) : screen.pbSaveScreen(true) 
      menu.pbEndScene
