...
module PluginManager
  ...
  def self.runPlugins
    ...
    scripts.each do |plugin|
      ...
      # Joiplay check
      next if ["Discord Rich Presence API", "FL's Controls Menu"].include?(name) && $joiplay 
      if !meta[:essentials] || !meta[:essentials].include?(Essentials::VERSION)
        #Console.echo_warn "Plugin '#{name}' may not be compatible with Essentials v#{Essentials::VERSION}. Trying to load anyway." - Ekat Note: commended this out bc it's annoying
      end
      # register plugin
      self.register(meta)
