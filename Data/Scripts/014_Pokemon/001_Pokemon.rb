...
class Pokemon
  ...
  def can_relearn_move?
    ...
    getMoveList.each { |m| return true if m[0] == 1 && !hasMove?(m[1]) && !@first_moves.include?(m[1]) }
  ...
  def check_evolution_internal
    ...
    return nil if $game_switches[420]
  ...
  def calcIV
    ...
    GameData::Stat.each_main do |s|
      if @ivMaxed[s.id] && self.iv[s.id] < 31  #DemICE bandaid change without having to edit every common event
        self.iv[s.id] = 31 
      end
  ...
  def calc_stats
    ...
    self.hp = self.totalhp if self.hp > self.totalhp
    @attack  = stats[:ATTACK]
