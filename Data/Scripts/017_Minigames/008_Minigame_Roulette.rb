...
#    end
  ...
  def pbDrawCredits
    ...
    overlay.clear
    textPosition=[$player.coins.to_s,
  ...
  def pbAddCredits(number)
    return false if $player.coins+number<0
    $player.coins+=number
...
#def pbRoulette(wager=1) (EKAT Note: OLD)
...
def pbRoulette(wager=1) #Ekat Note- I just felt like fuckign around to see if I could update this shit
  if !$bag.has?(:COINCASE)
    pbMessage(_INTL("It's a Roulette."))
