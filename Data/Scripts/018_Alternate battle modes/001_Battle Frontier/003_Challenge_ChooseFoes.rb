...
def pbGenerateBattleTrainer(idxTrainer, rules)
  ...
  # Randomize idxTrainer if Lucile/Fraser rolled
  idxTrainer -= rand(298) + 1 if idxTrainer > 299 
  # Account for the boss trainers
  if $game_switches[191]
    idxTrainer = $game_switches[190] ? 300 : 301
  end
  # Create the trainer
  ...
  loop do
    ...
    while opponent.party.length < rules.ruleset.suggestedNumber
      rnd = pokemonnumbers[rand(pokemonnumbers.length - 1)]
