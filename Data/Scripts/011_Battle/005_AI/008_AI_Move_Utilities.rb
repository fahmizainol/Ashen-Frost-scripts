class Battle::AI
  ...
  def pbTargetsMultiple?(move, user)
    ...
    return target.allAllies.length + 1 if target.allAllies.length > 0 && move.function == "HitTwoTimesTargetThenTargetAlly"
  ...
  def pbRoughDamage(move, user, target, skill, baseDmg)
    ...
    # Parental Bond
    if skill >= PBTrainerAI.mediumSkill && (user.hasActiveAbility?(:PARENTALBOND) || ($game_map.map_id == 488 && $game_variables[401] == 11))
  ...
  def pbCalcAccuracyModifiers(user, target, modifiers, move, type, skill)
    ...
    if skill >= PBTrainerAI.highSkill
      ...
      if type == :FIRE && @battle.pbWeather == :HolyInferno
        modifiers[:base_accuracy] = 0
      end
