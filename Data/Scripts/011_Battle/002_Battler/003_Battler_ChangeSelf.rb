class Battle::Battler
  ...
  def pbFaint(showMessage = true)
    ...
    self.statusCount = 0
    # Lose happiness
    # DemICE disable happiness loss when using grinding trainer
    losehapp=false
    if !@battle.wildBattle?
      losehapp = true if @battle.opponent[0].trainer_type != :IVTRAINER_Lvl
    end
    if @pokemon && @battle.internalBattle && losehapp
  ...
  def pbCheckFormOnWeatherChange(ability_changed = false)
    ...
    if isSpecies?(:CASTFORM)
      if hasActiveAbility?(:FORECAST)
        ...
        when :Sun, :HarshSun, :HolyInferno, :SolarWinds    then newForm = 1
        when :Rain, :HeavyRain, :FreezingRain              then newForm = 2
        when :Hail, :FrozenStorm, :Snow                    then newForm = 3
        end
        if @form != newForm
    ...
    if isSpecies?(:CHERRIM)
      if hasActiveAbility?(:FLOWERGIFT)
        newForm = 0
        newForm = 1 if [:Sun, :HarshSun, :HolyInferno, :SolarWinds].include?(effectiveWeather)
        if @form != newForm
          @battle.pbShowAbilitySplash(self, true)
          @battle.pbHideAbilitySplash(self)
          pbChangeForm(newForm, _INTL("{1} transformed!", pbThis))
    ...
    if !ability_changed && isSpecies?(:EISCUE) && self.ability == :ICEFACE &&
       @form == 1 && [:Hail, :FrozenStorm, :Snow, :FreezingRain].include?(effectiveWeather)
  ...
  def pbCheckForm(endOfRound = false)
    ...
    # Severn Trubbish - Shields Down
    if isSpecies?(:TRUBBISH) && self.ability == :SHIELDSDOWN
      if @hp > @totalhp / 2   # Turn into In-Recycling form
        if @form != 1
          @battle.pbShowAbilitySplash(self, true)
          @battle.pbHideAbilitySplash(self)
          pbChangeForm(1, _INTL("{1} deactivated!", abilityName))
        elsif !endOfRound
          @battle.pbDisplay(_INTL("{1} deactivated!", abilityName))
        end
      elsif @form != 2   # Turn into Out-of-Recycling form
        @battle.pbShowAbilitySplash(self, true)
        @battle.pbHideAbilitySplash(self)
        pbChangeForm(2, _INTL("{1} activated!", abilityName))
      end
    end    
  end
