...
#      :FORECAST,                                          # This can be stopped
  ...
  def takesFreezingRainDamage?
    return false if !takesIndirectDamage?
    return false if pbHasType?(:ICE) || pbHasType?(:WATER)
    return false if inTwoTurnAttack?("TwoTurnAttackInvulnerableUnderground",
                                     "TwoTurnAttackInvulnerableUnderwater")
    return false if hasActiveAbility?([:OVERCOAT, :ICEBODY, :SNOWCLOAK, :STORMDRAIN, :HYDRATION])
    return false if hasActiveItem?(:SAFETYGOGGLES)
    return true
  end

  def takesFrozenStormDamage?
    return false if !takesIndirectDamage?
    return false if (pbHasType?(:GROUND) || pbHasType?(:ELECTRIC)) && !hasActiveAbility?([:VOLTABSORB, :LIGHTNINGROD, :MOTORDRIVE, :ELECTROMORPHOSIS])
    return false if inTwoTurnAttack?("TwoTurnAttackInvulnerableUnderground",
                                     "TwoTurnAttackInvulnerableUnderwater")
    return false if hasActiveAbility?(:OVERCOAT)
    return false if hasActiveItem?(:SAFETYGOGGLES)
    # Modulo (%) gets remainder - should loop for entirety of the battle
    return false if !(@battle.turnCount % 6 == 1 && !pbOwnedByPlayer?) && !(@battle.turnCount % 6 == 3 && pbOwnedByPlayer?) && !(@battle.turnCount % 6 == 5)
    return true
  end

  def takesHolyInfernoDamage?
    return false if !takesIndirectDamage?
    return false if pbHasType?(:FIRE)
    return false if inTwoTurnAttack?("TwoTurnAttackInvulnerableUnderground",
                                     "TwoTurnAttackInvulnerableUnderwater")
    return false if hasActiveAbility?([:OVERCOAT, :FLASHFIRE, :HEATPROOF, :SOLARPOWER, :FLAMEBODY, :MAGMAARMOR, :WATERVEIL, :WATERBUBBLE, :THERMALEXCHANGE])
    return false if hasActiveItem?(:SAFETYGOGGLES)
    return true
  end
  ...
  def effectiveWeather
    ...
    ret = :None if [:Sun, :Rain, :HarshSun, :HeavyRain, :FreezingRain].include?(ret) && hasActiveItem?(:UTILITYUMBRELLA)
    return ret
  end
