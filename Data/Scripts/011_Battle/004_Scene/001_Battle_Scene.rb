...
class Battle::Scene
  ...
  MESSAGE_SHADOW_COLOR = Color.new(84, 85, 109)
  ...
  #DemICE adjusting message pause time depending on speed
  def messagePausetime
    case $GameSpeed
    when 0
      return MESSAGE_PAUSE_TIME
    when 2
      return MESSAGE_PAUSE_TIME*0.6
    when 4
      return MESSAGE_PAUSE_TIME*0.3
    end
  end
  ...
  def pbWaitMessage
    ...
    while System.uptime - timer_start < messagePausetime
      pbUpdate(cw)
    end
    cw.text    = ""
    cw.visible = false
  ...
  def pbDisplayMessage(msg, brief = false)
    ...
    loop do
      ...
      if !cw.busy?
        ...
        if System.uptime - timer_start >= messagePausetime   # Autoclose after 1 second
          cw.text = ""
          cw.visible = false
          break
        end
  ...
  def pbDisplayPausedMessage(msg)
    ...
    loop do
      ...
      if !cw.busy?
        ...
        if !@battleEnd
          if System.uptime - timer_start >= messagePausetime * 3   # Autoclose after 3 seconds
            cw.text = ""
            cw.visible = false
            break
          end
