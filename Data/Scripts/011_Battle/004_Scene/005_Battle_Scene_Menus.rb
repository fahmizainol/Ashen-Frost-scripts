  ...
  def initialize(viewport, z)
    ...
    if USE_GRAPHICS
      ...
      @buttons = Array.new(4) do |i|   # 4 command options, therefore 4 buttons
        ...
        button.y = self.y + 6
        button.y += (((i / 2) == 0) ? 0 : BUTTON_HEIGHT - 14)
...
class Battle::Scene::FightMenu < Battle::Scene::MenuBase
  ...
  PP_COLORS = [
    ...
    Color.new(72, 72, 72), Color.new(186, 186, 186)    # Black, more than 1/2 of total PP
  ]
  ...
  def initialize(viewport, z)
    ...
    if USE_GRAPHICS
      ...
      @statsBitmap   = AnimatedBitmap.new(_INTL("Graphics/Pictures/Battle/cursor_stats"))
      # Create background graphic
      ...
      @typeIcon.x      = self.x + 434 #Ekat Note: Edited from 416
      ...
      # Create Stats button
      @statsButton = Sprite.new(viewport)
      @statsButton.bitmap = @statsBitmap.bitmap
      @statsButton.x      = self.x + 4
      @statsButton.y      = self.y - @statsBitmap.height
      addSprite("statsButton", @statsButton)
    else
      ...
      @msgBox.baseColor   = Color.new(72, 72, 72) 
      @msgBox.shadowColor = Color.new(186, 186, 186)
  ...
  def refreshButtonNames
    ...
    @buttons.each_with_index do |button, i|
      ...
      textPos.push([moves[i].name, x, y, 2, moveNameBase, Color.new(186, 186, 186)])
    end
  ...
  def refreshMoveData(move)
    ...
    if !USE_GRAPHICS
      ...
      if move.total_pp <= 0
        ...
        @msgBox.text = _ISPRINTF(" {1: 2d}/{2: 2d}<br>TYPE/{3:s}",
...
class Battle::Scene::TargetMenu < Battle::Scene::MenuBase
  ...
  TEXT_BASE_COLOR   = Color.new(72, 72, 72)
