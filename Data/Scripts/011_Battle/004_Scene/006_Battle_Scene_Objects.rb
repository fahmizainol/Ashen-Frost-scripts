...
class Battle::Scene::PokemonDataBox < Sprite
  ...
  def initializeDataBoxGraphic(sideSize)
    ...
    case sideSize
    when 2
      @spriteX += [0,  12,  -12,  0][@battler.index]
...
#    pbSetSmallFont(@hpNumbers.bitmap)
  ...
  def draw_name
    ...
    nameOffset -= 10 if @battler.name == "Arceus-Steel" && !@battler.pbOwnedByPlayer? # The meme must stay accurate
    pbDrawTextPositions(self.bitmap,
      [[@battler.name, @spriteBaseX + 16 - nameOffset, 12, false, NAME_BASE_COLOR, NAME_SHADOW_COLOR]]
    )
  end
  ...
  def draw_level
    ...
    pbDrawImagePositions(self.bitmap,
      [["Graphics/Pictures/Battle/overlay_lv", @spriteBaseX + 152, 16]]
    )
    # Level number
    pbDrawNumber(@battler.level, self.bitmap, @spriteBaseX + 174, 16)
  end
  ...
  def draw_gender
    ...
    pbDrawTextPositions(self.bitmap, [[gender_text, @spriteBaseX + 140, 12, false, base_color, shadow_color]])
  end
  ...
  def draw_status
    ...
    return if s < 0
    pbDrawImagePositions(self.bitmap, [["Graphics/Pictures/Battle/icon_statuses", @spriteBaseX + 20, 34,
  ...
  def draw_shiny_icon
    ...
    pbDrawImagePositions(self.bitmap, [["Graphics/Pictures/shiny", @spriteBaseX + shiny_x, 34]])
  end
...
class Battle::Scene::AbilitySplashBar < Sprite
  ...
  TEXT_BASE_COLOR   = Color.new(234, 234, 234)
  TEXT_SHADOW_COLOR = Color.new(66, 66, 66) 
  ...
  def refresh
    ...
    textPos.push([_INTL("{1}'s", @battler.name), textX, 8, @side == 1,
                  TEXT_BASE_COLOR, TEXT_SHADOW_COLOR, false])
    # Draw Pokémon's ability
    textPos.push([@battler.abilityName, textX, 36, @side == 1,
                  TEXT_SHADOW_COLOR, TEXT_BASE_COLOR, false])
...
class Battle::Scene::BattlerSprite < RPG::Sprite
  ...
  def setPokemonBitmapSpecies(pkmn, species, back = false)
    @pkmn = pkmn
    @_iconbitmap&.dispose
    @_iconbitmap = (pkmn) ? GameData::Species.sprite_bitmap_from_pokemon(@pkmn, back, species) : nil
    self.bitmap = (@_iconbitmap) ? @_iconbitmap.bitmap : nil
    pbSetPosition
  end
