...
class Battle::Move::StartWeakenDamageAgainstUserSideIfHail < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    if user.effectiveWeather != :Hail && user.effectiveWeather != :FreezingRain && user.effectiveWeather != :FrozenStorm
      @battle.pbDisplay(_INTL("But it failed!"))
      return true
    end
...
class Battle::Move::TypeAndPowerDependOnWeather < Battle::Move
  ...
  def pbCalcTypeModSingle(moveType, defType, user, target)
    ret = super
    if user.effectiveWeather == :FreezingRain && GameData::Type.exists?(:WATER) 
