...
class Battle::Move::AttackAndSkipNextTurn < Battle::Move
  def pbEffectGeneral(user)
    if !user.hasActiveItem?(:BATTERYPACK)
...
class Battle::Move::TwoTurnAttackOneTurnInSun < Battle::Move::TwoTurnMove
  def pbIsChargingTurn?(user)
    ...
    if !user.effects[PBEffects::TwoTurnAttack] &&
       [:Sun, :HarshSun, :HolyInferno].include?(user.effectiveWeather)
      @powerHerb = false
      @chargingTurn = true
      @damagingTurn = true
      return false
  ...
  def pbBaseDamageMultiplier(damageMult, user, target)
    damageMult /= 2 if ![:None, :Sun, :HarshSun, :HolyInferno].include?(user.effectiveWeather) && !(user.isSpecies?(:MOLTRES) && user.hasActiveItem?(:ARDENTORB) && user.form == 0) && !user.hasActiveItem?(:POWERPACK)
    return damageMult
  end
end
...
class Battle::Move::TwoTurnAttackChargeRaiseUserSpAtk1OneTurnInRain < Battle::Move::TwoTurnMove
  def pbIsChargingTurn?(user)
    ret = super
    if !user.effects[PBEffects::TwoTurnAttack] &&
       [:Rain, :HeavyRain, :FreezingRain, :FrozenStorm].include?(user.effectiveWeather)
      @powerHerb = false
      @chargingTurn = true
      @damagingTurn = true
      return false
