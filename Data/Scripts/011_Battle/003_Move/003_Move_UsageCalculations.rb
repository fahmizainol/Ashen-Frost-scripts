class Battle::Move
  ...
  def pbBaseType(user)
    ...
    return :QMARKS if $game_variables[401] == 12 && @name == "Explosion"
    return ret
  end
  ...
  def pbCalcTypeModSingle(moveType, defType, user, target)
    ...
    if Effectiveness.ineffective_type?(moveType, defType)
      ...
      # Foresight
      if (user.hasActiveAbility?(:SCRAPPY) || target.effects[PBEffects::Foresight]) && #Michael Note: Reference this section when looking into coding an ability for Delcatty
         defType == :GHOST
        ret = Effectiveness::NORMAL_EFFECTIVE_ONE
      end
      # Miracle Eye
  ...
  def pbAccuracyCheck(user, target)
    ...
    return true if @calcType == :FIRE && @battle.field.weather == :HolyInferno
    baseAcc = pbBaseAccuracy(user, target)
    return true if baseAcc == 0
    # Calculate all multiplier effects
    modifiers = {}
    modifiers[:base_accuracy]  = baseAcc
    modifiers[:base_accuracy] += 20 if !user.pbOwnedByPlayer? && $PokemonGlobal.difficulty == 2
  ...
  def pbGetAttackStats(user, target)
    # Added power and energy converter
    if specialMove? && user.hasActiveItem?(:ENCONVERTER)
      return user.attack, user.stages[:ATTACK] + 6
    elsif specialMove? || (physicalMove? && user.hasActiveItem?(:PWCONVERTER))
  ...
  def pbCalcDamageMultipliers(user, target, numTargets, type, baseDmg, multipliers)
    ...
    if user.effects[PBEffects::ParentalBond] == 1 && !($game_variables[401] == 11 && $game_map.map_id == 488)
