...
class Battle::Move::SwitchOutTargetStatusMove < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if target.effects[PBEffects::Commander]
      @battle.pbDisplay(_INTL("{1} cannot be forced out!", target.pbThis)) if show_message
      return true
    end
    if !@battle.canRun
  ...
  def pbSwitchOutTargetEffect(user, targets, numHits, switched_battlers)
    if $PokemonTemp.orderData["hasAce"] 
      @battle.pbDisplay(_INTL("The enemy cannot be sent back during this battle!"))
      return
    end
...
class Battle::Move::TargetUsesItsLastUsedMoveAgain < Battle::Move
  ...
  def initialize(battle, move)
    ...
    @moveBlacklist = [
      ...
      "TwoTurnAttackChargeRaiseUserSpAtk1OneTurnInRain"   # Electro Shot               # Not listed on Bulbapedia
    ]
  end
...
class Battle::Move::StartSlowerBattlersActFirst < Battle::Move
  def pbEffectGeneral(user)
    if @battle.field.effects[PBEffects::TrickRoom] > 0
      ...
      TrainerDialogue.display("roomOff",@battle,@battle.scene)
    else
