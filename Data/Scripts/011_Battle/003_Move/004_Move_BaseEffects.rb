...
class Battle::Move::TwoTurnMove < Battle::Move
  ...
  def pbIsChargingTurn?(user)
    ...
    if !user.effects[PBEffects::TwoTurnAttack]
      @powerHerb = user.hasActiveItem?(:POWERHERB) || user.hasActiveItem?(:POWERPACK) || (user.isSpecies?(:MOLTRES) && user.hasActiveItem?(:ARDENTORB) && user.form == 0)
      @chargingTurn = true
  ...
  def pbInitialEffect(user, targets, hitNum)
    ...
    if @chargingTurn && @damagingTurn   # Move only takes one turn to use
      ...
      if @powerHerb
        ...
        if user.hasActiveItem?(:POWERPACK)
          @battle.pbDisplay(_INTL("{1} became fully charged due to its Power Pack!", user.pbThis))
        elsif (user.isSpecies?(:MOLTRES) && user.hasActiveItem?(:ARDENTORB) && user.form == 0)
            @battle.pbDisplay(_INTL("{1} became fully charged due to its Ardent Orb!", user.pbThis))
        else
          @battle.pbDisplay(_INTL("{1} became fully charged due to its Power Herb!", user.pbThis))
          user.pbConsumeItem
        end
      end
      # Solar Winds poisoning effect 
      if ["TwoTurnAttackInvulnerableInSky",
         "TwoTurnAttackInvulnerableInSkyParalyzeTarget",
         "TwoTurnAttackInvulnerableInSkyTargetCannotAct"].include?(@function) && 
          @battle.field.weather == :SolarWinds && user.pbCanPoison?
            user.pbPoison(nil, _INTL("The strange winds badly poisoned {1}!", user.pbThis), true)
      end
      if @function == "TwoTurnAttackInvulnerableInSkyTargetCannotAct" && @battle.field.weather == :SolarWinds && target.pbCanPoison?
         target.pbPoison(nil, _INTL("The strange winds badly poisoned {1}!", target.pbThis), true)
      end
    end
...
class Battle::Move::RecoilMove < Battle::Move
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    return if user.hasActiveItem?(:CRASHTESTHELMET)
    amt = pbRecoilDamage(user, target)
    halfamt = amt/2
    amt = 1 if amt < 1
    if user.isSpecies?(:ZAPDOS) && user.hasActiveItem?(:VOLTAICORB) && user.form == 0
      user.pbRecoverHP(amt, false)
      @battle.pbDisplay(_INTL("The Voltaic Orb converted the recoil into energy!", user.pbThis))
    else
...
class Battle::Move::WeatherMove < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    ...
    when :StrongWinds
      ...
      return true
    when :FrozenStorm
      @battle.pbDisplay(_INTL("The frozen thunderstorm is too fierce to be stopped!"))
      return true
    when :HolyInferno
      @battle.pbDisplay(_INTL("There is no relief from this inferno!"))
      return true
    when @weatherType
      @battle.pbDisplay(_INTL("But it failed!"))
      return true
