...
module Battle::AbilityEffects
  ...
  EndOfRoundWeatherDamage          = AbilityHandlerHash.new   # Ashen Frost weather effects
  ...
  def self.triggerEndOfRoundWeatherDamage(ability, weather, battler, battle)
    EndOfRoundWeatherDamage.trigger(ability, weather, battler, battle)
  end
...
Battle::AbilityEffects::SpeedCalc.add(:CHLOROPHYLL,
  proc { |ability, battler, mult|
    next mult * 2 if [:Sun, :HarshSun, :HolyInferno].include?(battler.effectiveWeather)
  }
)
...
Battle::AbilityEffects::SpeedCalc.add(:SLUSHRUSH,
  proc { |ability, battler, mult|
    next mult * 2 if [:Hail, :FreezingRain, :FrozenStorm].include?(battler.effectiveWeather)
  }
)

Battle::AbilityEffects::SpeedCalc.add(:SURGESURFER,
  proc { |ability, battler, mult|
    next mult * 2 if battler.battle.field.terrain == :Electric || battler.effectiveWeather == :FrozenStorm
  }
)

Battle::AbilityEffects::SpeedCalc.add(:SWIFTSWIM,
  proc { |ability, battler, mult|
    next mult * 2 if [:Rain, :HeavyRain, :FreezingRain, :FrozenStorm].include?(battler.effectiveWeather)
  }
)
...
Battle::AbilityEffects::StatusImmunity.add(:LEAFGUARD,
  proc { |ability, battler, status|
    next true if [:Sun, :HarshSun, :HolyInferno].include?(battler.effectiveWeather)
  }
)
...
Battle::AbilityEffects::StatusImmunityNonIgnorable.add(:SHIELDSDOWN,
  proc { |ability, battler, status|
    ...
    # Added Severn Trubbish code
    next true if battler.isSpecies?(:TRUBBISH) && battler.form == 1
  }
)
...
Battle::AbilityEffects::MoveBlocking.add(:DAZZLING,
  proc { |ability, bearer, user, targets, move, battle|
    next false if $game_variables[401] == 12 && move.name == "Explosion"
...
Battle::AbilityEffects::AccuracyCalcFromTarget.add(:SNOWCLOAK,
  proc { |ability, mods, user, target, move, type|
    mods[:evasion_multiplier] *= 1.25 if [:Hail, :FreezingRain, :FrozenStorm].include?(target.effectiveWeather)
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:FLAMEBODY,
  proc { |ability, user, target, move, mults, baseDmg, type|
...
Battle::AbilityEffects::DamageCalcFromUser.add(:FLOWERGIFT,
  proc { |ability, user, target, move, mults, baseDmg, type|
    if move.physicalMove? && [:Sun, :HarshSun, :HolyInferno].include?(user.effectiveWeather)
      mults[:attack_multiplier] *= 1.5
    end
  }
)
...
Battle::AbilityEffects::DamageCalcFromUser.add(:SOLARPOWER,
  proc { |ability, user, target, move, mults, baseDmg, type|
    if move.specialMove? && [:Sun, :HarshSun, :HolyInferno].include?(user.effectiveWeather)
      mults[:attack_multiplier] *= 1.5
    end
  }
)
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:FLOWERGIFT,
  proc { |ability, user, target, move, mults, baseDmg, type|
    if move.specialMove? && [:Sun, :HarshSun, :HolyInferno].include?(target.effectiveWeather)
...
#Ekat Note: Severn Delcatty's Ability
Battle::AbilityEffects::DamageCalcFromTarget.add(:PEDIGREE,
  proc { |ability, user, target, move, mults, baseDmg, type|
    mults[:defense_multiplier] *= 2 if move.physicalMove? ||
                                       move.function == "UseTargetDefenseInsteadOfTargetSpDef"   # Psyshock
...
Battle::AbilityEffects::EndOfRoundWeather.add(:DRYSKIN,
  proc { |ability, weather, battler, battle|
    ...
    when :Rain, :HeavyRain, :FreezingRain#, :FrozenStorm
...
Battle::AbilityEffects::EndOfRoundWeather.add(:ICEBODY,
  proc { |ability, weather, battler, battle|
    next unless [:Hail, :FreezingRain, :FrozenStorm].include?(weather)
    next if !battler.canHeal?
    battle.pbShowAbilitySplash(battler)
...
Battle::AbilityEffects::EndOfRoundWeather.add(:ICEFACE,
  proc { |ability, weather, battler, battle|
    next if ![:Hail, :FreezingRain, :FrozenStorm].include?(weather)
...
Battle::AbilityEffects::EndOfRoundWeather.add(:RAINDISH,
  proc { |ability, weather, battler, battle|
    next unless [:Rain, :HeavyRain, :FreezingRain, :FrozenStorm].include?(weather)
    next if !battler.canHeal?
    battle.pbShowAbilitySplash(battler)
    battler.pbRecoverHP(battler.totalhp / 16)
    if Battle::Scene::USE_ABILITY_SPLASH
...
Battle::AbilityEffects::EndOfRoundWeather.add(:SOLARPOWER,
  proc { |ability, weather, battler, battle|
    next unless [:Sun, :HarshSun, :HolyInferno].include?(weather)
    battle.pbShowAbilitySplash(battler)
    battle.scene.pbDamageAnimation(battler)
    battler.pbReduceHP(battler.totalhp / 8, false)
    battle.pbDisplay(_INTL("{1} was hurt by the sunlight!", battler.pbThis))
...
Battle::AbilityEffects::EndOfRoundWeather.add(:FLASHFIRE,
  proc { |ability, weather, battler, battle|
    next unless weather == :HolyInferno
...
Battle::AbilityEffects::EndOfRoundHealing.add(:HYDRATION,
  proc { |ability, battler, battle|
    ...
    next if ![:Rain, :HeavyRain, :FreezingRain, :FrozenStorm].include?(battler.effectiveWeather)
    battle.pbShowAbilitySplash(battler)
...
Battle::AbilityEffects::EndOfRoundGainItem.add(:HARVEST,
  proc { |ability, battler, battle|
    ...
    if ![:Sun, :HarshSun, :HolyInferno].include?(battler.effectiveWeather)
...
#===============================================================================
# EndOfRoundWeatherDamage handlers
#===============================================================================

Battle::AbilityEffects::EndOfRoundWeatherDamage.add(:VOLTABSORB,
  proc { |ability, weather, battler, battle|
    next unless weather == :FrozenStorm
    battle.pbShowAbilitySplash(battler)
    if battler.canHeal? && battler.pbRecoverHP(battler.totalhp / 4) > 0
      if Battle::Scene::USE_ABILITY_SPLASH
        battle.pbDisplay(_INTL("{1}'s HP was restored.", battler.pbThis))
      else
        battle.pbDisplay(_INTL("{1}'s {2} restored its HP.", battler.pbThis, battler.abilityName))
      ...
      battle.pbDisplay(_INTL("{1} grounded the lightning strike!", battler.pbThis))
    else
      battle.pbDisplay(_INTL("{1}'s {2} grounded the lightning strike!", battler.pbThis, battler.abilityName))  
    end
    battle.pbHideAbilitySplash(battler)
  }
)

Battle::AbilityEffects::EndOfRoundWeatherDamage.add(:LIGHTNINGROD,
  proc { |ability, weather, battler, battle|
    next unless weather == :FrozenStorm
    battle.pbShowAbilitySplash(battler)
    if battler.pbCanRaiseStatStage?(:SPECIAL_ATTACK, battler)
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        battler.pbRaiseStatStageByCause(:SPECIAL_ATTACK, 1, battler, battler.abilityName)
      end
    elsif Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("{1} grounded the lightning strike!", battler.pbThis))
    else
...
Battle::AbilityEffects::EndOfRoundWeatherDamage.add(:MOTORDRIVE,
  proc { |ability, weather, battler, battle|
    next unless weather == :FrozenStorm
    battle.pbShowAbilitySplash(battler)
    if battler.pbCanRaiseStatStage?(:SPEED, battler)
      if Battle::Scene::USE_ABILITY_SPLASH
        ...
        battler.pbRaiseStatStageByCause(:SPEED, 1, battler, battler.abilityName)
      end
    elsif Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("{1} grounded the lightning strike!", battler.pbThis))
    else
...
Battle::AbilityEffects::EndOfRoundWeatherDamage.add(:ELECTROMORPHOSIS,
  proc { |ability, weather, battler, battle|
    next unless weather == :FrozenStorm
    next if battler.fainted?
    next if battler.effects[PBEffects::Charge] > 0
    battle.pbShowAbilitySplash(battler)
    battler.effects[PBEffects::Charge] = 2
    battle.pbDisplay(_INTL("Being hit by the lightning strike charged {1} with power!", battler.pbThis(true)))
    battle.pbHideAbilitySplash(battler)
  }
)
...
Battle::AbilityEffects::OnSwitchIn.add(:ICEFACE,
  proc { |ability, battler, battle, switch_in|
    ...
    next if battler.effectiveWeather != :Hail && battler.effectiveWeather != :FreezingRain && battler.effectiveWeather != :FrozenStorm
    battle.pbShowAbilitySplash(battler)
    if !Battle::Scene::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("{1}'s {2} activated!", battler.pbThis, battler.abilityName))
    end
