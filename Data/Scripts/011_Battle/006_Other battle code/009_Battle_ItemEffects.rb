...
Battle::ItemEffects::AccuracyCalcFromUser.add(:BIONICEYE,
  proc { |item, mods, user, target, move, type|
    mods[:base_accuracy] = 0 if move.damagingMove? && !["OHKO", "OHKOIce", "OHKOHitsUndergroundTarget"].include?(move.function)
  }
)
...
Battle::ItemEffects::DamageCalcFromUser.add(:GLACIALORB,
  proc { |item, user, target, move, mults, baseDmg, type|
    next if !(user.isSpecies?(:ARTICUNO) && user.form == 0)
...
# Battle::ItemEffects::DamageCalcFromUser.add(:VOLTAICORB,
#   proc { |item, user, target, move, mults, baseDmg, type|
#     if user.isSpecies?(:ZAPDOS) && user.form == 0 && move.physicalMove?
...
Battle::ItemEffects::AfterMoveUseFromTarget.add(:REDCARD,
  proc { |item, battler, user, move, switched_battlers, battle|
    ...
    if user.effects[PBEffects::Commander]
      battle.pbDisplay(_INTL("{1} cannot be forced out!", user.pbThis))
      next
    end
...
Battle::ItemEffects::AfterMoveUseFromUser.add(:LEECHINGROOT,
  proc { |item, user, targets, move, numHits, battle|
    next if !move.pbContactMove?(user)
    # New check for damage done
    totalDamage = 0
    oozeDamage = 0
    liquidOoze = false
    oozeUsers=[]
    targets.each do |b|
      if b.hasActiveAbility?(:LIQUIDOOZE)
        liquidOoze = true 
        oozeUsers.push(b)
        oozeDamage += b.damageState.totalHPLost if !b.hasActiveItem?(:PROTECTIVEPADS)
      else
        totalDamage += b.damageState.totalHPLost if !b.hasActiveItem?(:PROTECTIVEPADS) && user.canHeal?
      end
    end
    if totalDamage > 0
      hpGain = (totalDamage / 4.0).round
      user.pbRecoverHP(hpGain)
      battle.pbDisplay(_INTL("{1} drained HP using its {2}!",
       user.pbThis, user.itemName))
    end
    # Added Liquid Ooze check for extremely rare situation (pointless)
    if oozeDamage > 0
      hpLost = (oozeDamage / 4.0).round
      oozeUsers.each do |target|
        battle.pbShowAbilitySplash(target)
        battle.pbDisplay(_INTL("{1} sucked up the liquid ooze using its {2}!", user.pbThis, user.itemName))
        battle.pbHideAbilitySplash(target)
      end
      user.pbReduceHP(hpLost)
      user.pbItemHPHealCheck
    end
  }
)
...
Battle::ItemEffects::WeatherExtender.add(:SANDBOTTLE,
  proc { |item, weather, duration, battler, battle|
    next 8 if weather == :Sandstorm && battler.isSpecies?(:SANDSLASH) && battler.form == 0
  }
)

Battle::ItemEffects::WeatherExtender.add(:GLACIALORB,
  proc { |item, weather, duration, battler, battle|
    next -1 if weather == :FreezingRain && battler.isSpecies?(:ARTICUNO) && battler.form == 0
  }
)

Battle::ItemEffects::WeatherExtender.add(:ARDENTORB,
  proc { |item, weather, duration, battler, battle|
    next -1 if weather == :Sun && battler.isSpecies?(:MOLTRES) && battler.form == 0
  }
)
...
Battle::ItemEffects::OnSwitchIn.add(:GLACIALORB,
  proc { |item, battler, battle|
    next if !(battler.isSpecies?(:ARTICUNO) && battler.form == 0)
    if ![:HolyInferno, :HeavyRain, :HarshSun, :FrozenStorm].include?(battle.field.weather)
      #battle.pbCommonAnimation("UseItem", battler) # That sound effect was dumb for this item
      battle.pbDisplay(_INTL("{1}'s {2} brought forth a Freezing Rain!",
         battler.pbThis, battler.itemName))
      battle.pbStartWeather(battler, :FreezingRain)
    end
    next if ![:FreezingRain, :FrozenStorm, :Hail].include?(battle.field.weather)
    next if !battler.pbCanRaiseStatStage?(:SPEED)
    battle.pbDisplay(_INTL("{1}'s {2} resonates with the weather!",
       battler.pbThis, battler.itemName))
    battler.pbRaiseStatStage(:SPEED, 1, nil)
  }
)

Battle::ItemEffects::OnSwitchIn.add(:ARDENTORB,
  proc { |item, battler, battle|
    next if !(battler.isSpecies?(:MOLTRES) && battler.form == 0)
    if ![:HolyInferno, :HeavyRain, :HarshSun, :StrongWinds, :FrozenStorm].include?(battle.field.weather)
      #battle.pbCommonAnimation("UseItem", battler) # That sound effect was dumb for this item
      battle.pbDisplay(_INTL("{1}'s {2} brought forth intense Sunlight!",
        battler.pbThis, battler.itemName))
      battle.pbStartWeather(battler, :Sun)
    end
    next if ![:Sun, :HolyInferno, :HarshSun].include?(battle.field.weather)
    next if !battler.pbCanRaiseStatStage?(:SPEED)
    battle.pbDisplay(_INTL("{1}'s {2} resonates with the weather!",
       battler.pbThis, battler.itemName))
    battler.pbRaiseStatStage(:SPEED, 1, nil)
...
# DemICE: Placeholder equivalent to the above for the voltaic orb. This one does not invoke the weather.
# Depending on how often would Mordecai would find himself under a Frozen Storm with zapdos, 
# and if such a boost would work narratively in such an occasion it might not be that bad?

# Battle::ItemEffects::OnSwitchIn.add(:VOLTAICORB,
#   proc { |item, battler, battle|
#     next if !(battler.isSpecies?(:ZAPDOS) && battler.form == 0)
#     next if battle.field.weather != :FrozenStorm
#     next if !battler.pbCanRaiseStatStage?(:SPEED)
#     battle.pbDisplay(_INTL("{1}'s {2} resonates with the weather!",
#        battler.pbThis, battler.itemName))
#     battler.pbRaiseStatStage(:SPEED, 1, nil)
...
Battle::ItemEffects::OnSwitchIn.add(:SANDBOTTLE,
  proc { |item, battler, battle|
    next if !(battler.isSpecies?(:SANDSLASH) && battler.form == 0)
    next if [:HolyInferno, :HeavyRain, :HarshSun, :StrongWinds, :FrozenStorm].include?(battle.field.weather)
    battle.pbCommonAnimation("UseItem", battler)
    battle.pbDisplay(_INTL("{1} opened its {2}!",
       battler.pbThis, battler.itemName))
    # Start Sandstorm
