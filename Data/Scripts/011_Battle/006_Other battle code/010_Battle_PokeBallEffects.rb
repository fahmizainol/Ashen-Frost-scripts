...
# Ashen Frost Poke Balls

Battle::PokeBallEffects::ModifyCatchRate.add(:SOLARBALL, proc { |ball, catchRate, battle, battler|
  multiplier = (Settings::NEW_POKE_BALL_CATCH_RATES) ? 3 : 3.5
  catchRate *= multiplier if battle.time == 0   # Day
  next catchRate
})

Battle::PokeBallEffects::ModifyCatchRate.add(:SURGEBALL, proc { |ball, catchRate, battle, battler|
  multiplier = (Settings::NEW_POKE_BALL_CATCH_RATES) ? 3.5 : 3
  catchRate *= multiplier if battler.pbHasType?(:ELECTRIC)
  next catchRate
})

Battle::PokeBallEffects::ModifyCatchRate.add(:POLARITYBALL, proc { |ball, catchRate, battle, battler|
  multiplier = (Settings::NEW_POKE_BALL_CATCH_RATES) ? 3.5 : 3
  catchRate *= multiplier if battler.pbHasType?(:STEEL)
  next catchRate
})

Battle::PokeBallEffects::ModifyCatchRate.add(:FLAKEBALL, proc { |ball, catchRate, battle, battler|
  catchRate *= 3.5 if battler.effectiveWeather == :Hail
  next catchRate
})

Battle::PokeBallEffects::ModifyCatchRate.add(:DROPLETBALL, proc { |ball, catchRate, battle, battler|
  catchRate *= 3.5 if [:Rain, :HeavyRain, :FrozenStorm, :FreezingRain].include?(battler.effectiveWeather)
  next catchRate
})

Battle::PokeBallEffects::ModifyCatchRate.add(:FOGBALL, proc { |ball, catchRate, battle, battler|
  catchRate *= 3.5 if battler.effectiveWeather == :Fog
  next catchRate
})

Battle::PokeBallEffects::ModifyCatchRate.add(:SANDBALL, proc { |ball, catchRate, battle, battler|
  catchRate *= 3.5 if [:SolarWinds, :Sandstorm].include?(battler.effectiveWeather)
  next catchRate
})
