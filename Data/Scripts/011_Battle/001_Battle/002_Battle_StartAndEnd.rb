class Battle
  ...
  def pbEnsureParticipants
    ...
    #       sides.
     if trainerBattle? && (@sideSizes[0] > 2 || @sideSizes[1] > 2) &&
        @player.length > 1 && @opponent.length > 3
  ...
  def pbStartBattleSendOut(sendOuts)
    ...
    if wildBattle?
      ...
      when 3
        ...
        DiscordRPC.triple_wild(foeParty[0], foeParty[1], foeParty[2]) if defined?(DiscordRPC) && $PokemonSystem.discordrpc == 0 
      end
    else   # Trainer battle
  ...
  def pbStartBattleCore
    ...
    when :FreezingRain then pbDisplay(_INTL("Freezing rain is falling."))
    when :FrozenStorm  then pbDisplay(_INTL("A frozen thunderstorm has formed."))
    when :HolyInferno  then pbDisplay(_INTL("The surrounding area is engulfed in flames!"))
    when :SolarWinds   then pbDisplay(_INTL("A strange wind is blowing."))
    end
