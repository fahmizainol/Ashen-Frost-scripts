class Battle
  ...
  def pbGainExp
    ...
    @battlers.each do |b|
      ...
      b.participants.each do |partic|
        #next unless p1[partic]&.able? && pbIsOwner?(0, partic)  # Making fainted members earn exp.
        next if p1[partic]&.egg?
      ...
      if !expAll
        eachInTeam(0, 0) do |pkmn, i|
          next if pkmn.egg?
          #next if !pkmn.able? # Making fainted members earn exp.
  ...
  def pbLearnMove(idxParty, newMove)
    ...
    pkmn.add_first_move(newMove) # DemICE Move Remember
