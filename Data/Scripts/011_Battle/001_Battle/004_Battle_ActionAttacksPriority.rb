class Battle
  ...
  def pbCanChooseMove?(idxBattler, idxMove, showMessages, sleepTalk = false)
    ...
    # Battle Roulette Explosion check
    board_chosen = $game_switches[421] ? 0 : 1
    return false if $game_variables[401] == 12 && idxBattler == board_chosen && $game_switches[192]
  ...
  def pbAutoChooseMove(idxBattler, showMessages = true)
    ...
    # Explosion check for Battle Roulette
    if $game_variables[401] == 12 && $game_switches[192]
      @choices[idxBattler][0] = :UseMove    # "Use move"
      @choices[idxBattler][1] = -1          # Index of move to be used
      @choices[idxBattler][2] = Move.from_pokemon_move(self, Pokemon::Move.new(:EXPLOSION)) # Explosion Battle::Move object
      @choices[idxBattler][3] = -1          # No target chosen yet
      return true if singleBattle?
    end    
    # Struggle
      ...
      (0..maxBattlerIndex).each do |i|
        ...
        if @choices[b.index][0] == :UseMove || @choices[b.index][0] == :Shift
          ...
          if @choices[b.index][0] == :UseMove
            ...
            # Battle Roulette Priority Check
            board_chosen = $game_switches[421] ? 0 : 1
            if move.name == "Explosion" && $game_variables[401] == 12 && 
              $game_switches[192] && b.index == board_chosen
              pri = 12
              $game_switches[192] = false
            end
            entry[5] = pri
