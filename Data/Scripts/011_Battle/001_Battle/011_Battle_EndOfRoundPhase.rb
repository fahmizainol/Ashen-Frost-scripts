class Battle
  ...
  def pbEOREndWeather(priority)
    ...
    @field.weatherDuration -= 1 if @field.weatherDuration > 0 && @field.effects[PBEffects::ClearFlute] <= 0 #Ashen Frost Exclusive
    # Weather wears off
    if @field.weatherDuration == 0
      ...
      when :FreezingRain then pbDisplay(_INTL("The freezing rain stopped.")) 
      when :FrozenStorm  then pbDisplay(_INTL("The frozen thunderstorm lifted."))
      when :HolyInferno  then pbDisplay(_INTL("The holy inferno burned out."))
      when :SolarWinds   then pbDisplay(_INTL("The solar winds dissipated."))
      end
      @field.weather = :None
    ...
    pbCommonAnimation(weather_data.animation) if weather_data && @field.effects[PBEffects::ClearFlute] <= 0
    if @field.effects[PBEffects::ClearFlute] <= 0
      case @field.weather
      ...
      when :FreezingRain then pbDisplay(_INTL("Freezing rain continues to fall."))
      when :FrozenStorm  then pbDisplay(_INTL("The frozen thunderstorm rages on."))
      when :HolyInferno  then pbDisplay(_INTL("The holy inferno continues to burn."))
      when :SolarWinds   then pbDisplay(_INTL("The strange wind blows strong."))
      end
    end
    # Frozen thunderstorm warning message
    if @field.weather == :FrozenStorm && @field.effects[PBEffects::ClearFlute] <= 0
      case (@turnCount % 6)
      when 0
        # Account for possible double battle
        case @opponent.length
        when 1
          pbDisplay(_INTL("Electricity is gathering on {1}'s side of the field!", @opponent[0].name))
        when 2
          pbDisplay(_INTL("Electricity is gathering on {1} and {2}'s side of the field!", 
          @opponent[0].name, @opponent[1].name))
        end
      when 2
        pbDisplay(_INTL("Electricity is gathering on {1}'s side of the field!", pbPlayer.name))
      when 4
        pbDisplay(_INTL("Electricity is gathering on both sides of the field!"))
      end
    end
  ...
  def pbEORWeatherDamage(battler)
    ...
    when :ShadowSky
      ...
      amt = battler.totalhp / 16
    when :FreezingRain
      return if !battler.takesFreezingRainDamage?
      pbDisplay(_INTL("{1} is pelted by the freezing rain!", battler.pbThis))
      amt = battler.totalhp / 16
    when :FrozenStorm
      return if !battler.takesFrozenStormDamage?
      pbAnimation(:THUNDER, battler, nil)
      return if Battle::AbilityEffects.triggerEndOfRoundWeather(battler.ability, battler.effectiveWeather, battler, self)
      # Electric Damage check
      bTypes = battler.pbTypes(true)
      eff = Effectiveness.calculate(:ELECTRIC, bTypes[0], bTypes[1], bTypes[2])
      pbDisplay(_INTL("{1} was struck by lightning!", battler.pbThis))
      return if Battle::AbilityEffects.triggerEndOfRoundWeatherDamage(battler.ability, battler.effectiveWeather, battler, self)
      amt = [(battler.totalhp * eff / 32).floor, 1].max
    when :HolyInferno
      return if !battler.takesHolyInfernoDamage?
      pbAnimation(:SACREDFIRE, battler, nil)      
      # Fire Damage check
      bTypes = battler.pbTypes(true)
      eff = Effectiveness.calculate(:FIRE, bTypes[0], bTypes[1], bTypes[2])
      eff *= 2 if battler.hasActiveAbility?(:FLUFFY)
      pbDisplay(_INTL("{1} was engulfed in flames!", battler.pbThis))
      amt = [(battler.totalhp * eff / 32).floor, 1].max
    end
    return if amt < 0
  ...
  def pbEORCountDownFieldEffect(effect, msg)
    ...
    if effect == PBEffects::ClearFlute
      @field.effects[PBEffects::ClearFlute] = -1 if @field.effects[PBEffects::ClearFlute] <= 0
    end
    return if @field.effects[effect] > 0
    pbDisplay(msg)
  ...
  #=============================================================================
  # End Of Round Battle Roulette Stat Up/Down Effect
  #=============================================================================
  def pbEORBattleRouletteStatChanges(battler)
    # Effect added for Battle Roulette
    # $game_variables[401] == 5: random stat changes
    randomUp = []
    randomDown = []
    ...
    if randomUp.length > 0
      ...
      battler.pbRaiseStatStage(randomUp[r], 1, battler, false)
      randomDown.delete(randomUp[r])
    end
    if randomDown.length > 0
      r = pbRandom(randomDown.length)
      battler.pbLowerStatStage(randomDown[r], 1, battler, false)
    end
  ...
  def pbEndOfRoundPhase
    ...
    # Weather
    # Clear Flute Ashen Frost Exclusive
    pbEORCountDownFieldEffect(PBEffects::ClearFlute,
      _INTL("Clear Flute's effect wore off!"))
    ...
    priority.each do |battler|
      ...
      # Battle Roulette stat up/down effect
      if $game_variables[401] == 5 && $game_map.map_id == 488
        pbEORBattleRouletteStatChanges(battler)
      end
