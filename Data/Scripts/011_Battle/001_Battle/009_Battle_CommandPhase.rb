class Battle
  ...
  def pbItemMenu(idxBattler, firstAction)
    # Added Item Clause into battle
    if !@internalBattle || $game_switches[420]
      pbDisplay(_INTL("Items can't be used here."))
      return false
    elsif !wildBattle? && $PokemonGlobal.bag_ban == 0
      pbDisplay(_INTL("You have banned bag item usage from the options."))
      return false
    end
    ret = false
