...
class Battle
  ...
  attr_accessor :noforfeit        # True if player won't be able to forfeit
  attr_accessor :alwayslosemoney  # True if player loses money on forfeit
  ...
  attr_accessor :canScale         # Whether a Pokémon's level scales to the party
  attr_accessor :rules
  ...
  attr_reader   :defeatItems     # DemICE attempt to recover items upon defeat/forfeit
  ...
  def pbSwapBattlers(idxA, idxB)
    ...
    !(pbPlayerBattlerCount == 0 && pbOwnedByPlayer?(idxB) && pbGetOwnerIndexFromBattlerIndex(idxA) != 0)
  ...
  def pbWeather
    ...
    return :None if @field.effects[PBEffects::ClearFlute]>0
  ...
  def pbStartWeather(user, newWeather, fixedDuration = false, showAnim = true)
    ...
    when :FrozenStorm  then pbDisplay(_INTL("A frozen thunderstorm brews!"))
  ...
  def pbStartWeatherAbility(new_weather, battler, ignore_primal = false)
    return if !ignore_primal && [:HarshSun, :HeavyRain, :StrongWinds, :FrozenStorm, :HolyInferno].include?(@field.weather)
    ...
    fixed_duration = true if Settings::FIXED_DURATION_WEATHER_FROM_ABILITY &&
                             ![:HarshSun, :HeavyRain, :StrongWinds, :FrozenStorm, :HolyInferno].include?(new_weather)
