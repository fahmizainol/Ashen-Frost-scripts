...
def pbDisplayBankWindow(msgwindow,goldwindow)
  bankString = ($player) ? $game_variables[Settings::BALANCE].to_s_formatted : "0"
...
def pbMessageDisplay(msgwindow, message, letterbyletter = true, commandProc = nil)
  ...
  text.gsub!(/\\r/i,   "<c3=E00808,D0D0C8>")  #Ekat Note: I ALWAYS FORGET WHERE THESE MOTHERFUCKERS ARE
  text.gsub!(/\\[Ww]\[([^\]]*)\]/) {
    w = $1.to_s
    if w == ""
  ...
  textchunks = []
  controls = []
  while text[/(?:\\(f|ff|ts|cl|me|se|wt|wtnp|ch)\[([^\]]*)\]|\\(g|cn|pt|dp|wd|wm|op|cl|wu|\.|\||\!|\^))/i]
